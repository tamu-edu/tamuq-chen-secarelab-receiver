...
# ╔═╡ 7a30a4b8-9041-45b4-a9a9-a9c4a206ecdf
md"""
__Optimal L/D ratio is 30.10__
...
# ╔═╡ 0f08d1c7-f21d-4925-818a-e7164c552b52
a=[model(range(LD[1], LD[end], length=100),coef(fit_Eff[m])) 
					for m = 1:length(M)]
...
begin
	# fit_Tout = [curve_fit(model, LD, vec(Eff_Ap[:,m]), p0) for m = 1:length(M)]
	# fit_Tout = [Real.(model(range(LD[1], LD[end], length=100),coef(fit_Tout[m]))) 					for m = 1:length(M)]
