...
# ╔═╡ a5f97255-53d1-44df-9173-8348c4ba7f99
Lc
...
# ╔═╡ e33918cf-494c-4eee-9715-3ed22f58331a
Gz

# ╔═╡ f1348529-c476-4d50-b0da-82978b8d63ea
Re

# ╔═╡ b39ded9c-9a8d-473e-a4a4-ecd0b1941616
Pr
...
begin
	optprob = PEtab.OptimizationProblem(petab_problem)
	optsol = solve(optprob, NLopt.GN_MLSL_LDS(), local_method = NLopt.LD_LBFGS(), maxtime = 10.0, local_maxiters = 10000)
	optsol.u
end
...
sorted_keys = sort(collect(keys(simulation_conditions)))

# Ensure the keys are in the desired order
ordered_keys = sort(sorted_keys, by = x -> parse(Int, x[2:end]))

# Create the Select widget with the ordered keys
@bind casesim Select(ordered_keys)
end
...
begin
    ...
    Tend_e = []
    cond = []
    df_bar = DataFrame(simcond=String[], me=String[], T=Float64[])
    
    # Extract and sort the keys alphanumerically

    for simcond in sorted_keys
        v = simulation_conditions[simcond]
        
        # Assuming `get_odesol` and other functions are defined and work as expected
...
# ╔═╡ 12952912-af80-49a0-9351-708b8eee1885
"E67"
