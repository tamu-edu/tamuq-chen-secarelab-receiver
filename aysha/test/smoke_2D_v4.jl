# ============================================================================
# smoke_2D_v4.jl - Unit smoke test suite for 2D_v4.jl (SciML ODE Solver)
# ============================================================================

using Test

include(joinpath(@__DIR__, "..", "2D_v4.jl"))
using .Receiver2D_v4

@testset "2D_v4 SciML ODE Solver Smoke Test" begin
    p = default_parameters2D()
    grid = Receiver2D_v4.build_grid2D(p)

    @test grid.nz == 30
    @test length(grid.dz_cells) == 30
    @test grid.dz_cells[1] < grid.dz_cells[end] # Check front mesh refinement
