# test/smoke_1D_v49.jl
# Automated test suite for 1D_v49 Entire Converter Model, mesh sensitivity, and conservation audits.

using Test
include("../1D_v49.jl")

@testset "1D_v49 Basic Model & Parameter Tests" begin
    @test length(pnew_v49) == 28
    @test length(lb_full_v49) == 28
    @test length(ub_full_v49) == 28
    @test all(lb_full_v49 .<= pnew_v49 .<= ub_full_v49)
    
    # Check physical geometric properties
    @test isapprox(A_frt_v49, pi * R_core_v49^2, rtol=1e-5)
    @test isapprox(epsilon_v49, (100 * 0.0015^2) / A_frt_v49, rtol=1e-5)
    @test isapprox(A_solid_v49, (1.0 - epsilon_v49) * A_frt_v49, rtol=1e-5)
    @test isapprox(P_exchange_v49, 4 * 100 * 0.0015, rtol=1e-5)
    
    # Check participating assembly capacitance near 301 J/K
    C_total = participating_total_heat_capacity_v49(pnew_v49, 15)
    @test 250.0 < C_total < 350.0
    @test isapprox(C_total, 301.0, atol=25.0)
end

@testset "1D_v49 Forward Simulation & Conservation Residual (Heating E67)" begin
    outputs, result, experiment = solve_case_v49(pnew_v49, "E67"; nodes=15)
    ...
    conditions, time, _, _ = experimental_case_v49("E67")
    t_end = time[end]
    ...
    balance = compute_energy_balance_v49(result, operating, t_end, pnew_v49; nodes=15)
...
@testset "1D_v49 Mesh Sensitivity & Macroscopic HTC Convergence" begin
    ...
    for N in node_counts
        _, res, _ = solve_case_v49(pnew_v49, "E67"; nodes=N)
        idx = length(res.time)
        htc_val = macroscopic_htc_v49(res, idx)
...
@testset "1D_v49 Forward Simulation (Cooling C69 & C80)" begin
    # C69 (60 LPM forced cooling)
    outputs_c69, _, _ = solve_case_v49(pnew_v49, "C69"; is_cooling=true, nodes=15)
    @test all(isfinite, outputs_c69)
    println("Cooling C69 solved successfully.")
    
    # C80 (0 LPM natural cooling)
    outputs_c80, _, _ = solve_case_v49(pnew_v49, "C80"; is_cooling=true, nodes=15)
