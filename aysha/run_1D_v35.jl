# ============================================================================
# run_1D_v35.jl - Energy-accounting 2-Zone Core/Perimeter Runner
# ============================================================================
...
begin # runner configuration
    runner_int_v35(name, default) = parse(Int, get(ENV, name, string(default)))
    runner_float_v35(name, default) = parse(Float64, get(ENV, name, string(default)))
    runner_bool_v35(name, default) =
    ...
    const RUNNER_PLOT_DIR_v35 = joinpath(RUNNER_OUTPUT_DIR_v35, "plots")
    const RUNNER_TRANSIENT_DIR_v35 = joinpath(RUNNER_PLOT_DIR_v35, "transients")
    const RUNNER_AXIAL_DIR_v35 = joinpath(RUNNER_PLOT_DIR_v35, "axial_profiles")
    const RUNNER_DIAGNOSTIC_DIR_v35 = joinpath(RUNNER_OUTPUT_DIR_v35, "diagnostics")
    mkpath(RUNNER_OUTPUT_DIR_v35)
    mkpath(RUNNER_PLOT_DIR_v35)
    mkpath(RUNNER_TRANSIENT_DIR_v35)
    mkpath(RUNNER_AXIAL_DIR_v35)
    mkpath(RUNNER_DIAGNOSTIC_DIR_v35)
end

begin # runner helpers
    const RUNNER_SENSOR_COLORS_v35 = Dict(
        :T8 => :blue,
        :T12_perim => :red,
        :T11_perim => :green,
        :T9_core => :orange,
    ...
    const TEMPERATURE_AXIS_LIMITS_v35 = (280.0, 1400.0)

    function save_runner_plot_v35(plot_object, filename)
        path = joinpath(RUNNER_PLOT_DIR_v35, filename)
        savefig(plot_object, path)
        println("[run_1D_v35] Saved plot: $path")
        return path
    end

    scaled_incident_flux_w_m2_v35(irradiance, scale) =
        max(0.0, irradiance) * scale

    absorbed_receiver_power_watts_v35(irradiance, scale) =
        ETA_ABS_FIXED_v35 * scaled_incident_flux_w_m2_v35(irradiance, scale) * A_frt

    function write_parameters_v35(path, variant)
        ...
        open(path, "w") do io
            ...
            println(io, "# fitted,G_core_perim_W_m_K,$(core_perimeter_conductance_per_length_v35(parameters))")
            println(io, "# fitted,C_perim_eff_J_K,$(perimeter_heat_capacity_total_v35(parameters))")
            println(io, "# fitted,k_perim_ref_W_m_K,$(perimeter_axial_conductivity_v35(900.0, parameters))")
            println(io, "# fitted,spill_capture,$(perimeter_spillover_capture_v35(parameters))")
            println(io, "# derived,f_perim_source,$(perimeter_source_fraction_v35(parameters))")
            println(io, "# derived,flux_receiver_fraction,$(flux_receiver_fraction_v35())")
            println(io, "# derived,flux_spillover_fraction,$(flux_spillover_fraction_v35())")
            println(io, "# fitted,beta_perim_1_m,$(perimeter_source_attenuation_v35(parameters))")
            println(io, "# fitted,f_core_rear,$(rear_core_fraction_v35(parameters))")
            println(io, "# fitted,flange_scale,$(flange_loss_scale_v35(parameters))")
            println(io, "# fitted,flange_cool_gain,$(flange_cooling_gain_v35(parameters))")
            println(io, "# fitted,flange_cool_tau_s,$(flange_cooling_time_constant_v35(parameters))")
            println(io, "# fitted,k_core_axial_scale,$(core_axial_conduction_scale_v35(parameters))")
            println(io, "# fitted,C_rear_eff_J_K,$(rear_reservoir_heat_capacity_v35(parameters))")
            println(io, "# fitted,G_receiver_rear_W_K,$(receiver_rear_conductance_v35(parameters))")
            println(io, "# fitted,G_rear_tube_W_K,$(rear_tube_conductance_v35(parameters))")
            println(io, "# fitted,G_rear_cavity_W_K,$(rear_cavity_conductance_v35(parameters))")
            println(io, "# fitted,G_rear_axial_W_K,$(rear_axial_conductance_v35(parameters))")
            println(io, "# derived,C_core_eff_J_K,$(core_receiver_heat_capacity_v35(default_nodes))")
            println(io, "# derived,C_receiver_participating_eff_J_K,$(participating_assembly_heat_capacity_v35(parameters))")
            println(io, "# derived,C_total_with_rear_eff_J_K,$(participating_total_heat_capacity_v35(parameters))")
            println(io, "# reference,measured_C_eff_J_K,$MEASURED_ASSEMBLY_CAPACITANCE_v35")
        end
        return path
    end

    function compute_metrics_v35(p=pnew_v35; heating_keys=sim_key_heat,
                                cooling_keys=sim_key_cool, nodes=RUNNER_PLOT_NODES_v35)
        metrics = NamedTuple[]
        for (keys, cooling) in ((heating_keys, false), (cooling_keys, true))
            for simulation_id in keys
                outputs, result, experiment = solve_case_v35(
                ...
                for (j, sensor) in enumerate(sensors)
                    push!(metrics, (
                        ...
                        shape_loss=normalized_slope_mse_v35(outputs[:, j], experiment[:, j]),
                    ))
                end
            end
        end
    ...
    function write_metrics_v35(path, metrics)
        open(path, "w") do io
    ...
    function build_steady_results_v35(params=pnew_v35; keys=sim_key_heat, nodes=RUNNER_PLOT_NODES_v35)
        results = NamedTuple[]
        core_weights = solar_weights_v5(params[13], params[6], nodes)
        perim_weights = axial_exponential_weights_v35(
            perimeter_source_attenuation_v35(params), nodes, L / nodes,
        )
        for simulation_id in keys
            outputs, result, experiment = solve_case_v35(params, simulation_id; nodes=nodes)
            conditions = simulation_conditions[simulation_id]
            irradiance = conditions[Io]
            scale = absorbed_power_scale_v35(irradiance, params)
            nominal_flux = max(0.0, irradiance)
            scaled_flux = scaled_incident_flux_w_m2_v35(irradiance, scale)
            nominal_absorbed = absorbed_receiver_power_watts_v35(irradiance, 1.0)
            scaled_absorbed = absorbed_receiver_power_watts_v35(irradiance, scale)
            f_perim_source = perimeter_source_fraction_v35(params)
            core_absorbed = core_absorbed_power_v35(irradiance, params)
            perim_absorbed = perimeter_absorbed_power_v35(irradiance, params)
            participating_absorbed = modeled_participating_absorbed_power_v35(irradiance, params)
            push!(results, (
                simulation_id=simulation_id,
                ...
                spill_capture=perimeter_spillover_capture_v35(params),
                flux_receiver_fraction=flux_receiver_fraction_v35(),
                flux_spillover_fraction=flux_spillover_fraction_v35(),
                f_perim_source=f_perim_source,
                beta_perim_1_m=perimeter_source_attenuation_v35(params),
                ...
                G_core_perim_W_m_K=core_perimeter_conductance_per_length_v35(params),
                C_perim_eff_J_K=perimeter_heat_capacity_total_v35(params),
                C_receiver_participating_eff_J_K=participating_assembly_heat_capacity_v35(params, nodes),
                C_rear_eff_J_K=rear_reservoir_heat_capacity_v35(params),
                C_total_with_rear_eff_J_K=participating_total_heat_capacity_v35(params, nodes),
                f_core_rear=rear_core_fraction_v35(params),
                G_receiver_rear_W_K=receiver_rear_conductance_v35(params),
                G_rear_tube_W_K=rear_tube_conductance_v35(params),
                G_rear_cavity_W_K=rear_cavity_conductance_v35(params),
                G_rear_axial_W_K=rear_axial_conductance_v35(params),
                ...
                flange_scale=flange_loss_scale_v35(params),
                flange_cool_gain=flange_cooling_gain_v35(params),
                flange_cool_tau_s=flange_cooling_time_constant_v35(params),
                flange_gate_end=lamp_off_gate_v35(result.time[end], irradiance, params),
                effective_flange_scale_end=effective_flange_loss_scale_v35(params, result.time[end], irradiance),
                k_core_axial_scale=core_axial_conduction_scale_v35(params),
            ))
        end
        return results
    end

    function write_steady_results_v35(path, steady_results)
    ...
    function write_flow_slopes_v35(path, steady_results)
            ...
            for irradiance in irradiances
                ...
                for sensor in sensors
                    ...
                    println(io, join((
                        irradiance,
                        sensor,
                        slope_v35(flows, model_values),
                        slope_v35(flows, experiment_values),
                    ), ','))
                end
            end
        end
    ...
    function steady_comparison_plot_v35(steady_results; title_suffix="")
        sensors = (:T8, :T12_perim, :T11_perim, :T9_core, :T10_core, :T3, :T2)
        plot_object = plot(
            title="1D_v35 $title_suffix steady-state comparison",
            xlabel="Model temperature (K)",
            ylabel="Experiment temperature (K)",
            legend=:outerright,
            aspect_ratio=:equal,
            xlims=TEMPERATURE_AXIS_LIMITS_v35,
            ylims=TEMPERATURE_AXIS_LIMITS_v35,
        )
        for sensor in sensors
            ...
            scatter!(plot_object, model_values, experiment_values;
                     label=String(sensor), color=RUNNER_SENSOR_COLORS_v35[sensor],
                     ms=4, markerstrokewidth=0)
        end
        lower, upper = TEMPERATURE_AXIS_LIMITS_v35
        plot!(plot_object, [lower, upper], [lower, upper];
              label="1:1", color=:gray, linestyle=:dash)
        return plot_object
    end

    function axial_profile_plot_v35(simulation_id, params=pnew_v35;
                                   nodes=RUNNER_PLOT_NODES_v35,
                                   variant_name="v31")
        outputs, result, experiment = solve_case_v35(params, simulation_id; nodes=nodes)
        conditions = simulation_conditions[simulation_id]
        ...
        scale = absorbed_power_scale_v35(conditions[Io], params)
        title_text = "1D_v35 $variant_name axial profile: $simulation_id"
        ...
        plot_object = plot(
            ...
            legend=:outerright,
            grid=true,
            xlims=(0.0, 1000.0 * (L + REAR_TUBE_LENGTH_v35)),
            ylims=TEMPERATURE_AXIS_LIMITS_v35,
        )
        ...
        scatter!(plot_object, [T3_SAMPLE_POSITION_v35 * 1000.0], [experiment[end, 6]];
                 label="T3 experiment", color=:purple, markershape=:star5,
                 markerstrokewidth=0, ms=7)
        scatter!(plot_object, [CAVITY_LENGTH_v35 * 1000.0], [experiment[end, 7]];
                 label="T2 experiment", color=:black, markershape=:diamond,
                 markerstrokewidth=0, ms=5)
        scatter!(plot_object, [CAVITY_LENGTH_v35 * 1000.0], [outputs[end, 7]];
                 label="T2 model", color=:black, markershape=:xcross,
                 markerstrokewidth=1, ms=5)
        vline!(plot_object, [L * 1000.0, (L + REAR_TUBE_CAVITY_LENGTH_v35) * 1000.0];
               label=false, color=:gray, linestyle=:dot, lw=1)
        return plot_object
    end

    function plot_case_v35(simulation_id, params=pnew_v35; is_cooling=false,
                          nodes=RUNNER_PLOT_NODES_v35, variant_name="v31")
        outputs, result, experiment = solve_case_v35(params, simulation_id; is_cooling=is_cooling, nodes=nodes)
        sensors = (:T8, :T12_perim, :T11_perim, :T9_core, :T10_core, :T3, :T2)
        colors = (:blue, :red, :green, :orange, :brown, :purple, :black)
        
        plot_object = plot(
            title="1D_v35 $variant_name transient: $simulation_id",
    ...
    function write_cell_diagnostics_v35(path, simulation_id, params;
                                       nodes=RUNNER_PLOT_NODES_v35,
                                       variant_name="v31")
        outputs, result, experiment = solve_case_v35(params, simulation_id; nodes=nodes)
        conditions = simulation_conditions[simulation_id]
        scaled_absorbed = absorbed_receiver_power_watts_v35(
            conditions[Io], absorbed_power_scale_v35(conditions[Io], params),
        )
        f_perim_source = perimeter_source_fraction_v35(params)
        core_absorbed = core_absorbed_power_v35(conditions[Io], params)
        perim_absorbed = perimeter_absorbed_power_v35(conditions[Io], params)
        dx = L / length(result.z_solid)
        f_core_rear = rear_core_fraction_v35(params)
        G_receiver_rear = receiver_rear_conductance_v35(params)
        ...
        Qrear_cavity = [
            rear_cavity_conductance_v35(params) * result.rear_contact_weights[i] *
            (Trear[i] - Tcavity)
            for i in eachindex(result.z_solid)
        ]
        Qrear_tube_exit = rear_tube_conductance_v35(params) * (Trear[end] - Ttube_in)
        open(path, "w") do io
            ...
            for i in eachindex(result.z_solid)
                ...
                println(io, join((
                    ...
                    perimeter_axial_conductivity_v35(result.perim_temperature[i, end], params),
                    ...
                    core_perimeter_conductance_per_length_v35(params) *
                    dx *
                    ...
                    flange_loss_scale_v35(params),
                    flange_cooling_gain_v35(params),
                    flange_cooling_time_constant_v35(params),
                    core_axial_conduction_scale_v35(params),
                    rear_axial_conductance_v35(params),
                ), ','))
            end
        end
        return path
...
begin # execution & output generation
    println("[run_1D_v35] Running energy-accounting 2-Zone Core/Perimeter Runner.")
    println("[run_1D_v35] Running stage $(RUNNER_FIT_STAGE_v35): capacity-constrained rear contact rail.")

    fit = calibrate_v35(
        nodes=RUNNER_FIT_NODES_v35,
    ...
    open(joinpath(RUNNER_OUTPUT_DIR_v35, "optimization_summary_1D_v35.txt"), "w") do io
        println(io, "objective=$(fit.objective)")
        println(io, "return_code=$(fit.retcode)")
        println(io, "stage=$(RUNNER_FIT_STAGE_v35)")
        println(io, "write_plots=$(RUNNER_WRITE_PLOTS_v35)")
        println(io, "parameters=$(fit.parameters)")
    end

    variants_v35 = NamedTuple[
        (name="fitted_energy_accounting", params=fitted_params),
    ]

    summary_rows = NamedTuple[]
    for variant in variants_v35
        println("[run_1D_v35] Running variant $(variant.name).")
        write_parameters_v35(
            joinpath(RUNNER_OUTPUT_DIR_v35, "parameters_$(variant.name)_1D_v35.csv"),
            variant,
        )
        metrics = compute_metrics_v35(variant.params; nodes=RUNNER_PLOT_NODES_v35)
        write_metrics_v35(
            joinpath(RUNNER_OUTPUT_DIR_v35, "analysis_results_$(variant.name)_1D_v35.csv"),
            metrics,
        )
        for row in metrics
            push!(summary_rows, merge(row, (variant=variant.name,)))
        end
        steady_results = build_steady_results_v35(
            variant.params; nodes=RUNNER_PLOT_NODES_v35,
        )
        write_steady_results_v35(
            joinpath(RUNNER_OUTPUT_DIR_v35, "steady_results_$(variant.name)_1D_v35.csv"),
            steady_results,
        )
        write_flow_slopes_v35(
            joinpath(RUNNER_OUTPUT_DIR_v35, "flow_slopes_$(variant.name)_1D_v35.csv"),
            steady_results,
        )
        if RUNNER_WRITE_PLOTS_v35
            save_runner_plot_v35(
                steady_comparison_plot_v35(steady_results; title_suffix=variant.name),
                "steady_comparison_$(variant.name)_1D_v35.png",
            )
        end
        ...
        for simulation_id in axial_keys
            if RUNNER_WRITE_PLOTS_v35
                save_runner_plot_v35(
                    axial_profile_plot_v35(
                        simulation_id, variant.params; variant_name=variant.name,
                        nodes=RUNNER_PLOT_NODES_v35,
                    ),
                    joinpath("axial_profiles", "axial_profile_$(simulation_id)_$(variant.name)_1D_v35.png"),
                )
            end
            write_cell_diagnostics_v35(
                joinpath(RUNNER_DIAGNOSTIC_DIR_v35, "cell_diagnostics_$(simulation_id)_$(variant.name)_1D_v35.csv"),
                simulation_id, variant.params; variant_name=variant.name,
                nodes=RUNNER_PLOT_NODES_v35,
            )
        end
        ...
        if RUNNER_WRITE_PLOTS_v35
            for simulation_id in transient_heat_keys
                save_runner_plot_v35(
                    plot_case_v35(
                        ...
                        variant_name=variant.name, nodes=RUNNER_PLOT_NODES_v35,
                    ),
                    joinpath("transients", "transient_$(simulation_id)_$(variant.name)_1D_v35.png"),
                )
            end
            ...
            for simulation_id in transient_cool_keys
                save_runner_plot_v35(
                    ...
                    joinpath("transients", "transient_$(simulation_id)_$(variant.name)_cooling_1D_v35.png"),
                )
            end
        end
    end

    summary_path = joinpath(RUNNER_OUTPUT_DIR_v35, "analysis_results_all_variants_1D_v35.csv")
    ...
    println("[run_1D_v35] Optimization completed with objective: $(fit.objective)")
    println("[run_1D_v35] Fitted parameters: $fitted_params")
    println("[run_1D_v35] Complete.")
    println("[run_1D_v35] Output directory: $RUNNER_OUTPUT_DIR_v35")
    
    println("\n[run_1D_v35] Running manuscript invariant diagnostics...")
    include("diagnostics_1D_v35_manuscript_invariants.jl")
end
