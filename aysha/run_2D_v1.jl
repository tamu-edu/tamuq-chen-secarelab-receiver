# ============================================================================
# run_2D_v1.jl - 2D Axisymmetric Continuum Macro-ECM Model Calibration & Runner
# ============================================================================
# Calibrates 2D parameters via NLopt.LN_BOBYQA and outputs fitted results only:
...
begin # Directory setup
    const OUTPUT_DIR_2D_v1 = joinpath(@__DIR__, "summaries", "2D_v1")
    const PLOT_DIR_2D_v1 = joinpath(OUTPUT_DIR_2D_v1, "plots")
    const TRANSIENT_DIR_2D_v1 = joinpath(PLOT_DIR_2D_v1, "transients")
    const PROFILES_DIR_2D_v1 = joinpath(PLOT_DIR_2D_v1, "2d_profiles")
    const AXIAL_DIR_2D_v1 = joinpath(PLOT_DIR_2D_v1, "axial_profiles")
    const DIAGNOSTICS_DIR_2D_v1 = joinpath(OUTPUT_DIR_2D_v1, "diagnostics")
    
    mkpath(OUTPUT_DIR_2D_v1)
    mkpath(PLOT_DIR_2D_v1)
    mkpath(TRANSIENT_DIR_2D_v1)
    mkpath(PROFILES_DIR_2D_v1)
    mkpath(AXIAL_DIR_2D_v1)
    mkpath(DIAGNOSTICS_DIR_2D_v1)
...
function plot_steady_parity_2D(steady_results)
    p_plot = plot(
        title="2D_v1 Multi-Domain Fitted Steady-State Parity Plot",
    ...
    path = joinpath(PLOT_DIR_2D_v1, "steady_comparison_2D_v1.png")
    savefig(p_plot, path)
    return path
end
...
function plot_axial_profile_2D(sim_id, case_data)
    ...
    path = joinpath(AXIAL_DIR_2D_v1, "axial_profile_$(sim_id)_2D_v1.png")
    savefig(p_plot, path)
    return path
end
...
function plot_transient_2D(sim_id, case_data; is_cooling=false)
    ...
    p_plot = plot(
        title="2D_v1 Fitted Transient: $sim_id $(is_cooling ? "(Cooling)" : "(Heating)")",
    ...
    path = joinpath(TRANSIENT_DIR_2D_v1, "transient_$(sim_id)$(suffix)_2D_v1.png")
    savefig(p_plot, path)
    return path
end
...
function plot_2d_heatmap(sim_id, result)
    ...
    p_heat = heatmap(
        z_mm, r_mm, T_final,
        title="2D_v1 Multi-Domain Temperature Field T_s(r, z) [K] - $sim_id",
        xlabel="Axial Position z (mm)", ylabel="Radial Radius r (mm)",
        c=:inferno, aspect_ratio=:equal, clim=(minimum(T_final), maximum(T_final))
    )
    
    path = joinpath(PROFILES_DIR_2D_v1, "heatmap_2D_$(sim_id)_2D_v1.png")
    savefig(p_heat, path)
    return path
end
...
function write_parameters_2D(path, p::ModelParameters2D)
    names = (
        ...
        "phi_0", "m_rec", "front_dep_fraction", "flange_conductance_scale"
    )
...
begin # Main Execution Loop
    println("==========================================================================")
    println("Running Multi-Domain 2D_v1 High-Resolution Calibration & Optimization...")
    println("==========================================================================")
    ...
    open(joinpath(OUTPUT_DIR_2D_v1, "optimization_summary_2D_v1.txt"), "w") do io
        println(io, "objective=$(calib.objective)")
        println(io, "return_code=$(calib.retcode)")
        println(io, "parameters=$(pack_parameters2D(p_fitted))")
    end
    
    write_parameters_2D(joinpath(OUTPUT_DIR_2D_v1, "parameters_fitted_2D_v1.csv"), p_fitted)
    
    println("[run_2D_v1] Computing metrics and generating figures for fitted 2D model...")
    metrics = compute_all_metrics_2D(p_fitted)
    open(joinpath(OUTPUT_DIR_2D_v1, "analysis_results_2D_v1.csv"), "w") do io
    ...
    steady_path = joinpath(OUTPUT_DIR_2D_v1, "steady_results_2D_v1.csv")
    ...
    slopes_path = joinpath(OUTPUT_DIR_2D_v1, "flow_slopes_2D_v1.csv")
    ...
    for sim_id in IDs
        ...
        write_cell_diagnostics_2D(
            joinpath(DIAGNOSTICS_DIR_2D_v1, "cell_diagnostics_$(sim_id)_2D_v1.csv"),
    ...
    println("==========================================================================")
    println("Multi-Domain 2D_v1 Calibration and Runner Execution Complete.")
