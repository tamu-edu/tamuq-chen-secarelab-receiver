...
module Receiver2D_v22

include("2D_v20.jl")
include("2D_v22_Properties.jl")
const V20 = Receiver2D_v20
const Receiver2D_v22_Properties = Receiver2D_v22_Properties
const V19 = V20.V19
const V18 = V19.V18
const V17 = V19.V17
const V16 = V19.V16
...
export Phase2Parameters2D, ModelParameters2D_v22
export simulate2D_v22, apply_v22_property_fixes!

function apply_v22_property_fixes!()
    @eval V11 begin
        # 1. Corrected air properties
        air_conductivity(T) = Main.Receiver2D_v22.Receiver2D_v22_Properties.air_conductivity_v22(T)
        air_heat_capacity(T) = Main.Receiver2D_v22.Receiver2D_v22_Properties.air_heat_capacity_v22(T)
        air_viscosity(T) = Main.Receiver2D_v22.Receiver2D_v22_Properties.air_viscosity_v22(T)
        air_density(T, p=101325.0) = Main.Receiver2D_v22.Receiver2D_v22_Properties.air_density_v22(T, p)
        
        # 2. Corrected SiC properties
        sic_conductivity(T) = Main.Receiver2D_v22.Receiver2D_v22_Properties.sic_conductivity_v22(T)
        sic_heat_capacity(T) = Main.Receiver2D_v22.Receiver2D_v22_Properties.sic_heat_capacity_v22(T)
        
        # 3. Corrected felt properties
        felt_conductivity_temp(T, ref_k=0.06) = Main.Receiver2D_v22.Receiver2D_v22_Properties.felt_conductivity_v22(T)
        ...
        function HeatTransferParameters2D(;
            ...
            fully_developed_nusselt = Main.Receiver2D_v22.Receiver2D_v22_Properties.fully_developed_nusselt_v22(),
...
Base.@kwdef struct Phase2Parameters2D
    ...
    core_preference::Float64 = 0.6
    spillage_axial_spread_m::Float64 = 5.0e-3
end

Base.@kwdef struct Phase4Parameters2D
    nu_multiplier::Float64 = 1.0
    ross_multiplier::Float64 = 1.0
end

Base.@kwdef struct ModelParameters2D_v22
    base::V20.ModelParameters2D = V20.default_parameters2D()
    phase2::Phase2Parameters2D = Phase2Parameters2D()
    phase4::Phase4Parameters2D = Phase4Parameters2D()
end

function Base.getproperty(p::ModelParameters2D_v22, name::Symbol)
    name === :base && return getfield(p, :base)
    name === :phase2 && return getfield(p, :phase2)
    return getproperty(getfield(p, :base), name)
end

function _solve_channel_flows2D_v22!(
...
function _gas_profile_enthalpy2D_v22!(
    ...
    convergence = _solve_channel_flows2D_v22!(
        work, mdot_total, inlet, channel_temperature,
        p14, exchange, grid, phase2
    )
...
function _v22_ode2D!(du, u, context, time)
...
function simulate2D_v22(
    p::ModelParameters2D_v22,
    ...
    apply_v22_property_fixes!()
    
    context = V20._model_context2D(p.base, op)
    context_v22 = merge(context, (p=p,))
