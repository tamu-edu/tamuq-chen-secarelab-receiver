...
begin
    ...
    # ]
    eq1 = [
        (1 - e) * Vs * Dt(Ts(t, x)) ~ (A_frt * ks_f(Ts(t,x)) * Dxx(Ts(t, x)) - (h_avg_f(qlpm) * A_exchange * ((Ts(t, x)) - Tf(t, x))) .- 0.1 * (kins * (r_ins / r0) .* (Ts(t, x) .- Tins_f(t)) * A_s_p / (r_ins - r0)))/(aloss * ρs * Cps(Ts(t,x))),
        e * Vf * Dt(Tf(t, x)) ~ (Af * e * kf_f(Tf(t,x)) * Dxx(Tf(t, x)) - m * cpf_f(Tf(t,x)) * Dx(Tf(t, x)) + (h_avg_f(qlpm) * A_exchange * ((Ts(t, x)) - Tf(t, x))))/(ρf * cpf_f(Tf(t,x)))
    ]
    bcs1 = [
        Ts(0.0, x) ~ Tamb, # initial
        Tf(0.0, x) ~ Tamb, # initial
        -A_frt * ks_f(Ts(t,x)) * Dx(Ts(t, x_max1)) ~ 0.0, # far right
        -A_frt * ks_f(Ts(t,x)) * Dx(Ts(t, x_min1)) ~ ϵ * Io * A_chnl_frt_all - ϵ * σ * A_chnl_frt_all * (Ts(t, x_min1)^4 - Tamb^4) - hext * A_chnl_frt_all * (Ts(t, x_min1) - Tamb),  # far left
        -Af * kf_f(Tf(t,x)) * Dx(Tf(t, x_max1)) ~ 0.0, #-ρf * Cpf * V * A_ft * (Tf(t, x_max1) - Tamb), # exiting fluid
        -Af * kf_f(Tf(t,x)) * Dx(Tf(t, x_min1)) ~ m * Cpf * (Tf(t, x_min1) - Tamb) # entering fluid (upstream temperature)
    ]
