# ============================================================================
# 2D_v9.jl - Pressure-Informed Conservative 2D Axisymmetric Macro-ECM
# ============================================================================
# Corrections relative to v8:
...
function _gas_profile2D!(gas, qgas, hcell, density, velocity, reynolds_flow,
...
function receiver_ode2D!(du, u, context, time)
    ...
    _gas_profile2D!(
        work.gas, work.qgas, work.h, work.density, work.velocity,
...
function calibrate2D(solve_fn; heating_cases=("E67", "E76", "E80"),
    ...
    function counted_objective(theta_fit, p_data)
        ...
        if isfinite(val)
            println("[calibrate2D v9] Heating-Only Eval $(evals[]): loss = $(round(val, digits=4))")
            flush(stdout)
        end
        return val
    end
    ...
    println("[calibrate2D v9] Starting Heating-Only Parameter Optimization...")
