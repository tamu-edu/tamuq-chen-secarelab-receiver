# ============================================================================
# 2D_v7.jl - Multi-Domain 2D Model with Heating-Only Optimization & Radiative Transport
# ============================================================================
# Key Physics & Numerical Enhancements in v7:
#   1. Heating-Only Optimization Objective (Cooling as Benchmark):
#      Calibration optimizes ONLY heating cases (E67, E76, E80). Cooling cases (C69, C80, C81)
#      are simulated as uncalibrated validation benchmarks.
#   2. High-Temperature Axial Radiative Diffusion in Channels (16 σ T^3 / 3 β_rad):
#      drives heat deep into z = 80 - 137 mm (T10, T11, T3), lowering front T8 and raising rear T10/T3.
#   3. Deep Core-Perimeter Radial Gradient Preservation (T10 vs T11 Offset):
#      Anisotropic core vs perimeter contact resistance maintaining parallel experimental offset.
#   4. Fittable Thermophysical Property Multipliers (SiC k(T) slope, beta_rad, R_gap).
# ============================================================================

module Receiver2D_v7
...
Base.@kwdef struct Geometry2D
    ...
    nodes_z::Int = 30                           # Refined 30-cell non-uniform axial mesh
    rear_tube_length::Float64 = 150.0e-3        # Rear alumina exit tube length
    rear_tube_nodes::Int = 20                   # Rear tube axial nodes
end

Base.@kwdef struct SolidProperties2D
    density::Float64 = 2150.0                   # Dense SiC solid density [kg/m3]
    radial_conductivity_scale::Float64 = 0.008   # Effective radial conductivity scale
    axial_conductivity_scale::Float64 = 1.50    # Effective axial conductivity scale
    sic_k_temp_scale::Float64 = 1.00            # Fittable multiplier on SiC k(T)
    ...
    perim_gap_resistance::Float64 = 1.00        # Radial contact resistance factor at receiver boundary
end

Base.@kwdef struct HeatTransferParameters2D
    coefficient::Float64 = 2.80                 # Developing flow Nu prefactor A_Nu
    reynolds_exponent::Float64 = 0.400          # Reynolds exponent B_Re
    prandtl_exponent::Float64 = 0.333           # Prandtl exponent C_Pr
    minimum_nusselt::Float64 = 3.61             # Fully developed laminar Nu floor
    entry_enhancement::Float64 = 1.50           # Entry region convective Nu acceleration factor
    phi_0::Float64 = 1.00                       # Active flow fraction at Re = 50
    m_rec::Float64 = 0.20                       # Flow recruitment exponent
...
Base.@kwdef struct OpticalParameters2D
    ...
    scale_456::Float64 = 0.720                   # Calibrated power scale factor at 456 kW/m2
    scale_304::Float64 = 0.720                   # Calibrated power scale factor at 304 kW/m2
    scale_256::Float64 = 0.580                   # Calibrated power scale factor at 256 kW/m2
end
...
function sic_conductivity(T, scale=1.0)
    Tc = clamp_temperature(T) - 273.15
    val = scale * (1.0 / (1.10e-4 + 3.42e-6 * Tc))
    return max(5.0, val)
end
...
function build_initial_state_2D(grid::DiscretizationGrid2D, p::ModelParameters2D, t0_sensors::Dict{Symbol, Float64}, T_ambient::Float64)
    ...
    for j in 1:nz
        z_val = grid.z_centers[j]
        
        if z_val <= 34.5e-3
            frac = clamp(z_val / 34.5e-3, 0.0, 1.0)
            T_core_z = (1.0 - frac) * T8 + frac * T9
            T_perim_z = (1.0 - frac) * T8 + frac * T12
        else
            frac = clamp((z_val - 34.5e-3) / (107.0e-3 - 34.5e-3), 0.0, 1.0)
            T_core_z = (1.0 - frac) * T9 + frac * T10
            T_perim_z = (1.0 - frac) * T12 + frac * T11
        end

        for i in 1:nr_total
            r_val = grid.r_centers[i]
            if r_val <= R_rec
                frac_r = (r_val / R_rec)^2
                T_s0[i, j] = (1.0 - frac_r) * T_core_z + frac_r * T_perim_z
            else
                frac_out = clamp((r_val - R_rec) / (R_case - R_rec), 0.0, 1.0)
    ...
    for j in 1:nz_rear
        frac_z = clamp((j - 1) / max(1, nz_rear - 1), 0.0, 1.0)
        T_rear0[j] = (1.0 - frac_z) * T11 + frac_z * T3
    end
...
function cell_thermal_properties2D(i, T, p::ModelParameters2D, grid::DiscretizationGrid2D)
    if i <= grid.nr_rec
        k_val = p.solid.radial_conductivity_scale * sic_conductivity(T, p.solid.sic_k_temp_scale)
...
function _gas_profile2D!(gas, qgas, hcell, gas_rear, qgas_rear, hrear, solid_temp, T_rear, time, p, op, grid)
    ...
    for i in 1:nr_rec
        ...
        for j in 1:nz
            ...
            entry_factor = 1.0 + hp.entry_enhancement * (dh / max(grid.z_centers[j], dh / 2.0))^0.5
...
function receiver_ode2D!(du, u, context, time)
    ...
    for i in 1:(nr_total - 1), j in 1:nz
        ...
        # Perimeter boundary gap resistance
        if i == nr_rec
            kr_face /= max(1e-3, solid.perim_gap_resistance)
        end
    ...
    for i in 1:nr_total, j in 1:(nz - 1)
        ...
        kz_scale = i <= nr_rec ? p.solid.axial_conductivity_scale : 1.0
        ...
        kz_j = kz_scale * prop_j.k + k_rad_j
        kz_jp1 = kz_scale * prop_jp1.k + k_rad_jp1
    ...
    for j in 1:nz
        ...
        Qcasing_loss = 10.0 * (2.0 * pi * g.casing_outer_radius * dz_j) * (T_s[nr_total, j] - ambient) +
                       loss.cavity_emissivity * SIGMA * (2.0 * pi * g.casing_outer_radius * dz_j) * (T_case_clamped^4 - ambient^4)
...
function simulate2D(
    ...
    solver=Rodas5P(autodiff=AutoFiniteDiff(fdtype=Val(:forward))),
    reltol=1e-6,
    abstol=1e-7,
    dtmax=30.0
)
...
function pack_parameters2D(p::ModelParameters2D)
    return [
        ...
        p.optics.scale_304,
        p.optics.scale_256,
        p.heat_transfer.entry_enhancement,
        p.solid.sic_k_temp_scale,
        p.solid.rad_extinction_coeff,
        p.solid.perim_gap_resistance,
...
function unpack_parameters2D(theta::AbstractVector, p_base::ModelParameters2D=default_parameters2D())
    ...
    s = SolidProperties2D(
        ...
        sic_k_temp_scale = theta[12],
        rad_extinction_coeff = theta[13],
        perim_gap_resistance = theta[14],
    ...
    ht = HeatTransferParameters2D(
        ...
        entry_enhancement = theta[11],
        phi_0 = p_base.heat_transfer.phi_0,
        m_rec = p_base.heat_transfer.m_rec,
    ...
    opt = OpticalParameters2D(
        ...
        extinction_coefficient = theta[15],
...
const LB_2D = [0.5, 0.10, 0.003, 0.80, 5.0, 0.01, 100.0, 0.55, 0.55, 0.40, 0.50, 0.50, 20.0, 0.10, 80.0]
const UB_2D = [6.0, 0.70, 0.015, 2.50, 35.0, 0.40, 500.0, 1.20, 1.20, 1.00, 4.00, 2.00, 100.0, 5.00, 160.0]
    ...
    for sim_id in heating_cases
        try
            ...
            for j in 1:7
                ...
                total_loss += w_sensor * (nmse + 0.20 * shape + 0.10 * t90)
            end
            
            # Explicit Deep Core-Perimeter Offset Penalty (T10 - T11 parallel experimental distance)
            t10_t11_model = model[end, 5] - model[end, 3] # T10 - T11 model
            t10_t11_exp = exp[end, 5] - exp[end, 3]     # T10 - T11 exp
            total_loss += 1.00 * (t10_t11_model - t10_t11_exp)^2 / 50.0^2
...
function calibrate2D(solve_fn; heating_cases=("E67", "E76", "E80"),
    ...
    function counted_objective(theta, p_data)
        ...
        if isfinite(val)
            println("[calibrate2D v7] Heating-Only Eval $(evals[]): loss = $(round(val, digits=4))")
            flush(stdout)
        end
        return val
    end
    ...
    prob = OptimizationProblem(opt_prob, theta0, (heating_cases, solve_fn), lb=LB_2D, ub=UB_2D)
    
    println("[calibrate2D v7] Starting Heating-Only Parameter Optimization...")
