# ============================================================================
# 2D_v11.jl - Graetz / Equal-Pressure 2D Axisymmetric Macro-ECM
# ============================================================================
# Corrections relative to v10:
#   1. A local square-channel Graetz/Kays law with a finite downstream
#      asymptote replaces the apparent Re^B*(Dh/z)^(1/3) law when selected.
#   2. Ring mass flows can be solved from a common pressure drop while the
#      corrected MFC total mass flow remains exactly conserved.
#   3. A quadratic outlet loss represents partial obstruction by the measured
#      rear support groove outside the 13 mm free diameter. Channels remain
#      open and retain their full internal heat-transfer perimeter.
#   4. The v10 exposed-front exchange is disabled in the v11 nominal model.
#
...
Base.@kwdef struct HeatTransferParameters2D
    ...
    nusselt_model::Symbol = :graetz              # :graetz or historical :apparent
    fully_developed_nusselt::Float64 = 3.61      # Fixed square-duct asymptote
    temperature_correction_exponent::Float64 = 0.0 # Capuano alternative: 0.45
    heat_transfer_scale::Float64 = 1.0           # At most one order-unity discrepancy scale
end
...
Base.@kwdef struct HydraulicParameters2D
    ...
    flow_distribution_model::Symbol = :equal_pressure # :equal_pressure or :prescribed
    groove_free_diameter::Float64 = 13.0e-3    # Measured completely free diameter [m]
    groove_loss_coefficient::Float64 = 0.0     # Combined K/clearance^2 from cold t0 DP1
    equal_pressure_max_iterations::Int = 24
    equal_pressure_tolerance::Float64 = 1.0e-6
    equal_pressure_relaxation::Float64 = 0.55
end
...
struct SimulationResult2D
    ...
    gas_prandtl::Array{Float64, 3}             # (Nr_rec, Nz, Nt)
    gas_graetz::Array{Float64, 3}              # (Nr_rec, Nz, Nt)
    gas_nusselt::Array{Float64, 3}             # (Nr_rec, Nz, Nt)
    cell_pressure_drop::Array{Float64, 3}      # (Nr_rec, Nz, Nt) [Pa]
    ring_mass_flow_kg_s::Array{Float64, 2}     # (Nr_rec, Nt)
    ring_pressure_drop_Pa::Array{Float64, 2}   # (Nr_rec, Nt)
    groove_pressure_drop_Pa::Array{Float64, 2} # (Nr_rec, Nt)
    groove_overlap_fraction::Vector{Float64}   # (Nr_rec)
...
function groove_overlap_fractions2D(
    grid::DiscretizationGrid2D,
    p::ModelParameters2D,
)
    r_free = clamp(
        0.5 * p.hydraulics.groove_free_diameter,
        0.0,
        grid.r_faces[grid.nr_rec + 1],
    )
    overlap = zeros(grid.nr_rec)
    for i in 1:grid.nr_rec
        r0 = grid.r_faces[i]
        r1 = grid.r_faces[i + 1]
        annular_measure = r1^2 - r0^2
        exposed_measure = r1^2 - max(r0, r_free)^2
        overlap[i] = clamp(
            exposed_measure / max(annular_measure, eps(Float64)),
            0.0,
            1.0,
        )
    end
...
function graetz_nusselt2D(
    reynolds::Real,
    prandtl::Real,
    z::Real,
    dh::Real,
    hp::HeatTransferParameters2D;
    gas_temperature::Real=300.0,
    wall_temperature::Real=300.0,
)
    re = max(Float64(reynolds), eps(Float64))
    pr = max(Float64(prandtl), eps(Float64))
    gz = re * pr * Float64(dh) / max(Float64(z), eps(Float64))
    nu_base = hp.fully_developed_nusselt +
              0.104 * gz / (1.0 + 0.016 * gz^0.8)
    ratio = clamp(
        Float64(gas_temperature) / max(Float64(wall_temperature), 1.0),
        0.5,
        1.5,
    )
    nu = hp.heat_transfer_scale * nu_base *
         ratio^hp.temperature_correction_exponent
    return (nusselt=max(0.0, nu), graetz=gz)
end
...
function _initialize_ring_flows2D!(work, mdot_total, p, grid)
    nr_rec = grid.nr_rec
    if p.hydraulics.flow_distribution_model == :prescribed
        R_rec = grid.r_faces[nr_rec + 1]
        psi = [
            1.0 - p.heat_transfer.c_radial_flow *
            (grid.r_centers[i] / R_rec)^2 for i in 1:nr_rec
        ]
        normalization = sum(
            psi[i] * grid.area_flow[i] for i in 1:nr_rec
        )
        for i in 1:nr_rec
            work.ring_mdot[i] = mdot_total *
                psi[i] * grid.area_flow[i] / normalization
        end
    elseif p.hydraulics.flow_distribution_model == :equal_pressure
        total_area = sum(grid.area_flow[1:nr_rec])
        for i in 1:nr_rec
            work.ring_mdot[i] = mdot_total *
                grid.area_flow[i] / total_area
        end
    else
        error(
            "unknown flow_distribution_model: " *
            string(p.hydraulics.flow_distribution_model),
        )
    end
    return nothing
end

function _march_receiver_ring2D!(
    work,
    i,
    mdot_ring,
    inlet,
    ...
    work.front_h[i] = nu_front * k_in / dh
    front_ntu = work.front_h[i] * grid.area_solid[i] /
                max(mdot_ring * cp_in, eps(Float64))
    front_effectiveness = -expm1(-front_ntu)
    work.gas[i, 1] = inlet +
        front_effectiveness * (solid_temp[i, 1] - inlet)
    work.front_qgas[i] = mdot_ring * cp_in *
        (work.gas[i, 1] - inlet)

    for j in 1:nz
        dz_j = grid.dz_cells[j]
        Tgas_in = clamp_temperature(work.gas[i, j])
        Twall = clamp_temperature(solid_temp[i, j])

        if hp.nusselt_model == :apparent
            Tproperty = clamp_temperature(0.5 * (Twall + Tgas_in))
            cp = air_heat_capacity(Tproperty)
            k_pr = air_conductivity(Tproperty)
            k_h = k_pr
            mu = air_viscosity(Tproperty)
            reynolds = mdot_ring * dh / (area_ring * mu)
            prandtl = cp * mu / k_pr
            entry_factor = (
                dh / max(grid.z_centers[j], dh / 2.0)
            )^(1.0 / 3.0)
            nu_dev = hp.coefficient *
                     max(reynolds, eps(Float64))^hp.reynolds_exponent *
                     max(prandtl, eps(Float64))^hp.prandtl_exponent *
                     entry_factor
            nusselt = max(hp.minimum_nusselt, nu_dev)
            graetz = reynolds * prandtl * dh /
                      max(grid.z_centers[j], eps(Float64))
        elseif hp.nusselt_model == :graetz
            # Cornejo/Hayes convention: mu and cp at local bulk-gas
            # temperature, Nu conductivity at the local wall temperature.
            cp = air_heat_capacity(Tgas_in)
            k_pr = air_conductivity(Tgas_in)
            k_h = air_conductivity(Twall)
            mu = air_viscosity(Tgas_in)
            reynolds = mdot_ring * dh / (area_ring * mu)
            prandtl = cp * mu / k_pr
            correlation = graetz_nusselt2D(
                reynolds,
                prandtl,
                grid.z_centers[j],
                dh,
                hp;
                gas_temperature=Tgas_in,
                wall_temperature=Twall,
            )
            nusselt = correlation.nusselt
            graetz = correlation.graetz
        else
            error("unknown nusselt_model: " * string(hp.nusselt_model))
        end

        work.prandtl[i, j] = prandtl
        work.graetz[i, j] = graetz
        work.nusselt[i, j] = nusselt
        work.h[i, j] = nusselt * k_h / dh
        ua = work.h[i, j] * grid.perimeter_ex[i] * dz_j
        effectiveness = -expm1(
            -ua / max(mdot_ring * cp, eps(Float64)),
        )
        work.gas[i, j + 1] = Tgas_in +
            effectiveness * (Twall - Tgas_in)
        work.qgas[i, j] = mdot_ring * cp *
            (work.gas[i, j + 1] - Tgas_in)

        Tbulk = clamp_temperature(
            0.5 * (work.gas[i, j] + work.gas[i, j + 1]),
        )
        rho_bulk = air_density(
            Tbulk,
            hydraulics.atmospheric_pressure,
        )
        mu_bulk = air_viscosity(Tbulk)
        u_bulk = mdot_ring / (rho_bulk * area_ring)
        re_bulk = rho_bulk * u_bulk * dh / mu_bulk
        work.density[i, j] = rho_bulk
        work.velocity[i, j] = u_bulk
        work.reynolds[i, j] = re_bulk
        work.dp_cell[i, j] =
            hydraulics.hydraulic_resistance_scale *
            0.5 * SQUARE_DUCT_DARCY_POISEUILLE *
            mu_bulk * dz_j * u_bulk / dh^2
    end

    rho_out = work.density[i, end]
    u_out = work.velocity[i, end]
    groove_dp = work.groove_overlap[i] *
                max(0.0, hydraulics.groove_loss_coefficient) *
                0.5 * rho_out * u_out^2
    work.groove_dp[i] = groove_dp
    work.dp_cell[i, end] += groove_dp

    # Retained only for historical apparent-law comparisons. The physical
    # groove branch sets this coefficient to zero to avoid double counting.
    if hydraulics.minor_loss_coefficient != 0.0
        rho_ref = air_density(
            hydraulics.standard_temperature,
            hydraulics.atmospheric_pressure,
        )
        u_ref = mdot_ring / (rho_ref * area_ring)
        dynamic_excess = 0.5 * (
            rho_out * u_out^2 - rho_ref * u_ref^2
        )
        work.dp_cell[i, end] +=
            hydraulics.minor_loss_coefficient * dynamic_excess
    end
    work.ring_dp[i] = sum(view(work.dp_cell, i, :))
    return nothing
end

function _solve_receiver_rings2D!(
    work,
    mdot_total,
    inlet,
    solid_temp,
    ...
    _initialize_ring_flows2D!(work, mdot_total, p, grid)
    nr_rec = grid.nr_rec
    hydraulics = p.hydraulics

    if hydraulics.flow_distribution_model == :prescribed
        for i in 1:nr_rec
            _march_receiver_ring2D!(
                work, i, work.ring_mdot[i], inlet,
                solid_temp, p, grid,
            )
        end
        dp_mean = sum(
            work.ring_mdot[i] * work.ring_dp[i] for i in 1:nr_rec
        ) / mdot_total
        work.common_dp = dp_mean
        work.equal_pressure_error = maximum(
            abs(work.ring_dp[i] - dp_mean) for i in 1:nr_rec
        ) / max(dp_mean, eps(Float64))
        return nothing
    end

    relaxation = clamp(
        hydraulics.equal_pressure_relaxation, 0.05, 1.0,
    )
    tolerance = max(
        hydraulics.equal_pressure_tolerance, 1.0e-10,
    )
    max_iterations = max(
        hydraulics.equal_pressure_max_iterations, 1,
    )
    ...
    for iteration in 1:max_iterations
        ...
        dp_mean = sum(work.ring_dp) / nr_rec
        relative_error = maximum(
            abs(work.ring_dp[i] - dp_mean) for i in 1:nr_rec
        ) / max(dp_mean, eps(Float64))
        work.common_dp = dp_mean
        work.equal_pressure_error = relative_error
        relative_error <= tolerance && return nothing

        conductance = [
            work.ring_mdot[i] /
            max(work.ring_dp[i], eps(Float64)) for i in 1:nr_rec
        ]
        conductance_sum = sum(conductance)
        candidate = mdot_total .* conductance ./ conductance_sum
        for i in 1:nr_rec
            work.ring_mdot[i] =
                (1.0 - relaxation) * work.ring_mdot[i] +
                relaxation * candidate[i]
        end
        work.ring_mdot .*= mdot_total / sum(work.ring_mdot)
    end

    # Store fields at the final conserved iterate.
    for i in 1:nr_rec
        _march_receiver_ring2D!(
            work, i, work.ring_mdot[i], inlet,
            solid_temp, p, grid,
    ...
    work.common_dp = sum(work.ring_dp) / nr_rec
    work.equal_pressure_error = maximum(
        abs(work.ring_dp[i] - work.common_dp) for i in 1:nr_rec
    ) / max(work.common_dp, eps(Float64))
    return nothing
end

function _gas_profile2D!(
    work,
    solid_temp,
    T_rear,
    time,
    ...
    if flow_lpm <= 1e-12
        ...
        fill!(work.ring_mdot, 0.0)
        fill!(work.ring_dp, 0.0)
        ...
        work.gas .= inlet
        work.density .= air_density(
            inlet, p.hydraulics.atmospheric_pressure,
        )
        work.gas_rear .= inlet
        return nothing
    end

    _solve_receiver_rings2D!(
        work, mdot_total, inlet, solid_temp, p, grid,
    )

    total_enthalpy = sum(
        work.ring_mdot[i] * air_heat_capacity(work.gas[i, end]) *
        work.gas[i, end] for i in 1:nr_rec
    )
    total_mcp = sum(
        work.ring_mdot[i] * air_heat_capacity(work.gas[i, end])
        for i in 1:nr_rec
    ...
    work.gas_rear[1] =
        total_mcp > 0 ? total_enthalpy / total_mcp : inlet
    ...
    for j in 1:nz_rear
        Tfilm = clamp_temperature(
            0.5 * (T_rear[j] + work.gas_rear[j]),
        )
        cp = air_heat_capacity(Tfilm)
        kf = air_conductivity(Tfilm)
        mu = air_viscosity(Tfilm)
        ...
        work.hrear[j] = nusselt * kf / r_tube_dh
        ua = work.hrear[j] * r_tube_perim * grid.dz_rear
        effectiveness = -expm1(-ua / max(mdot_total * cp, eps(Float64)))
        
        work.gas_rear[j + 1] = work.gas_rear[j] +
            effectiveness * (T_rear[j] - work.gas_rear[j])
...
function receiver_ode2D!(du, u, context, time)
    ...
    _gas_profile2D!(
        work, T_s[1:nr_rec, :], T_rear, time, p, op, grid,
    )
    fill!(du_s, 0.0)
    fill!(du_rear, 0.0)
...
mutable struct Workspace2D
    ...
    prandtl::Array{Float64, 2}
    graetz::Array{Float64, 2}
    nusselt::Array{Float64, 2}
    dp_cell::Array{Float64, 2}
    ring_mdot::Vector{Float64}
    ring_dp::Vector{Float64}
...
function Workspace2D(grid::DiscretizationGrid2D, p::ModelParameters2D)
    return Workspace2D(
        ...
        zeros(grid.nr_rec),
        zeros(grid.nr_rec),
        groove_overlap_fractions2D(grid, p),
        0.0,
        0.0,
...
function hydraulic_summary2D(work::Workspace2D, p::ModelParameters2D,
    ...
    flow_lpm = max(0.0, Float64(op.flow_lpm(time)))
    mdot_total = standard_mass_flow2D(flow_lpm, p.hydraulics)
    receiver_dp_mbar = mdot_total <= eps(Float64) ?
        0.0 : work.common_dp / 100.0
    return (
...
function unpack_parameters2D(theta::AbstractVector, p_base::ModelParameters2D=default_parameters2D())
    ...
    ht = HeatTransferParameters2D(
        ...
        nusselt_model = p_base.heat_transfer.nusselt_model,
        fully_developed_nusselt =
            p_base.heat_transfer.fully_developed_nusselt,
        temperature_correction_exponent =
            p_base.heat_transfer.temperature_correction_exponent,
        heat_transfer_scale =
            p_base.heat_transfer.heat_transfer_scale,
    )
    l = LossParameters2D(
...
function calibrate2D(solve_fn; heating_cases=("E67", "E76", "E80"),
    ...
    function counted_objective(theta_fit, p_data)
        ...
        if isfinite(val)
            println("[calibrate2D v11] Heating-Only Eval $(evals[]): loss = $(round(val, digits=4))")
            flush(stdout)
        end
        return val
    end
    ...
    println("[calibrate2D v11] Starting Heating-Only Parameter Optimization...")
