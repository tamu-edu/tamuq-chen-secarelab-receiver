...
const INVARIANT_OUTPUT_DIR_1D = joinpath(@__DIR__, "summaries", "1D_v31")
...
function tube_loss_totals_1D(result, p, time, irradiance)
    ...
    dx_rear = REAR_TUBE_LENGTH_v31 / length(z_local)
    ...
    for j in eachindex(z_local)
        ...
        G_flange = rear_tube_flange_conductance_per_length_v31(
            z_local[j], Ttube[j], p, time, irradiance,
        )
        tube_to_cavity += G_cavity * dx_rear * (Ttube[j] - Tcavity)
        tube_to_flange += G_flange * dx_rear * (Ttube[j] - WATER_FLANGE_TEMPERATURE_v31)
    end
...
function rear_rail_heat_totals_1D(result, p)
    ...
    Qrear_tube = rear_tube_conductance_v31(p) * (Trear[end] - Ttube_in)
    return Qcore_rear, Qperim_rear, Qrear_tube, Qrear_cavity
end

function invariant_row_1D(simulation_id, p=pnew_v31; nodes=default_nodes)
    ...
    Tfront = Tperim[1]
    q_front = H_FRONT_FIXED_v31 * A_frt * (Tfront - ambient) +
              EPS_FRONT_FIXED_v31 * sigma_sb * A_frt * (Tfront^4 - ambient^4)
    q_cavity_ambient = cavity_loss_to_ambient_v31(result.cavity_temperature[end], ambient)
    ...
    return (
        ...
        power_scale=absorbed_power_scale_v31(irradiance, p),
        ...
        C_receiver_participating_J_K=participating_assembly_heat_capacity_v31(p, nodes),
        C_total_with_rear_J_K=participating_total_heat_capacity_v31(p, nodes),
...
function write_summary_1D(path, rows)
    ...
    open(path, "w") do io
        ...
        receiver_capacity = participating_assembly_heat_capacity_v31(pnew_v31)
        total_capacity = participating_total_heat_capacity_v31(pnew_v31)
...
rows = [invariant_row_1D(simulation_id, pnew_v31) for simulation_id in sim_key_heat]
write_rows_1D(joinpath(INVARIANT_OUTPUT_DIR_1D, "invariants_1D_v31.csv"), rows)
write_summary_1D(joinpath(INVARIANT_OUTPUT_DIR_1D, "invariant_summary_1D_v31.csv"), rows)

println("[diagnostics_1D_manuscript_invariants] wrote summaries/1D_v31/invariants_1D_v31.csv")
println("[diagnostics_1D_manuscript_invariants] wrote summaries/1D_v31/invariant_summary_1D_v31.csv")
