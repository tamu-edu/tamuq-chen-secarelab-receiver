# ============================================================================
# run_2D_v14.jl - shared v14 experiment loader and parameter builders
# ============================================================================
...
const V12_v14run = Receiver2D_v14.V12
const V11_v14run = Receiver2D_v14.V11
const OUTPUT_DIR_2D_v14 = joinpath(@__DIR__, "summaries", "2D_v14")
const HEATING_IDS_2D_v14 = Tuple(IDs)
const COOLING_IDS_2D_v14 = ("C69", "C80", "C81")
const TRAIN_HEATING_2D_v14 = (
    "E67", "E69", "E71", "E72", "E74", "E76",
    "E77", "E79", "E81",
)
const VALID_HEATING_2D_v14 = (
    "E68", "E70", "E73", "E75", "E78", "E80",
)
const SENSOR_NAMES_2D_v14 = (
    :T8, :T12, :T11, :T9, :T10, :T3, :T2,
)

function geometry_2D_v14(; nominal_mesh=true, screen_mesh=false)
    g0 = Geometry2D()
    if screen_mesh
        return Geometry2D(
            length=g0.length,
...
function inherited_parameters_2D_v14(;
    nominal_mesh=true,
    screen_mesh=false,
    network=NetworkParameters2D(),
)
    geometry = geometry_2D_v14(
    ...
    hydraulics = HydraulicParameters2D(
        ...
        groove_loss_coefficient=335.0,
        equal_pressure_max_iterations=24,
        equal_pressure_tolerance=1.0e-5,
        equal_pressure_relaxation=0.55,
    )
    base11 = V11_v14run.ModelParameters2D(
    ...
    base12 = V12_v14run.ModelParameters2D(
        base=base11, assembly=assembly, observation=observation,
    )
    return ModelParameters2D(base=base12, network=network)
end

function rebuild_network_2D_v14(
    p::ModelParameters2D;
    lateral_scale=p.network.lateral_conductivity_scale,
    edge_felt_scale=p.network.edge_felt_contact_scale,
)
    network = NetworkParameters2D(
        lateral_conductivity_scale=lateral_scale,
        edge_felt_contact_scale=edge_felt_scale,
        groove_quadrature_points=
            p.network.groove_quadrature_points,
    )
    return ModelParameters2D(base=p.base, network=network)
end

function rebuild_heating_2D_v14(
    p::ModelParameters2D;
    beta_opt=p.optics.extinction_coefficient,
    beam_sigma=p.optics.beam_radius_sigma,
    ...
    return ModelParameters2D(
        base=base12, network=p.network,
    )
end

function rebuild_hydraulics_2D_v14(
    p::ModelParameters2D;
    resistance_scale=
        p.hydraulics.hydraulic_resistance_scale,
    groove_coefficient=
        p.hydraulics.groove_loss_coefficient,
    dp1_offset=p.hydraulics.dp1_zero_offset_mbar,
)
    h0 = p.hydraulics
    hydraulics = HydraulicParameters2D(
        ...
        dp1_zero_offset_mbar=dp1_offset,
        hydraulic_resistance_scale=resistance_scale,
        minor_loss_coefficient=h0.minor_loss_coefficient,
        flow_distribution_model=h0.flow_distribution_model,
        groove_free_diameter=h0.groove_free_diameter,
        groove_loss_coefficient=groove_coefficient,
        equal_pressure_max_iterations=
            h0.equal_pressure_max_iterations,
        equal_pressure_tolerance=h0.equal_pressure_tolerance,
        equal_pressure_relaxation=h0.equal_pressure_relaxation,
    )
    b0 = p.base.base
    base11 = V11_v14run.ModelParameters2D(
        b0.geometry, b0.solid, b0.heat_transfer, b0.losses,
        b0.optics, hydraulics,
    )
    base12 = V12_v14run.ModelParameters2D(
        base=base11, assembly=p.base.assembly,
        observation=p.base.observation,
...
function _history_2D_v14(data, id, suffix)
    return Float64.(observation(data, id, suffix))
end

function case_inputs_2D_v14(id; cooling=false, max_points=nothing)
    ...
    flow = _history_2D_v14(data, id, "_flow")[indices]
    inlet = _history_2D_v14(data, id, "_Tin")[indices]
    ambient = _history_2D_v14(data, id, "_Tamb")[indices]
    observed = hcat((
        _history_2D_v14(data, id, "_$(sensor)")[indices]
        for sensor in SENSOR_NAMES_2D_v14
    )...)
    nominal = Float64(conditions[id][Io])
    dp1 = _history_2D_v14(data, id, "_DP1")[indices]
    return (
        id=id, cooling=cooling, data=data, times=times,
        flow=flow, inlet=inlet, ambient=ambient,
        observed=observed, nominal=nominal, dp1=dp1,
    )
end

function simulate_case_2D_v14(
    inputs,
    p::ModelParameters2D;
    ...
    op = OperatingCondition2D(
        ...
        ambient_temperature=linear_history(
    ...
    initial = if inputs.cooling && full_initial_data
        ...
        grid = V11_v14run.build_grid2D(p.base.base)
...
function case_metrics_2D_v14(case)
    rows = NamedTuple[]
...
function _write_namedtuples_csv_2D_v14(path, rows)
    isempty(rows) && return
