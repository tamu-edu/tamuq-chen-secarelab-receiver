@0# Theory Manual: 5-Node Lumped Parameter Solar Receiver Model (v3 - Algebraic NTU Gas Node)

This document provides a comprehensive, technically rigorous description of the mathematical formulation, geometric parameters, boundary conditions, and evaluation metrics of the updated 0D lumped parameter model (`0D_v3.jl`). This documentation is formatted to be suitable for direct inclusion in a scientific thesis or academic journal manuscript.
...
In this model version (`v3`), the physical thermal masses of the solid domains are modeled as separate lumped temperature nodes to capture their transient heat storage capacities. However, because the fluid residence time in the channels is orders of magnitude shorter than the solid thermal response, the channel fluid is modeled as an algebraic **Number of Transfer Units (NTU)** element rather than a dynamic node. This structure represents idealized plug flow for heat extraction.
...
*   $Q_{fluid} = \dot{m} C_{p,f}(T_s) (T_{f,out} - T_{amb})$ is the exact enthalpy rise of the fluid extracted from the solid.
...
### 2.2 Algebraic NTU Gas Node ($T_{f,out}$)
Because the fluid residence time is nearly instantaneous relative to the solid transients, the gas outlet temperature is resolved algebraically via the NTU formulation rather than dynamically:
$$T_{f,out} = T_s - (T_s - T_{amb}) \exp\left(-\frac{h_{avg,f}(T_s) A_{exchange}}{\dot{m} C_{p,f}(T_s)}\right)$$

Where:
*   $\dot{m} = \rho_f(T_{amb}) \cdot \dot{V}_{lpm}$ is the mass flow rate (kg/s).
*   The effective channel heat transfer coefficient $h_{avg,f}$ is evaluated at the solid temperature $T_s$.
...
The heat transfer correlation coefficients ($A, B, C$) are estimated by minimizing the average normalized Mean Squared Error (MSE) between the simulated and experimental transient temperature responses for the core solid and exit gas.

### 6.1 Objective Function
The loss function for a single experimental campaign normalizes the squared error by the square of the total experimental temperature rise ($\Delta T_{exp}$), avoiding disproportionate weighting of experiments with larger absolute temperatures or varying sample sizes:
$$J_i = \frac{1}{N_{samples}} \left( \sum_{j}^{N} \frac{(T_{s,j}^{mod} - T_{s,j}^{exp})^2}{\Delta T_{s,exp}^2} + \sum_{j}^{N} \frac{(T_{f,j}^{mod} - T_{f,j}^{exp})^2}{\Delta T_{f,exp}^2} \right)$$
The overall objective is the arithmetic mean of $J_i$ across all 15 calibration experiments.

### 6.2 Fitted Parameters
Using the NLopt algorithm (MLSL with Nelder-Mead local optimizer), the final converged parameters for `0D_v3` (algebraic NTU model) are:
...
$$Nu = 10^{-1.306} \cdot Re^{1.256} \cdot Pr^{10^{0.655}} \approx 0.049 \cdot Re^{1.256} \cdot Pr^{4.52}$$

The super-linear dependence on the Reynolds number ($Re^{1.256}$) confirms convective behaviors characteristic of developing flow in short micro-channels. Using the normalized loss surface, the optimizer leverages the Prandtl exponent ($Pr^{4.52}$) to aggressively tune the pre-exponential magnitude scaling while preserving sensitivity to fluid property variations with temperature.
