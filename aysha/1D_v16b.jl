# ============================================================================
# 1D_v16b.jl - active/wall receiver with active ETC and T9/T10 constraints
# ============================================================================
# v16b keeps the v8b rear-tube/flange, predicted cavity/T2 structure, v12a
...
# thermocouple topology: T8/T12/T11 are wall targets; T9/T10 are active,
# interior flow-exposed targets.
#
# Geometry updates supplied for v16b:
...
# available as optional diagnostics, but the default v16b audit keeps them off.
#
...
begin # v16b fixed constants
    const EPS_FRONT_FIXED_v16b = EPS_FRONT_FIXED_V5
    const ETA_ABS_FIXED_v16b = ETA_ABS_FIXED_V5
    const BETA_OPT_FIXED_v16b = BETA_OPT_FIXED_V5
    const BETA_OPT_MIN_v16b = 0.10
    const BETA_OPT_MAX_v16b = 300.0
    const H_FRONT_FIXED_v16b = 10.0
    const PRANDTL_EXPONENT_FIXED_v16b = 1.0 / 3.0
    const STANDARD_AIR_DENSITY_v16b = 101325.0 / (287.05 * 294.25)
    const DEFAULT_ROSSELAND_MEAN_PATH_v16b = 1.0e-3
    const RADIATION_AREA_v16b = A_frt
    const AXIAL_VIEW_RADIATION_EMISSIVITY_v16b = 0.85
    const AXIAL_VIEW_RADIATION_ROW_SUM_v16b = 0.06
    const AXIAL_VIEW_RADIATION_LENGTH_v16b = 0.050
    const AXIAL_VIEW_RADIATION_AREA_FACTOR_v16b = 1.0
    const TC_WIRE_LOSS_FRONT_FRACTION_v16b = 0.25
    const TC_WIRE_LOSS_LENGTH_v16b = 0.025
    const TC_WIRE_SINK_TEMPERATURE_v16b = 25.0 + 273.15
    const TEMPERATURE_AXIS_LIMITS_v16b = (250.0, 1250.0)
    const ETA_OPT_FIXED_v16b = 1.0
    const FRONT_DEPOSITION_INITIAL_v16b = 1.0
    const POWER_SCALE_MIN_v16b = 0.25
    const POWER_SCALE_MAX_v16b = 4.00
    const IRRADIANCE_LEVELS_v16b = (456000.0, 304000.0, 256000.0)
    const ORDERING_WEIGHT_v16b = 0.25
    const ORDERING_SCALE_v16b = 100.0
    const CAPACITANCE_REGULARIZATION_WEIGHT_v16b = 0.05
    const ACTIVE_AXIAL_AREA_FRACTION_v16b = 0.80
    const WALL_AXIAL_AREA_FRACTION_v16b = 0.20
    const MEASURED_ASSEMBLY_CAPACITANCE_v16b = 301.0

    # Cavity geometry from the annotated v16b schematic.
    const CAVITY_OUTER_RADIUS_v16b = 75.0e-3
    const METAL_THICKNESS_v16b = 18.0e-3
    const INSULATION_OUTER_RADIUS_v16b = CAVITY_OUTER_RADIUS_v16b - METAL_THICKNESS_v16b
    const METAL_OUTER_RADIUS_v16b = CAVITY_OUTER_RADIUS_v16b
    const CAVITY_LENGTH_v16b = 165.0e-3
    const T3_SAMPLE_POSITION_v16b = 140.0e-3

    const ADAPTOR_DIAMETER_v16b = 77.6e-3
    const ADAPTOR_RADIUS_v16b = ADAPTOR_DIAMETER_v16b / 2.0
    const ADAPTOR_LENGTH_v16b = 57.0e-3
    const ADAPTOR_TUBE_RADIUS_v16b = 6.5e-3
    const ADAPTOR_RECEIVER_OVERLAP_LENGTH_v16b = 28.0e-3
    const ADAPTOR_TUBE_OVERLAP_LENGTH_v16b = ADAPTOR_LENGTH_v16b - ADAPTOR_RECEIVER_OVERLAP_LENGTH_v16b
    const ADAPTOR_OVERLAP_LENGTH_v16b = ADAPTOR_RECEIVER_OVERLAP_LENGTH_v16b
    const ADAPTOR_FREE_LENGTH_v16b = ADAPTOR_TUBE_OVERLAP_LENGTH_v16b
    const METAL_LENGTH_v16b = CAVITY_LENGTH_v16b
    const BACKPLATE_THICKNESS_v16b = METAL_THICKNESS_v16b
    const INSULATION_MONOLITH_LENGTH_v16b = L
    const INSULATION_ADAPTOR_LENGTH_v16b = ADAPTOR_LENGTH_v16b
    const INSULATION_CONDUCTIVITY_v16b = 0.08

    const RECEIVER_EQ_RADIUS_v16b = sqrt(A_frt / pi)
    const RECEIVER_TO_CAVITY_CONDUCTANCE_PER_LENGTH_v16b =
        4.0 * pi * INSULATION_CONDUCTIVITY_v16b /
        log(INSULATION_OUTER_RADIUS_v16b / RECEIVER_EQ_RADIUS_v16b)
    const ADAPTOR_TO_CAVITY_RESISTANCE_v16b =
        log(INSULATION_OUTER_RADIUS_v16b / ADAPTOR_RADIUS_v16b) /
        (4.0 * pi * INSULATION_CONDUCTIVITY_v16b * ADAPTOR_LENGTH_v16b)
    const ADAPTOR_CONTACT_RESISTANCE_v16b =
        1.0 / (100.0 * 4.0 * w_t * ADAPTOR_OVERLAP_LENGTH_v16b)

    # Rear extension geometry supplied for v16b.
    const REAR_TUBE_LENGTH_v16b = 150.0e-3
    const REAR_TUBE_CAVITY_LENGTH_v16b = 46.0e-3
    const REAR_TUBE_FLANGE_LENGTH_v16b = REAR_TUBE_LENGTH_v16b - REAR_TUBE_CAVITY_LENGTH_v16b
    const REAR_TUBE_DEFAULT_NODES_v16b = 12
    const REAR_TUBE_GAS_RADIUS_v16b = ADAPTOR_TUBE_RADIUS_v16b
    const REAR_TUBE_WALL_THICKNESS_v16b = 1.5e-3
    const REAR_TUBE_OUTER_RADIUS_v16b = REAR_TUBE_GAS_RADIUS_v16b + REAR_TUBE_WALL_THICKNESS_v16b
    const REAR_TUBE_FLOW_AREA_v16b = pi * REAR_TUBE_GAS_RADIUS_v16b^2
    const REAR_TUBE_HYDRAULIC_DIAMETER_v16b = 2.0 * REAR_TUBE_GAS_RADIUS_v16b
    const REAR_TUBE_EXCHANGE_PERIMETER_v16b = 2.0 * pi * REAR_TUBE_GAS_RADIUS_v16b

    const RECEIVER_TO_TUBE_CONDUCTANCE_v16b =
        1.0 / ADAPTOR_CONTACT_RESISTANCE_v16b

    # Alumina and aluminum properties follow the older 0D material set. The
    # flange is treated as an isothermal water-cooled sink in v16b.
    const ALUMINA_DENSITY_v16b = 3900.0
    const ALUMINUM_DENSITY_v16b = 2700.0
    const ALUMINUM_HEAT_CAPACITY_v16b = 900.0
    const ALUMINUM_CONDUCTIVITY_v16b = 205.0
    const METAL_EMISSIVITY_v16b = 0.2
    const H_NAT_CAVITY_v16b = 10.0
    const INSULATION_DENSITY_v16b = 140.0
    const INSULATION_HEAT_CAPACITY_v16b = 1360.0
    const WATER_FLANGE_TEMPERATURE_v16b = 25.0 + 273.15

    const CAVITY_FELT_VOLUME_v16b = max(
        0.0,
        pi * INSULATION_OUTER_RADIUS_v16b^2 * CAVITY_LENGTH_v16b -
        A_frt * L -
        pi * ADAPTOR_RADIUS_v16b^2 * ADAPTOR_LENGTH_v16b,
    )
    const CAVITY_METAL_VOLUME_v16b =
        pi * (METAL_OUTER_RADIUS_v16b^2 - INSULATION_OUTER_RADIUS_v16b^2) *
        CAVITY_LENGTH_v16b +
        pi * METAL_OUTER_RADIUS_v16b^2 * BACKPLATE_THICKNESS_v16b
    const CAVITY_OUTER_AREA_v16b =
        2.0 * pi * METAL_OUTER_RADIUS_v16b * CAVITY_LENGTH_v16b +
        pi * METAL_OUTER_RADIUS_v16b^2
    const CAVITY_HEAT_CAPACITY_v16b =
        INSULATION_DENSITY_v16b * INSULATION_HEAT_CAPACITY_v16b *
        CAVITY_FELT_VOLUME_v16b +
        ALUMINUM_DENSITY_v16b * ALUMINUM_HEAT_CAPACITY_v16b *
        CAVITY_METAL_VOLUME_v16b
end

# Final v16b diagnostic parameter vector p[1:13]
...
#   p[13] k_ETC_ref     active axial ETC/radiative transport at 900 K [W/m/K]
begin # v16b parameter values and bounds
    pnew_v16b = [
        6.706436947355168e-4,
        2.205938116638645,
        PRANDTL_EXPONENT_FIXED_v16b,
        1.0,
        184.67243519237965,
        1.0,
        1.669539299381132,
    ...
    fit_heat_transfer_indices_v16b = [1, 2, 10, 11, 12, 13]
    fit_source_indices_v16b = Int[]
    fit_power_scale_indices_v16b = Int[]
    fit_radiation_indices_v16b = Int[]
end

function irradiance_level_index_v16b(irradiance)
    irradiance <= 0.0 && return 0
    differences = abs.(collect(IRRADIANCE_LEVELS_v16b) .- Float64(irradiance))
    return argmin(differences)
end

power_scale_index_v16b(irradiance) = begin
    level_index = irradiance_level_index_v16b(irradiance)
    level_index == 0 ? 0 : 6 + level_index
end

function absorbed_power_scale_v16b(irradiance, p)
    index = power_scale_index_v16b(irradiance)
    return index == 0 ? 1.0 : p[index]
end

irradiance_factor_v16b(irradiance, p) =
    ETA_OPT_FIXED_v16b * absorbed_power_scale_v16b(irradiance, p)

wall_power_fraction_v16b(p) = clamp(p[10], 0.0, 0.95)

active_wall_conductance_per_length_v16b(p) = max(p[11], eps(Float64))

wall_heat_capacity_total_v16b(p) = max(p[12], eps(Float64))

active_receiver_heat_capacity_v16b(nodes) =
    sum(
        rho_s * Cps_f(900.0) * A_solid * (L / nodes) *
        ACTIVE_AXIAL_AREA_FRACTION_v16b
        for _ in 1:nodes
    )

participating_assembly_heat_capacity_v16b(p, nodes=default_nodes) =
    active_receiver_heat_capacity_v16b(nodes) + wall_heat_capacity_total_v16b(p)

active_axial_etc_conductivity_v16b(T, p) =
    max(p[13], 0.0) * (property_temperature(T) / 900.0)^3

function receiver_nusselt_v16b(z, reynolds, prandtl, p)
    return max(0.0, p[1]) *
           max(reynolds, eps(Float64))^p[2] *
           max(prandtl, eps(Float64))^p[3]
end

m_dot_standard_v16b(flow_lpm) =
    STANDARD_AIR_DENSITY_v16b * max(0.0, flow_lpm) / 60000.0

smoothstep_v16b(x) = begin
    xi = clamp(x, 0.0, 1.0)
    xi^2 * (3.0 - 2.0 * xi)
end

function rosseland_mean_path_v16b(z, p; profile=:uniform)
    ell_uniform = DEFAULT_ROSSELAND_MEAN_PATH_v16b
    xi = smoothstep_v16b(z / L)
    if profile == :uniform
        return ell_uniform
...
rosseland_beta_v16b(p; z=0.0, profile=:uniform) =
    1.0 / rosseland_mean_path_v16b(z, p; profile=profile)

rosseland_conductivity_v16b(T, p; z=0.0, profile=:uniform) =
    16.0 * sigma_sb * property_temperature(T)^3 /
    (3.0 * rosseland_beta_v16b(p; z=z, profile=profile))

rosseland_optical_thickness_v16b(length_scale, p) =
    rosseland_beta_v16b(p) * length_scale

function axial_view_matrix_v16b(z)
    nodes = length(z)
    raw = zeros(nodes, nodes)
    for i in 1:nodes, j in 1:nodes
        i == j && continue
        separation = abs(z[i] - z[j])
        geometry_decay = exp(-separation / AXIAL_VIEW_RADIATION_LENGTH_v16b)
    ...
    return AXIAL_VIEW_RADIATION_ROW_SUM_v16b .* raw ./ row_sum
end

function axial_view_radiation_net_v16b(Ts, view_matrix, dx; strength=1.0)
    nodes = length(Ts)
    qnet = zeros(nodes)
    area = AXIAL_VIEW_RADIATION_AREA_FACTOR_v16b * P_exchange * dx
    coefficient = strength * AXIAL_VIEW_RADIATION_EMISSIVITY_v16b * sigma_sb * area
...
alumina_conductivity_v16b(T) =
    5.5 + 34.5 * exp(-0.0033 * (property_temperature(T) - 273.15))

alumina_heat_capacity_v16b(T) =
    max(500.0, (1.00446 + 1.742e-4 * property_temperature(T) -
                2.796e4 * property_temperature(T)^-2) * 1000.0)

function rear_tube_outer_radius_v16b(z_rear)
    return z_rear <= ADAPTOR_TUBE_OVERLAP_LENGTH_v16b ?
           ADAPTOR_RADIUS_v16b : REAR_TUBE_OUTER_RADIUS_v16b
end

function rear_tube_solid_area_v16b(z_rear)
    outer_radius = rear_tube_outer_radius_v16b(z_rear)
    return pi * (outer_radius^2 - REAR_TUBE_GAS_RADIUS_v16b^2)
end

function rear_tube_cavity_conductance_per_length_v16b(z_rear, p)
    z_rear > REAR_TUBE_CAVITY_LENGTH_v16b && return 0.0
    outer_radius = min(rear_tube_outer_radius_v16b(z_rear), 0.98 * INSULATION_OUTER_RADIUS_v16b)
    return 2.0 * pi * INSULATION_CONDUCTIVITY_v16b /
           log(INSULATION_OUTER_RADIUS_v16b / outer_radius)
end

function rear_tube_flange_conductance_per_length_v16b(z_rear, Ttube)
    z_rear <= REAR_TUBE_CAVITY_LENGTH_v16b && return 0.0
    tube_wall_resistance =
        log(REAR_TUBE_OUTER_RADIUS_v16b / REAR_TUBE_GAS_RADIUS_v16b) /
        (2.0 * pi * alumina_conductivity_v16b(Ttube))
    aluminum_resistance =
        log(METAL_OUTER_RADIUS_v16b / REAR_TUBE_OUTER_RADIUS_v16b) /
        (2.0 * pi * ALUMINUM_CONDUCTIVITY_v16b)
    return 1.0 / (tube_wall_resistance + aluminum_resistance)
end

function cavity_loss_to_ambient_v16b(Tcavity, ambient)
    return H_NAT_CAVITY_v16b * CAVITY_OUTER_AREA_v16b * (Tcavity - ambient) +
           METAL_EMISSIVITY_v16b * sigma_sb * CAVITY_OUTER_AREA_v16b *
           (Tcavity^4 - ambient^4)
end

function rear_tube_capacity_v16b(Ttube, z_rear, dx_rear)
    return ALUMINA_DENSITY_v16b * alumina_heat_capacity_v16b(Ttube) *
           rear_tube_solid_area_v16b(z_rear) * dx_rear
end

function rear_tube_htc_v16b(Twall, Tgas, flow_lpm, Tin)
    flow_lpm <= 1e-12 && return 0.0
    mdot = m_dot_standard_v16b(flow_lpm)
    Tfilm = 0.5 * (Twall + Tgas)
    cp = cpf_f(Tfilm)
    mu = mu_f_f(Tfilm)
    kf = kf_f(Tfilm)
    reynolds = mdot * REAR_TUBE_HYDRAULIC_DIAMETER_v16b /
               (REAR_TUBE_FLOW_AREA_v16b * mu)
    prandtl = cp * mu / kf
    nusselt = reynolds < 2300.0 ?
              3.66 :
              0.023 * reynolds^0.8 * prandtl^0.4
    return nusselt * kf / REAR_TUBE_HYDRAULIC_DIAMETER_v16b
end

function gas_profile_v16b!(Tg, Qgas, hcell, Qgas_rear, hrear,
    ...
    for i in eachindex(Ts)
        ...
        prandtl = cp * mu / kf
        nusselt = receiver_nusselt_v16b(z[i], reynolds, prandtl, p)
    ...
    for j in eachindex(Ttube)
        ...
        cp = cpf_f(Tfilm)
        hrear[j] = rear_tube_htc_v16b(Ttube[j], Tg[inlet_index], flow, Tin)
        UA = hrear[j] * REAR_TUBE_EXCHANGE_PERIMETER_v16b * dx_rear
        effectiveness = -expm1(-UA / (mdot * cp))
...
function receiver_rhs_v16b!(du, u, time, p, operating, z, dx, solar_weights,
                          z_rear, dx_rear,
    ...
    gas_profile_v16b!(Tg, Qgas, hcell, Qgas_rear, hrear,
    ...
    for i in 1:(nodes - 1)
        ...
        Tface_active = 0.5 * (Ta[i] + Ta[i + 1])
        k_etc = active_axial_etc_conductivity_v16b(Tface_active, p)
        Qcond = (
            kface * A_solid * ACTIVE_AXIAL_AREA_FRACTION_v16b +
            k_etc * A_frt
        ) * (Ta[i] - Ta[i + 1]) / dx
        dTa[i] -= Qcond
        dTa[i + 1] += Qcond
        ...
        Qwall_cond = kwface * A_solid * WALL_AXIAL_AREA_FRACTION_v16b *
                     (Tw[i] - Tw[i + 1]) / dx
        dTw[i] -= Qwall_cond
        dTw[i + 1] += Qwall_cond
        ...
        if use_rosseland_radiation
            ...
            zface = 0.5 * (z[i] + z[i + 1])
            Qrad = rosseland_conductivity_v16b(
    ...
    if use_axial_view_radiation
        Qview = axial_view_radiation_net_v16b(
    ...
    Qsolar = ETA_ABS_FIXED_v16b * irradiance_factor_v16b(irradiance, p) *
             irradiance * A_frt
    f_wall = wall_power_fraction_v16b(p)
    Gaw_per_length = active_wall_conductance_per_length_v16b(p)
    radial_conductance = RECEIVER_TO_CAVITY_CONDUCTANCE_PER_LENGTH_v16b
    for i in 1:nodes
    ...
    Qfront = H_FRONT_FIXED_v16b * A_frt * (Tw[1] - ambient) +
             EPS_FRONT_FIXED_v16b * sigma_sb * A_frt * (Tw[1]^4 - ambient^4)
    Qreceiver_tube_active = RECEIVER_TO_TUBE_CONDUCTANCE_v16b *
                            (1.0 - f_wall) * (Ta[end] - Ttube[1])
    Qreceiver_tube_wall = RECEIVER_TO_TUBE_CONDUCTANCE_v16b *
    ...
    for j in 1:(rear_nodes - 1)
        ki = alumina_conductivity_v16b(Ttube[j])
        kj = alumina_conductivity_v16b(Ttube[j + 1])
        kface = 2.0 * ki * kj / (ki + kj)
        Aface = 0.5 * (rear_tube_solid_area_v16b(z_rear[j]) +
                       rear_tube_solid_area_v16b(z_rear[j + 1]))
    ...
    for j in 1:rear_nodes
        G_cavity = rear_tube_cavity_conductance_per_length_v16b(z_rear[j], p)
        G_flange = rear_tube_flange_conductance_per_length_v16b(z_rear[j], Ttube[j])
        Q_cavity = G_cavity * dx_rear * (Ttube[j] - Tcavity)
        Q_flange = G_flange * dx_rear * (Ttube[j] - WATER_FLANGE_TEMPERATURE_v16b)
        dTtube[j] -= Qgas_rear[j] + Q_cavity + Q_flange
        du[cavity_index] += Q_cavity
    end

    Qcavity_ambient = cavity_loss_to_ambient_v16b(Tcavity, ambient)
    ...
    for i in 1:nodes
        active_capacity = rho_s * Cps_f(Ta[i]) * cell_volume *
                          ACTIVE_AXIAL_AREA_FRACTION_v16b
        wall_capacity = wall_heat_capacity_total_v16b(p) / nodes
        dTa[i] /= max(active_capacity, eps(Float64))
        dTw[i] /= max(wall_capacity, eps(Float64))
    end
    for j in 1:rear_nodes
        dTtube[j] /= max(rear_tube_capacity_v16b(Ttube[j], z_rear[j], dx_rear),
                         eps(Float64))
    end
    du[cavity_index] /= max(CAVITY_HEAT_CAPACITY_v16b, eps(Float64))
    return nothing
end

function receiver_ode_v16b!(dTs, Ts, context, time)
    receiver_rhs_v16b!(
...
function simulate_v16b(p, operating, save_times;
                     initial_temperature=Tamb, nodes=default_nodes,
                     rear_nodes=REAR_TUBE_DEFAULT_NODES_v16b,
    ...
    length(p) == 13 || throw(ArgumentError("1D_v16b expects 13 parameters"))
    ...
    all(>(0.0), p[1:5]) && 0.0 <= p[6] <= 1.0 &&
        ...
        throw(ArgumentError("Nu coefficients, beta_opt, power scales, active-wall conductance, wall capacity, and k_ETC must be bounded; front_dep and f_wall must be bounded"))
    ...
    dx = L / nodes
    dx_rear = REAR_TUBE_LENGTH_v16b / rear_nodes
    ...
    axial_view_matrix = axial_view_matrix_v16b(z)
    ...
    problem = ODEProblem(receiver_ode_v16b!, u_initial, (time[1], time[end]), context)
    solution = solve(
    ...
    successful_retcode(solution) ||
        error("1D_v16b ODE solve failed with retcode $(solution.retcode)")
    ...
    for output_index in eachindex(time)
        ...
        gas_profile_v16b!(Tg, Qgas, hcell, Qgas_rear, hrear, Ta_now, Ttube_now,
        ...
        if flow_now <= 1e-12
            ...
            mdot_now = m_dot_standard_v16b(flow_now)
            for i in 1:nodes
        ...
        receiver_to_tube_heat_history[output_index] =
            RECEIVER_TO_TUBE_CONDUCTANCE_v16b *
            ((1.0 - f_wall) * (Ta_now[end] - Ttube_now[1]) +
             f_wall * (Tw_now[end] - Ttube_now[1]))
        receiver_to_cavity_heat_history[output_index] = sum(
            RECEIVER_TO_CAVITY_CONDUCTANCE_PER_LENGTH_v16b * dx *
            (Tw_now[i] - Tcavity_now)
            for i in 1:nodes
        )
        active_wall_heat_history[output_index] = sum(
            active_wall_conductance_per_length_v16b(p) * dx *
            (Ta_now[i] - Tw_now[i])
            for i in 1:nodes
        )
        tube_to_cavity_heat_history[output_index] = sum(
            rear_tube_cavity_conductance_per_length_v16b(z_rear[j], p) * dx_rear *
            (Ttube_now[j] - Tcavity_now)
            for j in 1:rear_nodes
        )
        cavity_ambient_heat_history[output_index] =
            cavity_loss_to_ambient_v16b(Tcavity_now, operating.ambient_temperature(t))
        flange_heat_history[output_index] = sum(
            rear_tube_flange_conductance_per_length_v16b(z_rear[j], Ttube_now[j]) *
            dx_rear * (Ttube_now[j] - WATER_FLANGE_TEMPERATURE_v16b)
            for j in 1:rear_nodes
        )
    end
...
begin # v16b experimental interface
    const WALL_CHAIN_POSITIONS_v16b = (
        T8=sensor_positions[:T8],
        T12=sensor_positions[:T9],
        T11=sensor_positions[:T10],
    )

    function measured_initial_wall_profile_v16b(data, simulation_id)
        z8 = WALL_CHAIN_POSITIONS_v16b.T8
        z12 = WALL_CHAIN_POSITIONS_v16b.T12
        z11 = WALL_CHAIN_POSITIONS_v16b.T11
    ...
    function measured_initial_active_profile_v16b(data, simulation_id)
    ...
    function measured_initial_rear_profile_v16b(data, simulation_id)
        receiver_profile = measured_initial_wall_profile_v16b(data, simulation_id)
        T_receiver_exit = receiver_profile(L)
        T_tube_exit = observation(data, simulation_id, "_Tf")[1]
        return function (z_rear)
            fraction = clamp(z_rear / REAR_TUBE_LENGTH_v16b, 0.0, 1.0)
            return T_receiver_exit + fraction * (T_tube_exit - T_receiver_exit)
        end
    end

    function experimental_case_v16b(simulation_id; is_cooling=false)
    ...
    function gas_at_position_v16b(result, position)
        z = result.z_gas
    ...
    tc_wire_loss_fraction_v16b(position) =
        TC_WIRE_LOSS_FRONT_FRACTION_v16b *
        exp(-max(position, 0.0) / TC_WIRE_LOSS_LENGTH_v16b)

    function thermocouple_at_v16b(result, position; use_tc_measurement_model=false)
        solid = solid_at_v3(result, position)
        use_tc_measurement_model || return solid
        fraction = tc_wire_loss_fraction_v16b(position)
        return solid .- fraction .* (solid .- TC_WIRE_SINK_TEMPERATURE_v16b)
    end

    function active_at_v16b(result, position)
        z = result.z_solid
    ...
    function extract_outputs_v16b(result, p, T3_initial;
                                use_tc_measurement_model=false)
        T8 = thermocouple_at_v16b(
            result, sensor_positions[:T8];
            use_tc_measurement_model=use_tc_measurement_model,
        )
        T12_wall = thermocouple_at_v16b(
            result, WALL_CHAIN_POSITIONS_v16b.T12;
            use_tc_measurement_model=use_tc_measurement_model,
        )
        T11_wall = thermocouple_at_v16b(
            result, WALL_CHAIN_POSITIONS_v16b.T11;
            use_tc_measurement_model=use_tc_measurement_model,
        )
        T9_active = active_at_v16b(result, sensor_positions[:T9])
        T10_active = active_at_v16b(result, sensor_positions[:T10])
        Tf = gas_at_position_v16b(result, T3_SAMPLE_POSITION_v16b)
    ...
    function solve_case_v16b(p, simulation_id; is_cooling=false,
                           nodes=default_nodes,
                           rear_nodes=REAR_TUBE_DEFAULT_NODES_v16b,
                           use_rosseland_radiation=false,
                           rosseland_profile=:uniform,
                           ...
                           solver=Rodas5P(autodiff=AutoFiniteDiff()),
                           reltol=1e-6, abstol=1e-7, dtmax=30.0)
        conditions, time, experiment, data = experimental_case_v16b(
        ...
        result = simulate_v16b(
            p, operating, time;
            initial_temperature=measured_initial_active_profile_v16b(data, simulation_id),
            initial_wall_temperature=measured_initial_wall_profile_v16b(data, simulation_id),
            initial_rear_temperature=measured_initial_rear_profile_v16b(data, simulation_id),
...
begin # v16b objective helpers
    const TIMING_WEIGHT_v16b = 0.15
    const SLOPE_WEIGHT_v16b = 0.25

    function normalized_slope_mse_v16b(model, experiment)
    ...
    function timing_penalty_v16b(time, model, experiment)
        duration = max(time[end] - time[1], 1.0)
        return ((get_t90_v3(time, model) - get_t90_v3(time, experiment)) / duration)^2
    end

    function signal_loss_v16b(time, model, experiment)
        return normalized_mse_v3(model, experiment) +
               SLOPE_WEIGHT_v16b * normalized_slope_mse_v16b(model, experiment) +
               TIMING_WEIGHT_v16b * timing_penalty_v16b(time, model, experiment)
    end

    function ordering_loss_v16b(model, experiment)
        model_offsets = (
        ...
        return mean(abs2, collect(model_offsets) .- collect(experiment_offsets)) /
               ORDERING_SCALE_v16b^2
    end

    function capacitance_regularization_v16b(p; nodes=15)
        capacitance = participating_assembly_heat_capacity_v16b(p, nodes)
        return CAPACITANCE_REGULARIZATION_WEIGHT_v16b *
               ((capacitance - MEASURED_ASSEMBLY_CAPACITANCE_v16b) /
                MEASURED_ASSEMBLY_CAPACITANCE_v16b)^2
    end

    function loss_cases_v16b(p, keys; is_cooling=false, nodes=15,
                            use_rosseland_radiation=false,
                            rosseland_profile=:uniform,
                            use_axial_view_radiation=false,
                            axial_view_strength=1.0,
        ...
        for simulation_id in keys
            try
                model, result, experiment = solve_case_v16b(
                ...
                total += mean(
                    signal_loss_v16b(result.time, model[:, j], experiment[:, j])
                    for j in 1:7
                )
                if !is_cooling
                    total += ORDERING_WEIGHT_v16b * ordering_loss_v16b(model, experiment)
                end
            catch err
    ...
    function loss_heating_v16b(p, keys=sim_key_heat; nodes=15)
        regularization = capacitance_regularization_v16b(p; nodes=nodes)
        return loss_cases_v16b(p, keys; is_cooling=false, nodes=nodes) + regularization
    end

    function loss_cooling_v16b(p, keys=sim_key_cool; nodes=15)
        regularization = capacitance_regularization_v16b(p; nodes=nodes)
        return loss_cases_v16b(p, keys; is_cooling=true, nodes=nodes) + regularization
    end
end

function optimize_parameter_subset_v16b(objective, p_initial, indices, lower, upper;
...
function calibrate_v16b(; heating_keys=sim_key_heat,
                      nodes=15,
                      maximum_iterations=80,
                      maximum_time_seconds=240.0,
                      optimizer=NLopt.LN_NELDERMEAD())
    objective = p -> loss_heating_v16b(p, heating_keys; nodes=nodes)
    return optimize_parameter_subset_v16b(
        objective, pnew_v16b, fit_heat_transfer_indices_v16b,
        lb_full_v16b, ub_full_v16b;
        maximum_iterations=maximum_iterations,
        maximum_time_seconds=maximum_time_seconds,
        optimizer=optimizer,
        label="v16b-apparent-heat-transfer",
    )
end

function quick_calibration_v16b()
    return calibrate_v16b(maximum_iterations=20, maximum_time_seconds=60.0)
end

begin # v16b post-processing
    function transient_case_v16b(simulation_id, p=pnew_v16b; is_cooling=false,
        ...
        return (
            ...
            T3_sample_gas=gas_at_position_v16b(result, T3_SAMPLE_POSITION_v16b),
    ...
    function plot_case_v16b(simulation_id, params=pnew_v16b; is_cooling=false,
                          nodes=default_nodes, use_rosseland_radiation=false,
                          rosseland_profile=:uniform,
                          use_axial_view_radiation=false,
                          axial_view_strength=1.0,
        ...
        data = transient_case_v16b(simulation_id, params;
        ...
        plot_object = plot(title="1D_v16b transient: $simulation_id",
    ...
    function compute_metrics_v16b(p=pnew_v16b; heating_keys=sim_key_heat,
            ...
            for simulation_id in keys
                ...
                for (sensor, column) in ((:T8, 1), (:T12_wall, 2),
                    ...
                    push!(metrics, (
                        ...
                        shape_loss=normalized_slope_mse_v16b(
                ...
                for (sensor, model_delta, experiment_delta) in (
                    ...
                    push!(metrics, (
                        ...
                        t90_error_s=0.0,
                        shape_loss=normalized_slope_mse_v16b(model_delta, experiment_delta),
                    ))
                end
            end
        end
...
println("""
[1D_v16b] Model loaded.
Use run_1D_v16b.jl for the apparent heat-transfer refit.
The default coefficient vector is stored in pnew_v16b.
""")
