# ============================================================================
# run_2D_v2.jl - 2D_v2 Model Parameter Optimization, Calibration, and Runner
# ============================================================================
# Calibrates 2D_v2 parameters via NLopt.LN_BOBYQA and outputs fitted results only:
# metrics (RMSE, steady error, t90 error, shape loss), steady parity plot,
# axial profiles, flow slopes, cell diagnostics, and 2D spatial heatmaps.
# ============================================================================
...
begin # Directory setup
    const OUTPUT_DIR_2D_v2 = joinpath(@__DIR__, "summaries", "2D_v2")
    const PLOT_DIR_2D_v2 = joinpath(OUTPUT_DIR_2D_v2, "plots")
    const TRANSIENT_DIR_2D_v2 = joinpath(PLOT_DIR_2D_v2, "transients")
    const PROFILES_DIR_2D_v2 = joinpath(PLOT_DIR_2D_v2, "2d_profiles")
    const AXIAL_DIR_2D_v2 = joinpath(PLOT_DIR_2D_v2, "axial_profiles")
    const DIAGNOSTICS_DIR_2D_v2 = joinpath(OUTPUT_DIR_2D_v2, "diagnostics")
    
    mkpath(OUTPUT_DIR_2D_v2)
    mkpath(PLOT_DIR_2D_v2)
    mkpath(TRANSIENT_DIR_2D_v2)
    mkpath(PROFILES_DIR_2D_v2)
    mkpath(AXIAL_DIR_2D_v2)
    mkpath(DIAGNOSTICS_DIR_2D_v2)
    ...
    const SENSOR_COLORS_2D = Dict(
        :T8 => :blue, :T12_perim => :red, :T11_perim => :green,
        :T9_core => :orange, :T10_core => :brown, :T3 => :purple, :T2 => :black
    )
    const COOLING_HEATING_MAP = Dict(
        "C69" => "E69",
        "C80" => "E80",
        "C81" => "E81",
    )
...
function run_single_case_2D(simulation_id, p=default_parameters2D(); is_cooling=false)
    ...
    scale_factor = 1.0
    if irr_val >= 400000.0
        scale_factor = p.optics.scale_456
    elseif irr_val >= 280000.0
        scale_factor = p.optics.scale_304
    else
        scale_factor = p.optics.scale_256
    end
    ...
    if is_cooling
        heat_id = get(COOLING_HEATING_MAP, simulation_id, "E" * simulation_id[2:end])
        heat_data = run_single_case_2D(heat_id, p; is_cooling=false)
        u_hot = heat_data.result.ode_solution.u[end]
        result = simulate2D(p, op, save_times; initial_temperature=u_hot)
    else
...
function plot_steady_parity_2D(steady_results)
    p_plot = plot(
        title="2D_v2 Multi-Domain Fitted Steady-State Parity Plot",
    ...
    path = joinpath(PLOT_DIR_2D_v2, "steady_comparison_2D_v2.png")
    savefig(p_plot, path)
    return path
end
...
function plot_axial_profile_2D(sim_id, case_data)
    ...
    scatter!(p_plot, [107.0], [exp[end, 3]], label="T11 (Perim 107mm)", color=:green, markershape=:dtriangle, ms=6)
    ...
    path = joinpath(AXIAL_DIR_2D_v2, "axial_profile_$(sim_id)_2D_v2.png")
    savefig(p_plot, path)
    return path
end
...
function plot_transient_2D(sim_id, case_data; is_cooling=false)
    ...
    p_plot = plot(
        title="2D_v2 Fitted Transient: $sim_id $(is_cooling ? "(Cooling)" : "(Heating)")",
    ...
    for (j, sensor) in enumerate(SENSOR_NAMES_2D)
        ...
        plot!(p_plot, times, model[:, j], label="$(sensor) Model", lw=2, color=col)
        scatter!(p_plot, times, exp[:, j], label="$(sensor) Exp", ms=2, color=col, markerstrokewidth=0)
    end
    
    suffix = is_cooling ? "_cooling" : ""
    path = joinpath(TRANSIENT_DIR_2D_v2, "transient_$(sim_id)$(suffix)_2D_v2.png")
    savefig(p_plot, path)
    return path
end
...
function plot_2d_heatmap(sim_id, result)
    ...
    p_heat = heatmap(
        z_mm, r_mm, T_final,
        title="2D_v2 Multi-Domain Temperature Field T_s(r, z) [K] - $sim_id",
        xlabel="Axial Position z (mm)", ylabel="Radial Radius r (mm)",
        c=:inferno, aspect_ratio=:equal, clim=(minimum(T_final), maximum(T_final))
    )
    
    path = joinpath(PROFILES_DIR_2D_v2, "heatmap_2D_$(sim_id)_2D_v2.png")
    savefig(p_heat, path)
    return path
end
...
function write_parameters_2D(path, p::ModelParameters2D)
    names = (
        ...
        "phi_0", "m_rec", "front_dep_fraction", "flange_conductance_scale", "c_radial_flow"
    )
...
begin # Main Execution Loop
    println("==========================================================================")
    println("Running 2D_v2 Multi-Domain Calibration & Optimization...")
    println("==========================================================================")
    ...
    open(joinpath(OUTPUT_DIR_2D_v2, "optimization_summary_2D_v2.txt"), "w") do io
        println(io, "objective=$(calib.objective)")
        println(io, "return_code=$(calib.retcode)")
        println(io, "parameters=$(pack_parameters2D(p_fitted))")
    end
    
    write_parameters_2D(joinpath(OUTPUT_DIR_2D_v2, "parameters_fitted_2D_v2.csv"), p_fitted)
    
    println("[run_2D_v2] Computing metrics and generating figures for fitted 2D_v2 model...")
    metrics = compute_all_metrics_2D(p_fitted)
    open(joinpath(OUTPUT_DIR_2D_v2, "analysis_results_2D_v2.csv"), "w") do io
    ...
    steady_path = joinpath(OUTPUT_DIR_2D_v2, "steady_results_2D_v2.csv")
    ...
    slopes_path = joinpath(OUTPUT_DIR_2D_v2, "flow_slopes_2D_v2.csv")
    ...
    for sim_id in IDs
        ...
        write_cell_diagnostics_2D(
            joinpath(DIAGNOSTICS_DIR_2D_v2, "cell_diagnostics_$(sim_id)_2D_v2.csv"),
    ...
    println("==========================================================================")
    println("Multi-Domain 2D_v2 Calibration and Runner Execution Complete.")
