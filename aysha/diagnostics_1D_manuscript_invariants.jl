...
const INVARIANT_OUTPUT_DIR_1D = joinpath(@__DIR__, "summaries", "1D_v30")
...
function tube_loss_totals_1D(result, p, time, irradiance)
    ...
    dx_rear = REAR_TUBE_LENGTH_v30 / length(z_local)
    ...
    for j in eachindex(z_local)
        ...
        G_flange = rear_tube_flange_conductance_per_length_v30(
            z_local[j], Ttube[j], p, time, irradiance,
        )
        tube_to_cavity += G_cavity * dx_rear * (Ttube[j] - Tcavity)
        tube_to_flange += G_flange * dx_rear * (Ttube[j] - WATER_FLANGE_TEMPERATURE_v30)
    end
...
function rear_rail_heat_totals_1D(result, p)
    ...
    Qrear_tube = rear_tube_conductance_v30(p) * (Trear[end] - Ttube_in)
    return Qcore_rear, Qperim_rear, Qrear_tube, Qrear_cavity
end

function invariant_row_1D(simulation_id, p=pnew_v30; nodes=default_nodes)
    ...
    Tfront = Tperim[1]
    q_front = H_FRONT_FIXED_v30 * A_frt * (Tfront - ambient) +
              EPS_FRONT_FIXED_v30 * sigma_sb * A_frt * (Tfront^4 - ambient^4)
    q_cavity_ambient = cavity_loss_to_ambient_v30(result.cavity_temperature[end], ambient)
    ...
    return (
        ...
        power_scale=absorbed_power_scale_v30(irradiance, p),
        ...
        C_receiver_participating_J_K=participating_assembly_heat_capacity_v30(p, nodes),
        C_total_with_rear_J_K=participating_total_heat_capacity_v30(p, nodes),
...
function write_summary_1D(path, rows)
    ...
    open(path, "w") do io
        ...
        println(io, "C_receiver_participating,$(participating_assembly_heat_capacity_v30(pnew_v30)),301 +/- 23,ingested/failed")
        println(io, "C_total_with_rear,$(participating_total_heat_capacity_v30(pnew_v30)),301 +/- 23,failed")
    end
    return path
end

rows = [invariant_row_1D(simulation_id, pnew_v30) for simulation_id in sim_key_heat]
write_rows_1D(joinpath(INVARIANT_OUTPUT_DIR_1D, "invariants_1D_v30.csv"), rows)
write_summary_1D(joinpath(INVARIANT_OUTPUT_DIR_1D, "invariant_summary_1D_v30.csv"), rows)

println("[diagnostics_1D_manuscript_invariants] wrote summaries/1D_v30/invariants_1D_v30.csv")
println("[diagnostics_1D_manuscript_invariants] wrote summaries/1D_v30/invariant_summary_1D_v30.csv")
