...
const INVARIANT_OUTPUT_DIR_1D = joinpath(@__DIR__, "summaries", "1D_v35")
...
function tube_loss_totals_1D(result, p, time, irradiance)
    ...
    dx_rear = REAR_TUBE_LENGTH_v35 / length(z_local)
    Ttube = result.rear_tube_temperature[:, end]
    Tcavity = result.cavity_temperature[end]
    tube_to_cavity = 0.0
    tube_to_flange = 0.0
    for j in eachindex(z_local)
        G_cavity = rear_tube_cavity_conductance_per_length_v35(z_local[j], p)
        G_flange = rear_tube_flange_conductance_per_length_v35(
            z_local[j], Ttube[j], p, time, irradiance,
        )
        tube_to_cavity += G_cavity * dx_rear * (Ttube[j] - Tcavity)
        tube_to_flange += G_flange * dx_rear * (Ttube[j] - WATER_FLANGE_TEMPERATURE_v35)
    end
...
function rear_rail_heat_totals_1D(result, p)
    ...
    f_core = rear_core_fraction_v35(p)
    ...
    Qrear_cavity = sum(
        rear_cavity_conductance_v35(p) * result.rear_contact_weights[i] *
        (Trear[i] - Tcavity)
        for i in eachindex(Trear)
    )
    Qrear_tube = rear_tube_conductance_v35(p) * (Trear[end] - Ttube_in)
    return Qcore_rear, Qperim_rear, Qrear_tube, Qrear_cavity
end

function invariant_row_1D(simulation_id, p=(@isdefined(fitted_params) ? fitted_params : pnew_v35); nodes=default_nodes)
    outputs, result, experiment = solve_case_v35(p, simulation_id; nodes=nodes)
    ...
    Tfront = Tperim[1]
    q_front = H_FRONT_FIXED_v35 * A_frt * (Tfront - ambient) +
              EPS_FRONT_FIXED_v35 * sigma_sb * A_frt * (Tfront^4 - ambient^4)
    q_cavity_ambient = cavity_loss_to_ambient_v35(result.cavity_temperature[end], ambient)
    ...
    return (
        ...
        power_scale=absorbed_power_scale_v35(irradiance, p),
        ...
        C_receiver_participating_J_K=participating_assembly_heat_capacity_v35(p, nodes),
        C_total_with_rear_J_K=participating_total_heat_capacity_v35(p, nodes),
...
function write_summary_1D(path, rows)
    ...
    open(path, "w") do io
        ...
        receiver_capacity = participating_assembly_heat_capacity_v35((@isdefined(fitted_params) ? fitted_params : pnew_v35))
        total_capacity = participating_total_heat_capacity_v35((@isdefined(fitted_params) ? fitted_params : pnew_v35))
...
rows = [invariant_row_1D(simulation_id, (@isdefined(fitted_params) ? fitted_params : pnew_v35)) for simulation_id in sim_key_heat]
write_rows_1D(joinpath(INVARIANT_OUTPUT_DIR_1D, "invariants_1D_v35.csv"), rows)
write_summary_1D(joinpath(INVARIANT_OUTPUT_DIR_1D, "invariant_summary_1D_v35.csv"), rows)

println("[diagnostics_1D_manuscript_invariants] wrote summaries/1D_v35/invariants_1D_v35.csv")
println("[diagnostics_1D_manuscript_invariants] wrote summaries/1D_v35/invariant_summary_1D_v35.csv")
