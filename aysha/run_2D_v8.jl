# ============================================================================
# run_2D_v8.jl - 2D_v8 Heating-Only Runner with Cooling Validation Benchmarks
# ============================================================================
...
begin # Directory setup
    const OUTPUT_DIR_2D_v8 = joinpath(@__DIR__, "summaries", "2D_v8")
    const PLOT_DIR_2D_v8 = joinpath(OUTPUT_DIR_2D_v8, "plots")
    const TRANSIENT_DIR_2D_v8 = joinpath(PLOT_DIR_2D_v8, "transients")
    const PROFILES_DIR_2D_v8 = joinpath(PLOT_DIR_2D_v8, "2d_profiles")
    const AXIAL_DIR_2D_v8 = joinpath(PLOT_DIR_2D_v8, "axial_profiles")
    const DIAGNOSTICS_DIR_2D_v8 = joinpath(OUTPUT_DIR_2D_v8, "diagnostics")
    
    mkpath(OUTPUT_DIR_2D_v8)
    mkpath(PLOT_DIR_2D_v8)
    mkpath(TRANSIENT_DIR_2D_v8)
    mkpath(PROFILES_DIR_2D_v8)
    mkpath(AXIAL_DIR_2D_v8)
    mkpath(DIAGNOSTICS_DIR_2D_v8)
...
function calibration_mesh_parameters_2D(p)
    g0 = p.geometry
    g = Geometry2D(
        length=g0.length,
...
function run_calibration_case_2D(simulation_id, p=default_parameters2D(); is_cooling=false)
    return run_single_case_2D(simulation_id, p; is_cooling=is_cooling)
end
...
function plot_steady_parity_2D(steady_results)
    p_plot = plot(
        title="2D_v8 Heating-Calibrated Steady-State Parity Plot",
    ...
    path = joinpath(PLOT_DIR_2D_v8, "steady_comparison_2D_v8.png")
    savefig(p_plot, path)
    return path
end
...
function plot_axial_profile_2D(sim_id, case_data)
    ...
    perim_T = result.solid_temperature[result.nr_rec, :, end]
    ...
    path = joinpath(AXIAL_DIR_2D_v8, "axial_profile_$(sim_id)_2D_v8.png")
    savefig(p_plot, path)
    return path
end
...
function plot_transient_2D(sim_id, case_data; is_cooling=false)
    ...
    p_plot = plot(
        title="2D_v8 Fitted Transient: $sim_id $(is_cooling ? "(Cooling Benchmark)" : "(Heating Calibrated)")",
    ...
    path = joinpath(TRANSIENT_DIR_2D_v8, "transient_$(sim_id)$(suffix)_2D_v8.png")
    savefig(p_plot, path)
    return path
end
...
function plot_2d_heatmap(sim_id, result)
    ...
    p_heat = heatmap(
        z_mm, r_mm, T_final,
        title="2D_v8 Multi-Domain Temperature Field T_s(r, z) [K] - $sim_id",
        xlabel="Axial Position z (mm)", ylabel="Radial Radius r (mm)",
        c=:inferno, aspect_ratio=:equal, clim=(minimum(T_final), maximum(T_final))
    )
    
    path = joinpath(PROFILES_DIR_2D_v8, "heatmap_2D_$(sim_id)_2D_v8.png")
    savefig(p_heat, path)
    return path
end
...
function write_parameters_2D(path, p::ModelParameters2D)
    ...
    open(path, "w") do io
        println(io, "index,name,value,fitted")
        for i in eachindex(vals)
            println(io, join((i, names[i], vals[i], i in FIT_INDICES_2D), ','))
        end
    end
    return path
end
...
function main_2D_v8()
    println("==========================================================================")
    println("Running 2D_v8 Multi-Domain HEATING-ONLY Calibration with Radiative Transport...")
    println("==========================================================================")
    ...
    println("[run_2D_v8] Optimization checkpoint:")
    println("  objective=$(calib.objective)")
    println("  return_code=$(calib.retcode)")
    println("  evaluations=$(calib.evaluations)")
    println("  parameters=$(pack_parameters2D(p_fitted))")
    flush(stdout)
    
    open(joinpath(OUTPUT_DIR_2D_v8, "optimization_summary_2D_v8.txt"), "w") do io
        println(io, "objective=$(calib.objective)")
        println(io, "return_code=$(calib.retcode)")
        println(io, "evaluations=$(calib.evaluations)")
        println(io, "calibration_mesh=nominal_(14,7,3,45,30)")
        println(io, "fitted_indices=$(calib.fitted_indices)")
        println(io, "parameters=$(pack_parameters2D(p_fitted))")
    end
    
    write_parameters_2D(joinpath(OUTPUT_DIR_2D_v8, "parameters_fitted_2D_v8.csv"), p_fitted)
    
    println("[run_2D_v8] Computing metrics and generating figures for fitted 2D_v8 model...")
    metrics = compute_all_metrics_2D(p_fitted)
    open(joinpath(OUTPUT_DIR_2D_v8, "analysis_results_2D_v8.csv"), "w") do io
    ...
    steady_path = joinpath(OUTPUT_DIR_2D_v8, "steady_results_2D_v8.csv")
    ...
    slopes_path = joinpath(OUTPUT_DIR_2D_v8, "flow_slopes_2D_v8.csv")
    ...
    for sim_id in IDs
        ...
        write_cell_diagnostics_2D(
            joinpath(DIAGNOSTICS_DIR_2D_v8, "cell_diagnostics_$(sim_id)_2D_v8.csv"),
    ...
    println("==========================================================================")
    println("Multi-Domain 2D_v8 Calibration and Runner Execution Complete.")
