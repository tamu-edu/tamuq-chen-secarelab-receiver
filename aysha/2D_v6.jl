# ============================================================================
# 2D_v6.jl - Multi-Domain 2D Model with Enhanced Entry Heat Transfer & Deeper Penetration
# ============================================================================
# Key Physics & Numerical Enhancements in v6:
#   1. Entry-Region Gas Convection Acceleration (A_entry * (Dh/z)^0.5):
#      rapidly transfers heat from front face T8 into gas stream, delivering enthalpy to T9/T12.
#   2. Deeper Optical Penetration (beta_opt ∈ [80, 150] m^-1):
#      deposits solar flux deeper into z = 15 - 45 mm, relieving T8 overprediction and boosting T9/T12.
#   3. Distinct Anisotropic Conductivities (chi_z ∈ [0.80, 2.50] vs chi_r ∈ [0.003, 0.015]):
#      reflecting continuous axial web walls vs radial void gaps.
#   4. Enhanced Plotting for T11 Visibility (distinct line style & marker overlay).
# ============================================================================

module Receiver2D_v6
...
Base.@kwdef struct SolidProperties2D
    ...
    radial_conductivity_scale::Float64 = 0.008   # Effective radial conductivity scale (k_r_eff ~ 0.8 W/m/K)
    axial_conductivity_scale::Float64 = 1.50    # Effective axial conductivity scale (k_z_eff ~ 25 W/m/K)
...
function _gas_profile2D!(gas, qgas, hcell, gas_rear, qgas_rear, hrear, solid_temp, T_rear, time, p, op, grid)
    ...
    for i in 1:nr_rec
        ...
        for j in 1:nz
            ...
            # Enhanced entry region factor: A_entry * (Dh / z)^0.5
...
function receiver_ode2D!(du, u, context, time)
    ...
    for i in 1:nr_total, j in 1:(nz - 1)
        ...
        kz_j = kz_scale * prop_j.k + (i <= nr_rec ? 16.0 * SIGMA * T_s[i, j]^3 / (3.0 * p.optics.extinction_coefficient) : 0.0)
        kz_jp1 = kz_scale * prop_jp1.k + (i <= nr_rec ? 16.0 * SIGMA * T_s[i, j + 1]^3 / (3.0 * p.optics.extinction_coefficient) : 0.0)
    ...
    for j in 1:nz
        ...
        Qcasing_loss = 10.0 * (2.0 * pi * g.casing_outer_radius * dz_j) * (T_s[nr_total, j] - ambient) +
                       loss.cavity_emissivity * SIGMA * (2.0 * pi * g.casing_outer_radius * dz_j) * (T_s[nr_total, j]^4 - ambient^4)
...
function unpack_parameters2D(theta::AbstractVector, p_base::ModelParameters2D=default_parameters2D())
    ...
    opt = OpticalParameters2D(
        ...
        extinction_coefficient = theta[14],
...
# Deeper optical penetration beta_opt ∈ [80, 150] m^-1
# and accelerated entry convective Nu enhancement A_entry ∈ [0.5, 3.5]
const LB_2D = [0.5, 0.10, 0.003, 0.80, 5.0, 0.01, 100.0, 0.55, 0.55, 0.40, 0.40, 0.00, 0.10, 80.0, 0.50]
const UB_2D = [6.0, 0.70, 0.015, 2.50, 35.0, 0.40, 500.0, 1.20, 1.20, 1.00, 1.00, 0.60, 0.40, 150.0, 3.50]
    ...
    for (sim_id, is_cool) in vcat([(id, false) for id in heating_cases], [(id, true) for id in cooling_cases])
        try
            ...
            for j in 1:7
                ...
                # Weight T11, T12, T9 heavily to force good fit
                w_sensor = (j in [2, 3, 4]) ? 1.5 : 1.0
            ...
            if !is_cool
                ...
                total_loss += 0.50 * (t12_t9_diff^2 + t11_t10_diff^2) / 100.0^2
            end
...
function calibrate2D(solve_fn; heating_cases=("E67", "E76", "E80"),
    ...
    function counted_objective(theta, p_data)
        ...
        if isfinite(val)
            println("[calibrate2D v6] Eval $(evals[]): loss = $(round(val, digits=4))")
            flush(stdout)
        end
        return val
    end
    ...
    println("[calibrate2D v6] Starting NLopt.LN_BOBYQA parameter optimization...")
