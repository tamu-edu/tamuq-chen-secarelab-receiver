...
# ╔═╡ 09122b47-e468-4e24-90a8-f890f5d84eaa
using MethodOfLines, ModelingToolkit, DomainSets, OrdinaryDiffEq, NonlinearSolve, PlutoUI, DifferentialEquations, Plots, XLSX, Statistics, Interpolations, Optim, DifferentialEvolutionMCMC, LsqFit, Optimization, NLopt, Symbolics

# ╔═╡ b70f9ab8-15b6-4215-9af3-44f950070acd
begin
...
# ╔═╡ d4885115-03ae-4024-81a1-41c8ed228e19
# begin
# # Define your Nu number correlation function
# function Nu_corr(p, input_data)
#     predicted_data = p[1] * (1 .+ (p[2] .* (input_data .^ p[3]) .* exp.(-p[4] ./ input_data)))  # Use .+ and .* for element-wise operations
#     return predicted_data
# end

# # Define your objective function with parameters as the first argument
# function objective_function(p)
#     predicted_data = Nu_corr(p, input_data)
#     return sum((predicted_data .- experimental_data).^2)  # Use .- for element-wise subtraction
# end

# # Define your experimental data and input data (Gz)
# input_data = [453.4, 340.0, 226.7, 170.0, 136.0, 113.3, 97.2, 85.0, 75.6, 68.0, 13.6]
# experimental_data = [4.389, 3.865, 3.206, 2.856, 2.652, 2.532, 2.480, 2.452, 2.447, 2.450, 2.825]

# # Initial guess for parameters
# p0 = [2.976, 0.2046, 0.545, 48.2]
# #p0 = [A, B, n, C]
# # Perform the optimization using NelderMead method
# result = Optim.optimize(objective_function, p0, Optim.NelderMead())
...
# ╔═╡ 2136b0ba-6c99-465f-860a-7fa78c1eb783
begin 
# Define your Nu number correlation function
function Nu_corr(p, input_data)
    predicted_data = p[1] * (1 .+ (p[2] .* (input_data .^ p[3]) .* exp.(-p[4] ./ input_data)))  # Use .+ and .* for element-wise operations
...
function objective_function(p)
    ...
    lower_bounds = [0.0, 0.0, 0, 0]  # Adjust these values as needed
    upper_bounds = [2.976, 0.23, 0.6, 100]  # Adjust these values as needed
    if any(p .< lower_bounds) || any(p .> upper_bounds)
        return Inf  # Return a large value if any parameter is out of bounds
    end
...
# Perform the optimization using NelderMead method
result = Optim.optimize(p -> objective_function(p), p0, Optim.NelderMead())
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "847fb69e3718770b3fc1a260dbcc3100b51177ad"
...
git-tree-sha1 = "c5ae46b489c8ad029b5b3e76d3264485b43f4378"
