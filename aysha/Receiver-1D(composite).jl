...
# ╔═╡ 35035ba0-37a4-11ee-024a-9356ebe33d97
using MethodOfLines, ModelingToolkit, DomainSets, OrdinaryDiffEq, NonlinearSolve, PlutoUI, DifferentialEquations, Plots, XLSX, Statistics, Interpolations, Optim, DifferentialEvolutionMCMC, Optimization, OptimizationOptimJL, LsqFit, Symbolics

# ╔═╡ ea529929-fa55-4001-9b90-6f668253f4e9
begin 
...
# ╔═╡ 0264c46a-4eb8-498b-9f11-307c863b48ce
begin 
	md" I0: $(@bind I0 Slider(0.0:0.01:700.0, 296.0, true))"
	
end 

# ╔═╡ e38d2b8b-bdd6-460e-897d-baaa87d6b0fe
begin
#solid
#using exp. 028
...
D= 1.5e-3 #m (hydrolic diameter)
tau= 10 #Tmax/Tmin for exp.28 (Solid temp. T8)
Re= 3.6707
Pr= 0.68374
#"Adequacy of Lumped Parameter Models for SCR Reactors with Monolith Structure"
A= 2.976 #Nu-inf 
#"Improved Nu number correlations for gas flow in monolith reactors using temperature-dependent fluid properties" for tau >=1.33
B= 0.0093*(tau^2)-(0.058*tau)+0.116
C_= 21.063*tau+40.012
n= -0.0132*(tau^2)+(0.0948*tau)+0.3836
inv_Gzz= 0.00294*(tau^2)-0.000639*tau+0.0497 #axial position value at the minimum of the curve
inv_Gz= L/(D*Re*Pr)
Nu_Tmin= 0.144*(tau^2)-0.934*tau+4.433
ξ= 0.502*(inv_Gz-inv_Gzz)-15.575
end


# ╔═╡ 5e8d9a73-93c4-4bda-808d-df0646100e1c
V

# ╔═╡ 6579fb54-9eaa-49a9-89c2-f100fabcadf4
Nu_Tmin

# ╔═╡ 749bc4f5-4e08-4bf1-8775-06bdff6d5650
begin
...
keff(t,x)= (0.82 * ((1.52e-11 * (T(t, x)^3) - 4.86e-8 * (T(t, x)^2) + 1.02e-4 * T(t, x) - 3.93e-3) / 1000) +(1 - 0.82)*(52000 * exp(-1.24e-5 * T(t, x)) / (T(t, x) + 437)) / 1000)

eq =  keff(t,x) * Dxx(T(t, x)) + I0 * exp(-2300 * T(t, x)) - (3.018 * exp(-0.00574 * T(t, x)) + 0.8063 * exp(-0.0008381 * T(t, x))) * Cpf * V* Dx(T(t, x)) ~ ρs * Cps * Dt(T(t, x))

 bcs = [T(t_min, x) ~ Ts,
        - keff(t,x) * Dx(T(t, 	x_max)) ~ 0,
        -keff(t,x) * Dx(T(t, x_min)) ~ (-(ϵ * σ * ((T(t, x_min)^4) - (Ts)^4)) + Q - h * (T(t, x_min) - Ts))-91.64]
...
# ╔═╡ fb74c221-1196-4d89-91fb-5fd4fa377123
sol[x]

# ╔═╡ a43248c8-f0b4-4119-84b5-2f390177e4b4
sol.t

# ╔═╡ 56dfc3c0-20cf-4373-9764-046166c56d94
sol[T(t,x)]

# ╔═╡ af5de456-1007-4860-9e3b-047b62be281e
begin
	plot(
	sol.t, 
	sol[T(t,x)][:,1], 
...
#plot(sol.t, sol[T(t,x)][:, 1]) #front of channel
#plot(sol.t, sol[T(t,x)][:, end]) #Back of channel

# ╔═╡ e85584da-5f2f-4fde-9cdb-c958e4dc94e9
# ╠═╡ disabled = true
#=╠═╡
begin
# Calculate local Nusselt number
function calculate_nusselt_number(A, Nu_Tmin, ξ)
    # Insert Nusselt number correlation here based on the temperature and geometry
    Nu_Tmin = 0.144 * (tau^2) - 0.934 * tau + 4.433
    ξ = 0.502 * (inv_Gz - inv_Gzz) - 15.575
    
    Nu_Tsig = (A/Nu_Tmin) * ((1/(1+exp(-ξ))) + Nu_Tmin - 1)
    
    return Nu_Tsig
end
# Arrays to store Gz and Nu values
gz_values = []
nu_values = []

# Loop over all points and apply the Nusselt number
local_nu = 0.0  # Initialize local_nu before the loop
for i in 1:nc
	A_value = A * i
    Nu_Tmin_value = Nu_Tmin * i
    ξ_value = ξ * i

    local local_nu = calculate_nusselt_number(A_value, Nu_Tmin_value, ξ_value)
    push!(gz_values, i)  
    push!(nu_values, local_nu)
end
    # Calculate heat transfer coefficient
    h_ = local_nu * keff / L
    
    # Calculate local Graetz number (Gz) based on relevant variables
    gz = (V * x_val) / (keff/(3.018 * exp(-0.00574 * T(t, x)) + 0.8063 * exp(-0.0008381 * T(t, x)))*Cpf)
    
 # Display calculated values
println("Gz Values: ", gz_values)
println("Nu Values: ", nu_values)


# Create a Nu vs Gz plot
plot(gz_values, nu_values, xlabel = "Gz", ylabel = "Nu", legend = false) 
end
  ╠═╡ =#

# ╔═╡ 2bca9140-2632-45ae-88a0-f07756b83656
#=╠═╡
begin
	local_nu_numeric = float(local_nu)
	push!(nu_values, local_nu_numeric)
end
  ╠═╡ =#

# ╔═╡ 237680f5-0fe4-45d1-b9ee-993a131ede53
#=╠═╡
begin
	println(typeof(gz_values[1]))
	println(typeof(nu_values[1]))
end
  ╠═╡ =#

# ╔═╡ ae1d99be-fc61-403d-8ec7-91c26a8ed5c7
begin
#Exp 28
# flux = 296 kW/m2
# flowrate = 4.52 lpm
...
# ╔═╡ cbdf28fa-1771-42c5-8fc7-3aadc046f100
# begin 
# #Exp 59
# # flux = 219 kW/m2
# # flowrate = 4.80 lpm
...
# ╔═╡ b0e01b43-2399-4212-be47-89269d186482
begin
plot(
	sol.t, 
	sol[T(t,x)][:,1], 
	...
	scatter!(
		xc_data, 
		y2c_data,  
		label = "Experimental at 10 mm",
		xlabel = "Time (s)", 
		ylabel = "Temperature (K)")
end

# ╔═╡ 34124456-535d-4e03-b4cb-4b12b8ad60b2
begin
...
plot(x_num, T_exp_at_time_array, xlabel="Position x (m)", ylabel="Temperature (K)", linewidth=2, ylim=(y_min, y_max), xlim=(x__min, x__T3), title="Temperature Profile at t = $time_point s", label="T")
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "a0a0b9b9aca71b6b70fb6e92bbd05bb3a2a2a6dd"
...
git-tree-sha1 = "09c91a1ed02e998cc8666186a498fa71a23a4c00"
...
[[deps.MethodOfLines]]
deps = ["Combinatorics", "DiffEqBase", "DomainSets", "IfElse", "Interpolations", "Latexify", "LinearAlgebra", "ModelingToolkit", "OrdinaryDiffEq", "PDEBase", "PrecompileTools", "SciMLBase", "StaticArrays", "SymbolicUtils", "Symbolics", "TermInterface"]
git-tree-sha1 = "d637181aa6edf67a280d0d5958b30eaf946c64f2"
...
git-tree-sha1 = "1c94b750d4ae89d3bd6172929171adff961e2d2b"
