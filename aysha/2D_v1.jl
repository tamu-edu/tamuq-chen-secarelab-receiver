# ============================================================================
# 2D_v1.jl - Multi-Domain High-Resolution 2D Axisymmetric Macro-ECM Solar Receiver Model
# ============================================================================
# Solves 2D solid/insulation/casing energy equation T_s(r, z, t) coupled to
# quasi-steady 2D gas flow T_g(r, z, t), rear alumina exit tube, adaptor sleeve,
# housing insulation, and water-cooled flange sink (25°C).
# Formulated using SciML OrdinaryDiffEq / DifferentialEquations.jl.
# ============================================================================
...
Base.@kwdef struct SolidProperties2D
    ...
    felt_conductivity::Float64 = 0.08           # Alumina felt thermal conductivity [W/m/K]
...
Base.@kwdef struct LossParameters2D
    front_convection::Float64 = 10.0            # Front face h [W/m2/K]
...
function cell_thermal_properties2D(i, T, p::ModelParameters2D, grid::DiscretizationGrid2D)
    ...
    elseif i <= (grid.nr_rec + grid.nr_felt)
        k_val = p.solid.felt_conductivity
...
function _gas_profile2D!(gas, qgas, hcell, gas_rear, qgas_rear, hrear, solid_temp, T_rear, time, p, op, grid)
    ...
    for i in 1:nr_rec
        mdot_ring = mdot_active * (grid.area_flow[i] / sum(grid.area_flow[1:nr_rec]))
        gas[i, 1] = inlet
    ...
    total_enthalpy = sum(mdot_active * (grid.area_flow[i] / sum(grid.area_flow[1:nr_rec])) * air_heat_capacity(gas[i, end]) * gas[i, end] for i in 1:nr_rec)
    total_mcp = sum(mdot_active * (grid.area_flow[i] / sum(grid.area_flow[1:nr_rec])) * air_heat_capacity(gas[i, end]) for i in 1:nr_rec)
...
function receiver_ode2D!(du, u, context, time)
    ...
    for i in 1:nr_total
        ...
        Qfront_conv = loss.front_convection * area_f * (T_s[i, 1] - ambient)
...
function unpack_parameters2D(theta::AbstractVector, p_base::ModelParameters2D=default_parameters2D())
    ...
    s = SolidProperties2D(
        ...
        felt_conductivity = p_base.solid.felt_conductivity,
    ...
    l = LossParameters2D(
        front_convection = p_base.losses.front_convection,
...
const LB_2D = [0.5, 0.10, 0.001, 0.10, 5.0, 0.01, 100.0, 0.50, 0.50, 0.40, 0.40, 0.00, 0.00, 0.10]
const UB_2D = [6.0, 0.60, 0.100, 3.00, 35.0, 0.40, 500.0, 1.80, 1.80, 1.50, 1.00, 0.60, 0.50, 5.00]
...
function calibrate2D(solve_fn; heating_cases=("E67", "E76", "E80"),
    ...
    function counted_objective(theta, p_data)
        ...
        if isfinite(val)
            println("[calibrate2D] Eval $(evals[]): loss = $(round(val, digits=4))")
            flush(stdout)
        end
        return val
    end
    ...
    println("[calibrate2D] Starting NLopt.LN_BOBYQA parameter optimization...")
