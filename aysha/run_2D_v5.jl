# ============================================================================
# run_2D_v5.jl - 2D_v5 Runner with Enhanced Visibility & Radial Decoupling
# ============================================================================
...
begin # Directory setup
    const OUTPUT_DIR_2D_v5 = joinpath(@__DIR__, "summaries", "2D_v5")
    const PLOT_DIR_2D_v5 = joinpath(OUTPUT_DIR_2D_v5, "plots")
    const TRANSIENT_DIR_2D_v5 = joinpath(PLOT_DIR_2D_v5, "transients")
    const PROFILES_DIR_2D_v5 = joinpath(PLOT_DIR_2D_v5, "2d_profiles")
    const AXIAL_DIR_2D_v5 = joinpath(PLOT_DIR_2D_v5, "axial_profiles")
    const DIAGNOSTICS_DIR_2D_v5 = joinpath(OUTPUT_DIR_2D_v5, "diagnostics")
    
    mkpath(OUTPUT_DIR_2D_v5)
    mkpath(PLOT_DIR_2D_v5)
    mkpath(TRANSIENT_DIR_2D_v5)
    mkpath(PROFILES_DIR_2D_v5)
    mkpath(AXIAL_DIR_2D_v5)
    mkpath(DIAGNOSTICS_DIR_2D_v5)
    ...
    const SENSOR_STYLES_2D = Dict(
        :T8 => :solid, :T12_perim => :dash, :T11_perim => :dash,
...
function run_single_case_2D(simulation_id, p=default_parameters2D(); is_cooling=false)
    ...
    grid = Receiver2D_v5.build_grid2D(p)
...
function plot_steady_parity_2D(steady_results)
    p_plot = plot(
        title="2D_v5 Multi-Domain Fitted Steady-State Parity Plot",
    ...
    path = joinpath(PLOT_DIR_2D_v5, "steady_comparison_2D_v5.png")
    savefig(p_plot, path)
    return path
end
...
function plot_axial_profile_2D(sim_id, case_data)
    ...
    path = joinpath(AXIAL_DIR_2D_v5, "axial_profile_$(sim_id)_2D_v5.png")
    savefig(p_plot, path)
    return path
end
...
function plot_transient_2D(sim_id, case_data; is_cooling=false)
    ...
    p_plot = plot(
        title="2D_v5 Fitted Transient: $sim_id $(is_cooling ? "(Cooling)" : "(Heating)")",
    ...
    for (j, sensor) in enumerate(SENSOR_NAMES_2D)
        ...
        plot!(p_plot, times, model[:, j], label="$(sensor) Model", lw=2.5, color=col, linestyle=style)
        scatter!(p_plot, times, exp[:, j], label="$(sensor) Exp", ms=2, color=col, markerstrokewidth=0)
    end
    
    suffix = is_cooling ? "_cooling" : ""
    path = joinpath(TRANSIENT_DIR_2D_v5, "transient_$(sim_id)$(suffix)_2D_v5.png")
    savefig(p_plot, path)
    return path
end
...
function plot_2d_heatmap(sim_id, result)
    ...
    p_heat = heatmap(
        z_mm, r_mm, T_final,
        title="2D_v5 Multi-Domain Temperature Field T_s(r, z) [K] - $sim_id",
        xlabel="Axial Position z (mm)", ylabel="Radial Radius r (mm)",
        c=:inferno, aspect_ratio=:equal, clim=(minimum(T_final), maximum(T_final))
    )
    
    path = joinpath(PROFILES_DIR_2D_v5, "heatmap_2D_$(sim_id)_2D_v5.png")
    savefig(p_heat, path)
    return path
end
...
begin # Main Execution Loop
    println("==========================================================================")
    println("Running 2D_v5 Multi-Domain Calibration with Decoupled chi_r & Visibility Fix...")
    println("==========================================================================")
    ...
    open(joinpath(OUTPUT_DIR_2D_v5, "optimization_summary_2D_v5.txt"), "w") do io
        println(io, "objective=$(calib.objective)")
        println(io, "return_code=$(calib.retcode)")
        println(io, "parameters=$(pack_parameters2D(p_fitted))")
    end
    
    write_parameters_2D(joinpath(OUTPUT_DIR_2D_v5, "parameters_fitted_2D_v5.csv"), p_fitted)
    
    println("[run_2D_v5] Computing metrics and generating figures for fitted 2D_v5 model...")
    metrics = compute_all_metrics_2D(p_fitted)
    open(joinpath(OUTPUT_DIR_2D_v5, "analysis_results_2D_v5.csv"), "w") do io
    ...
    steady_path = joinpath(OUTPUT_DIR_2D_v5, "steady_results_2D_v5.csv")
    ...
    slopes_path = joinpath(OUTPUT_DIR_2D_v5, "flow_slopes_2D_v5.csv")
    ...
    for sim_id in IDs
        ...
        write_cell_diagnostics_2D(
            joinpath(DIAGNOSTICS_DIR_2D_v5, "cell_diagnostics_$(sim_id)_2D_v5.csv"),
    ...
    println("==========================================================================")
    println("Multi-Domain 2D_v5 Calibration and Runner Execution Complete.")
