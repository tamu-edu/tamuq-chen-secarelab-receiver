...
begin #define parameters
    ...
    Vi = w * w * n_channel * L#*deltax #m3
    Av = 4*(w*L) / (w^2*L) #1662 #specific area (m-1)
    ...
    #begin
        ...
        eq1 = [ 
               ...
               Vf* ρf * Dt(Tf(t, x)) ~ 1/Cpf * ((- Vf* Cpf * ρf * V * Dx(Tf(t,x))) + (hlocal/1000) * Av * Vi * ((Ts(t,x) - Tf(t,x))))    
              ]
        bcs1 = [
            Ts(0., x) ~ Tamb, # initial
            Tf(0., x) ~ Tamb, # initial
    ...
    initialerror=(loss(p0, []))
        ...
        optsol = solve(optprob, NLopt.GN_MLSL_LDS(), local_method = NLopt.LN_NELDERMEAD(), maxtime = 180, local_maxiters = 1000)
...
begin
    ...
    plot1 =  plot(
        ...
        scatter!(
            ...
            plot!(
                psol.t, 
                psol.u[Ts(t,x)][:,end-1], # around 5 mm in T8
                label = "Num 135")
...
plot2 = plot(
   ...
   plot!(
    psol.t, 
    psol.u[Tf(t,x)][:,3], # around 5 mm in T8
    label = "gas 5mm")
