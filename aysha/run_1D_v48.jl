# run_1D_v48.jl
# Calibration, Verification, and Post-Processing Runner for Entire Converter Model 1D_v48.
...
begin # imports & directory setups
    ...
    const RUNNER_OUTPUT_DIR_v48 = joinpath(@__DIR__, "summaries", "1D_v48")
    const RUNNER_PLOT_DIR_v48 = joinpath(RUNNER_OUTPUT_DIR_v48, "plots")
    const RUNNER_TRANSIENT_DIR_v48 = joinpath(RUNNER_PLOT_DIR_v48, "transients")
    const RUNNER_AXIAL_DIR_v48 = joinpath(RUNNER_PLOT_DIR_v48, "axial_profiles")

    mkpath(RUNNER_OUTPUT_DIR_v48)
    mkpath(RUNNER_PLOT_DIR_v48)
    mkpath(RUNNER_TRANSIENT_DIR_v48)
    mkpath(RUNNER_AXIAL_DIR_v48)

    function runner_env_v48(key, default)
        haskey(ENV, key) ? ENV[key] : default
    end

    function runner_bool_v48(key, default)
        val = runner_env_v48(key, string(default))
        return lowercase(strip(val)) in ("1", "true", "yes", "on")
    end

    function runner_float_v48(key, default)
        parse(Float64, runner_env_v48(key, string(default)))
    end

    function runner_int_v48(key, default)
        parse(Int, runner_env_v48(key, string(default)))
    end

    const RUNNER_FIT_STAGE_v48 = runner_env_v48("RECEIVER1D_v48_FIT_STAGE", "full")
    const RUNNER_FIT_SECONDS_v48 = runner_float_v48("RECEIVER1D_v48_FIT_SECONDS", 600.0)
    const RUNNER_FIT_ITERATIONS_v48 = runner_int_v48("RECEIVER1D_v48_FIT_ITERATIONS", 500)
    const RUNNER_FIT_NODES_v48 = runner_int_v48("RECEIVER1D_v48_FIT_NODES", 15)
    const RUNNER_CALIBRATION_DATASET_v48 = runner_env_v48("RECEIVER1D_v48_CALIBRATION_DATASET", "heating")
    const RUNNER_WRITE_PLOTS_v48 = runner_bool_v48("RECEIVER1D_v48_WRITE_PLOTS", true)
end

begin # metrics & analysis helpers
    const SENSOR_NAMES_v48 = ["T8", "T12_perim", "T11_perim", "T9_core", "T10_core", "T3", "T2"]

    function compute_t90_v48(time, trace; is_cooling=false)
        t0 = time[1]
        t_end = time[end]
        v0 = trace[1]
        v_end = trace[end]
    ...
    function evaluate_case_metrics_v48(p, simulation_id; is_cooling=false, nodes=15)
        id_str = string(simulation_id)
        outputs, result, experiment = solve_case_v48(p, id_str; is_cooling=is_cooling, nodes=nodes)
        ...
        for j in 1:7
            ...
            t90_m = compute_t90_v48(time, m_trace; is_cooling=is_cooling)
            t90_e = compute_t90_v48(time, e_trace; is_cooling=is_cooling)
            t90_err = t90_m - t90_e
            
            shape = signal_loss_v48(time, m_trace, e_trace)
            
            push!(metrics, (
                simulation_id=id_str,
                phase=is_cooling ? "cooling" : "heating",
                sensor=SENSOR_NAMES_v48[j],
                rmse_K=rmse,
    ...
    function compute_metrics_v48(p; nodes=15)
        all_metrics = []
        for id in keys_heating_v48
            m, _, _, _ = evaluate_case_metrics_v48(p, id; is_cooling=false, nodes=nodes)
            append!(all_metrics, m)
        end
        for id in keys_cooling_v48
            m, _, _, _ = evaluate_case_metrics_v48(p, id; is_cooling=true, nodes=nodes)
            append!(all_metrics, m)
        end
        return all_metrics
    end
...
begin # steady state accounting & reporting
    function build_steady_results_v48(p; nodes=15)
        rows = []
        for id in keys_heating_v48
            id_str = string(id)
            conditions, time, experiment, data = experimental_case_v48(id_str)
            ...
            outputs, result, _ = solve_case_v48(p, id_str; is_cooling=false, nodes=nodes)
            t_end = time[end]
            balance = compute_energy_balance_v48(result, operating, t_end, p; nodes=nodes)
            
            push!(rows, (
                simulation_id=id_str,
                flow_lpm=flow,
                irradiance=irradiance,
                power_scale=absorbed_power_scale_v48(irradiance, p),
...
begin # flow slope calculation
    function compute_flow_slopes_v48(steady_rows)
        ...
        for (irr, group) in clusters
            ...
            if length(flows) >= 2
                for (sig_idx, sig_name) in enumerate(SENSOR_NAMES_v48)
...
begin # invariant checking
    function compute_invariants_v48(p, steady_rows)
        invs = []
        C_tot = participating_total_heat_capacity_v48(p, 15)
...
begin # table export writers
    function write_parameters_v48(path, variant)
        p = variant.params
        names = [
        ...
        open(path, "w") do io
            ...
            println(io, "# reference,measured_C_eff_J_K,$MEASURED_ASSEMBLY_CAPACITANCE_v48")
        end
    end

    function write_metrics_v48(path, metrics)
        open(path, "w") do io
    ...
    function write_steady_results_v48(path, rows)
        open(path, "w") do io
    ...
    function write_flow_slopes_v48(path, slopes)
        open(path, "w") do io
    ...
    function write_invariants_v48(path, invs)
        open(path, "w") do io
...
begin # plotting generators
    function plot_transient_cases_v48(p; nodes=15)
        for id in vcat(keys_heating_v48, keys_cooling_v48)
            is_cool = id in keys_cooling_v48
            id_str = string(id)
            outputs, result, experiment = solve_case_v48(p, id_str; is_cooling=is_cool, nodes=nodes)
            data = is_cool ? measurements_cooling : measurements
            time = observation_time(data, id_str)
            
            p_plot = plot(title="1D_v48 Transient: Case $(id_str) ($(is_cool ? "Cooling" : "Heating"))",
            ...
            out_file = joinpath(RUNNER_TRANSIENT_DIR_v48, "transients_$(id_str)_1D_v48.png")
            savefig(p_plot, out_file)
        end
    end

    function plot_axial_profiles_v48(p; nodes=25)
        for id in keys_heating_v48
            id_str = string(id)
            outputs, result, experiment = solve_case_v48(p, id_str; is_cooling=false, nodes=nodes)
            
            p_ax = plot(title="1D_v48 Axial Temperature Profile: Case $(id_str)",
            ...
            exp_z = [sensor_positions[:T8], WALL_CHAIN_POSITIONS_v48.T12, WALL_CHAIN_POSITIONS_v48.T11,
            ...
            out_file = joinpath(RUNNER_AXIAL_DIR_v48, "axial_profile_$(id_str)_fitted_energy_accounting_1D_v48.png")
            savefig(p_ax, out_file)
        end
    end

    function steady_comparison_plot_v48(steady_results; title_suffix="fitted")
        p = scatter(
            title="1D_v48 Steady Parity Comparison ($(title_suffix))",
    ...
    function write_cell_diagnostics_v48(p; nodes=25)
        for id in keys_heating_v48
            id_str = string(id)
            outputs, result, experiment = solve_case_v48(p, id_str; is_cooling=false, nodes=nodes)
            conditions, time, _, data = experimental_case_v48(id_str)
            irradiance = conditions[Io]
            delivered = absorbed_power_scale_v48(irradiance, p) * irradiance * A_frt_v48
            chi = core_source_fraction_v48(p)
            core_absorbed = chi * delivered
            perim_absorbed = (1.0 - chi) * delivered
            dx = L_v48 / length(result.z_solid)
            Trear = result.rear_reservoir_temperature[:, end]

            path = joinpath(RUNNER_AXIAL_DIR_v48, "cell_diagnostics_$(id_str)_1D_v48.csv")
            open(path, "w") do io
...
begin # optimization runner
    function run_calibration_v48(; stage=RUNNER_FIT_STAGE_v48, max_time=RUNNER_FIT_SECONDS_v48,
                                nodes=RUNNER_FIT_NODES_v48, dataset=RUNNER_CALIBRATION_DATASET_v48)
        println("[run_1D_v48] Starting Option A calibration on heating runs (stage=$(stage), dataset=$(dataset), nodes=$(nodes), max_time=$(max_time) s)...")
        res = calibrate_v48(
            nodes=nodes,
            maximum_iterations=RUNNER_FIT_ITERATIONS_v48,
            maximum_time_seconds=max_time,
            stage=stage,
            dataset=dataset,
        )
        println("[run_1D_v48] Optimization completed with status: $(res.retcode)")
        println("[run_1D_v48] Final objective: $(res.objective)")
        return (params=res.parameters, objective=res.objective, retcode=res.retcode)
    end
end

function main_1D_v48()
    println("============================================================================")
    println("1D_v48 Calibration and Analysis Pipeline")
    println("============================================================================")

    run_cal = runner_bool_v48("RECEIVER1D_v48_RUN_CALIBRATION", true)
    
    variant = if run_cal
        run_calibration_v48()
    else
        println("[run_1D_v48] Using baseline v48 parameter vector.")
        (params=pnew_v48, objective=calibration_loss_v48(pnew_v48), retcode=:Manual)
    end

    # Write summary
    sum_path = joinpath(RUNNER_OUTPUT_DIR_v48, "optimization_summary_1D_v48.txt")
    open(sum_path, "w") do io
        println(io, "objective=$(variant.objective)")
        println(io, "return_code=$(variant.retcode)")
        println(io, "stage=$(RUNNER_FIT_STAGE_v48)")
        println(io, "calibration_dataset=$(RUNNER_CALIBRATION_DATASET_v48)")
        println(io, "parameters=$(variant.params)")
    end

    # Write parameters
    param_path = joinpath(RUNNER_OUTPUT_DIR_v48, "parameters_fitted_energy_accounting_1D_v48.csv")
    write_parameters_v48(param_path, variant)

    # Compute & write metrics
    println("[run_1D_v48] Computing multi-signal metrics...")
    metrics = compute_metrics_v48(variant.params)
    write_metrics_v48(joinpath(RUNNER_OUTPUT_DIR_v48, "analysis_results_fitted_energy_accounting_1D_v48.csv"), metrics)

    # Build & write steady results
    println("[run_1D_v48] Computing steady-state results and conservation residuals...")
    steady_results = build_steady_results_v48(variant.params)
    write_steady_results_v48(joinpath(RUNNER_OUTPUT_DIR_v48, "steady_results_fitted_energy_accounting_1D_v48.csv"), steady_results)

    # Compute flow slopes
    println("[run_1D_v48] Computing flow slopes...")
    slopes = compute_flow_slopes_v48(steady_results)
    write_flow_slopes_v48(joinpath(RUNNER_OUTPUT_DIR_v48, "flow_slopes_fitted_energy_accounting_1D_v48.csv"), slopes)

    # Compute invariants
    println("[run_1D_v48] Computing invariants...")
    inv_rows = compute_invariants_v48(variant.params, steady_results)
    write_invariants_v48(joinpath(RUNNER_OUTPUT_DIR_v48, "invariant_summary_1D_v48.csv"), inv_rows)

    # Plotting & Diagnostics
    if RUNNER_WRITE_PLOTS_v48
        println("[run_1D_v48] Generating transient plots...")
        plot_transient_cases_v48(variant.params)

        println("[run_1D_v48] Generating axial temperature profile plots...")
        plot_axial_profiles_v48(variant.params)

        println("[run_1D_v48] Generating steady-state comparison parity plot...")
        p_steady = steady_comparison_plot_v48(steady_results; title_suffix="fitted")
        savefig(p_steady, joinpath(RUNNER_PLOT_DIR_v48, "steady_comparison_fitted_energy_accounting_1D_v48.png"))

        println("[run_1D_v48] Writing cell-by-cell axial diagnostics CSVs...")
        write_cell_diagnostics_v48(variant.params)
    end

    println("[run_1D_v48] Pipeline completed successfully. Output in: $RUNNER_OUTPUT_DIR_v48")
end
