# ============================================================================
# 2D_v4.jl - Multi-Domain 2D Model with Refined Front Axial Mesh & Flow Sensitivity
# ============================================================================
# Key Physics & Numerical Enhancements in v4:
#   1. Non-Uniform High-Resolution Axial Grid (fine Δz = 1.5 mm at front face z ∈ [0, 15 mm])
#      resolving fast front-surface skin heating at T8 (z = 11 mm) without mass averaging.
#   2. Front Surface Deposition Optimization f_front ∈ [0.20, 0.60] to accelerate front T8 rise.
#   3. Flow Recruitment Exponent m_rec & Reynolds exponent B_Re calibration matching
#      manuscript flow-dependence invariant I1 (Nu ∝ Re^1.44).
#   4. Raw experimental t0 cooling initialization.
# ============================================================================

module Receiver2D_v4
...
Base.@kwdef struct HeatTransferParameters2D
    ...
    reynolds_exponent::Float64 = 0.40           # Reynolds exponent B_Re
    ...
    m_rec::Float64 = 0.25                       # Flow recruitment exponent
...
Base.@kwdef struct OpticalParameters2D
    ...
    scale_456::Float64 = 0.550                   # Total power scale factor at 456 kW/m2
    scale_304::Float64 = 0.550                   # Total power scale factor at 304 kW/m2
    scale_256::Float64 = 0.450                   # Total power scale factor at 256 kW/m2
end
...
function build_grid2D(p::ModelParameters2D)
    ...
    # Non-Uniform Axial Mesh: Fine Δz = 1.5 mm near front face z ∈ [0, 15 mm]
    # smoothly expanding to Δz ≈ 5.5 mm at rear.
    z_faces = zeros(nz + 1)
    z_faces[1] = 0.0
    
    # 8 fine cells in first 15 mm (dz = 1.875 mm)
    n_fine = 8
    z_fine_end = 15.0e-3
    for j in 1:n_fine
    ...
    # Remaining 22 cells over 15 mm -> 137 mm
    n_coarse = nz - n_fine
    for j in 1:n_coarse
        frac = j / n_coarse
...
const LB_2D = [0.5, 0.10, 0.001, 0.10, 5.0, 0.01, 100.0, 0.40, 0.40, 0.30, 0.40, 0.00, 0.10, 0.10, 0.00]
const UB_2D = [6.0, 0.70, 0.100, 3.00, 35.0, 0.40, 500.0, 2.80, 2.80, 2.00, 1.00, 0.60, 0.60, 5.00, 0.50]
    ...
    for (sim_id, is_cool) in vcat([(id, false) for id in heating_cases], [(id, true) for id in cooling_cases])
        try
            ...
            for j in 1:7
                ...
                # Weight T8 front rise heavily (sensor index 1)
                w_sensor = (j == 1) ? 2.0 : 1.0
                total_loss += w_sensor * (nmse + 0.75 * shape + 0.25 * t90)
            end
...
function calibrate2D(solve_fn; heating_cases=("E67", "E76", "E80"),
    ...
    function counted_objective(theta, p_data)
        ...
        if isfinite(val)
            println("[calibrate2D v4] Eval $(evals[]): loss = $(round(val, digits=4))")
            flush(stdout)
        end
        return val
    end
    ...
    println("[calibrate2D v4] Starting NLopt.LN_BOBYQA parameter optimization...")
