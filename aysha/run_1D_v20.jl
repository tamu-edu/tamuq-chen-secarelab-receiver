# ============================================================================
# run_1D_v20.jl - Energy-accounting 2-Zone Core/Perimeter Runner
# ============================================================================
# Runner for v20: energy-accounting two-zone macro model.
# Fits total power scales, core/perimeter source partition, active/bypass gas
# participation, core-perimeter coupling, thermal mass, and rear-tube split.
...
begin # runner configuration
    runner_int_v20(name, default) = parse(Int, get(ENV, name, string(default)))
    runner_float_v20(name, default) = parse(Float64, get(ENV, name, string(default)))

    const RUNNER_PLOT_NODES_v20 = runner_int_v20("RECEIVER1D_v20_RUNNER_PLOT_NODES", default_nodes)
    const RUNNER_FIT_NODES_v20 = runner_int_v20("RECEIVER1D_v20_FIT_NODES", 15)
    const RUNNER_FIT_ITERATIONS_v20 = runner_int_v20("RECEIVER1D_v20_FIT_ITERATIONS", 100)
    const RUNNER_FIT_SECONDS_v20 = runner_float_v20("RECEIVER1D_v20_FIT_SECONDS", 360.0)

    const RUNNER_OUTPUT_DIR_v20 = joinpath(@__DIR__, "summaries", "1D_v20")
    const RUNNER_PLOT_DIR_v20 = joinpath(RUNNER_OUTPUT_DIR_v20, "plots")
    const RUNNER_TRANSIENT_DIR_v20 = joinpath(RUNNER_PLOT_DIR_v20, "transients")
    const RUNNER_AXIAL_DIR_v20 = joinpath(RUNNER_PLOT_DIR_v20, "axial_profiles")
    const RUNNER_DIAGNOSTIC_DIR_v20 = joinpath(RUNNER_OUTPUT_DIR_v20, "diagnostics")
    mkpath(RUNNER_OUTPUT_DIR_v20)
    mkpath(RUNNER_PLOT_DIR_v20)
    mkpath(RUNNER_TRANSIENT_DIR_v20)
    mkpath(RUNNER_AXIAL_DIR_v20)
    mkpath(RUNNER_DIAGNOSTIC_DIR_v20)
end

begin # runner helpers
    const RUNNER_SENSOR_COLORS_v20 = Dict(
        :T8 => :blue,
        :T12_perim => :red,
        :T11_perim => :green,
        :T9_core => :orange,
    ...
    const TEMPERATURE_AXIS_LIMITS_v20 = (280.0, 1400.0)

    function save_runner_plot_v20(plot_object, filename)
        path = joinpath(RUNNER_PLOT_DIR_v20, filename)
        savefig(plot_object, path)
        println("[run_1D_v20] Saved plot: $path")
        return path
    end

    absorbed_power_watts_v20(irradiance, scale) =
        ETA_ABS_FIXED_v20 * scale * irradiance * A_frt

    function write_parameters_v20(path, variant)
        ...
        names = (
            ...
            "f_perim_source", "beta_perim_1_m", "f_core_tube",
        )
        open(path, "w") do io
            ...
            println(io, "# fitted,G_core_perim_W_m_K,$(core_perimeter_conductance_per_length_v20(parameters))")
            println(io, "# fitted,C_perim_eff_J_K,$(perimeter_heat_capacity_total_v20(parameters))")
            println(io, "# fitted,k_perim_ref_W_m_K,$(perimeter_axial_conductivity_v20(900.0, parameters))")
            println(io, "# fitted,f_perim_source,$(perimeter_source_fraction_v20(parameters))")
            println(io, "# fitted,beta_perim_1_m,$(perimeter_source_attenuation_v20(parameters))")
            println(io, "# fitted,f_core_tube,$(core_tube_fraction_v20(parameters))")
            println(io, "# derived,C_core_eff_J_K,$(core_receiver_heat_capacity_v20(default_nodes))")
            println(io, "# derived,C_participating_eff_J_K,$(participating_assembly_heat_capacity_v20(parameters))")
            println(io, "# reference,measured_C_eff_J_K,$MEASURED_ASSEMBLY_CAPACITANCE_v20")
        end
        return path
    end

    function compute_metrics_v20(p=pnew_v20; heating_keys=sim_key_heat,
                                cooling_keys=sim_key_cool, nodes=RUNNER_PLOT_NODES_v20)
        metrics = NamedTuple[]
        for (keys, cooling) in ((heating_keys, false), (cooling_keys, true))
            for simulation_id in keys
                outputs, result, experiment = solve_case_v20(
                ...
                for (j, sensor) in enumerate(sensors)
                    push!(metrics, (
                        ...
                        shape_loss=normalized_slope_mse_v20(outputs[:, j], experiment[:, j]),
                    ))
                end
            end
        end
    ...
    function write_metrics_v20(path, metrics)
        open(path, "w") do io
    ...
    function build_steady_results_v20(params=pnew_v20; keys=sim_key_heat, nodes=RUNNER_PLOT_NODES_v20)
        results = NamedTuple[]
        core_weights = solar_weights_v5(params[13], params[6], nodes)
        perim_weights = axial_exponential_weights_v20(
            perimeter_source_attenuation_v20(params), nodes, L / nodes,
        )
        for simulation_id in keys
            outputs, result, experiment = solve_case_v20(params, simulation_id; nodes=nodes)
            conditions = simulation_conditions[simulation_id]
            irradiance = conditions[Io]
            scale = absorbed_power_scale_v20(irradiance, params)
            nominal_absorbed = absorbed_power_watts_v20(irradiance, 1.0)
            scaled_absorbed = absorbed_power_watts_v20(irradiance, scale)
            f_perim_source = perimeter_source_fraction_v20(params)
            ...
            push!(results, (
                ...
                beta_perim_1_m=perimeter_source_attenuation_v20(params),
                ...
                G_core_perim_W_m_K=core_perimeter_conductance_per_length_v20(params),
                C_perim_eff_J_K=perimeter_heat_capacity_total_v20(params),
                C_participating_eff_J_K=participating_assembly_heat_capacity_v20(params, nodes),
                f_core_tube=core_tube_fraction_v20(params),
            ))
        end
        return results
    end

    function write_steady_results_v20(path, steady_results)
    ...
    function write_flow_slopes_v20(path, steady_results)
            ...
            for irradiance in irradiances
                ...
                for sensor in sensors
                    ...
                    println(io, join((
                        irradiance,
                        sensor,
                        slope_v20(flows, model_values),
                        slope_v20(flows, experiment_values),
                    ), ','))
                end
            end
        end
    ...
    function steady_comparison_plot_v20(steady_results; title_suffix="")
        sensors = (:T8, :T12_perim, :T11_perim, :T9_core, :T10_core, :T3, :T2)
        plot_object = plot(
            title="1D_v20 $title_suffix steady-state comparison",
            xlabel="Model temperature (K)",
            ylabel="Experiment temperature (K)",
            legend=:outerright,
            aspect_ratio=:equal,
            xlims=TEMPERATURE_AXIS_LIMITS_v20,
            ylims=TEMPERATURE_AXIS_LIMITS_v20,
        )
        for sensor in sensors
            ...
            scatter!(plot_object, model_values, experiment_values;
                     label=String(sensor), color=RUNNER_SENSOR_COLORS_v20[sensor],
                     ms=4, markerstrokewidth=0)
        end
        lower, upper = TEMPERATURE_AXIS_LIMITS_v20
        plot!(plot_object, [lower, upper], [lower, upper];
              label="1:1", color=:gray, linestyle=:dash)
        return plot_object
    end

    function axial_profile_plot_v20(simulation_id, params=pnew_v20;
                                   nodes=RUNNER_PLOT_NODES_v20,
                                   variant_name="v20")
        outputs, result, experiment = solve_case_v20(params, simulation_id; nodes=nodes)
        conditions = simulation_conditions[simulation_id]
        ...
        scale = absorbed_power_scale_v20(conditions[Io], params)
        title_text = "1D_v20 $variant_name axial profile: $simulation_id"
        ...
        plot_object = plot(
            ...
            legend=:outerright,
            grid=true,
            xlims=(0.0, 1000.0 * (L + REAR_TUBE_LENGTH_v20)),
            ylims=TEMPERATURE_AXIS_LIMITS_v20,
        )
        ...
        scatter!(plot_object, [T3_SAMPLE_POSITION_v20 * 1000.0], [experiment[end, 6]];
                 label="T3 experiment", color=:purple, markershape=:star5,
                 markerstrokewidth=0, ms=7)
        scatter!(plot_object, [CAVITY_LENGTH_v20 * 1000.0], [experiment[end, 7]];
                 label="T2 experiment", color=:black, markershape=:diamond,
                 markerstrokewidth=0, ms=5)
        scatter!(plot_object, [CAVITY_LENGTH_v20 * 1000.0], [outputs[end, 7]];
                 label="T2 model", color=:black, markershape=:xcross,
                 markerstrokewidth=1, ms=5)
        vline!(plot_object, [L * 1000.0, (L + REAR_TUBE_CAVITY_LENGTH_v20) * 1000.0];
               label=false, color=:gray, linestyle=:dot, lw=1)
        return plot_object
    end

    function plot_case_v20(simulation_id, params=pnew_v20; is_cooling=false,
                          nodes=RUNNER_PLOT_NODES_v20, variant_name="v20")
        outputs, result, experiment = solve_case_v20(params, simulation_id; is_cooling=is_cooling, nodes=nodes)
        sensors = (:T8, :T12_perim, :T11_perim, :T9_core, :T10_core, :T3, :T2)
        colors = (:blue, :red, :green, :orange, :brown, :purple, :black)
        
        plot_object = plot(
            title="1D_v20 $variant_name transient: $simulation_id",
    ...
    function write_cell_diagnostics_v20(path, simulation_id, params;
                                       nodes=RUNNER_PLOT_NODES_v20,
                                       variant_name="v20")
        outputs, result, experiment = solve_case_v20(params, simulation_id; nodes=nodes)
        conditions = simulation_conditions[simulation_id]
        scaled_absorbed = absorbed_power_watts_v20(
            conditions[Io], absorbed_power_scale_v20(conditions[Io], params),
        )
        f_perim_source = perimeter_source_fraction_v20(params)
        core_absorbed = (1.0 - f_perim_source) * scaled_absorbed
        perim_absorbed = f_perim_source * scaled_absorbed
        ...
        open(path, "w") do io
            println(io, "variant,simulation_id,z_mm,Tperim_K,Tcore_K,Tgas_in_K,Tgas_out_K,Nu,h_W_m2K,k_perim_W_m_K,effectiveness,Qgas_W,Qcore_source_W,Qperim_source_W,Qcore_perim_W,active_flow_fraction")
            for i in eachindex(result.z_solid)
                println(io, join((
                    ...
                    perimeter_axial_conductivity_v20(result.perim_temperature[i, end], params),
                    ...
                    core_perimeter_conductance_per_length_v20(params) *
                    dx *
...
begin # execution & output generation
    println("[run_1D_v20] Running energy-accounting 2-Zone Core/Perimeter Runner.")
    println("[run_1D_v20] Fitting total power scales, source partition, active/bypass gas mixing, and core-perimeter/rear parameters...")

    fit = calibrate_v20(
        nodes=RUNNER_FIT_NODES_v20,
        maximum_iterations=RUNNER_FIT_ITERATIONS_v20,
        maximum_time_seconds=RUNNER_FIT_SECONDS_v20,
    )
    fitted_params = fit.parameters

    open(joinpath(RUNNER_OUTPUT_DIR_v20, "optimization_summary_1D_v20.txt"), "w") do io
        println(io, "objective=$(fit.objective)")
        println(io, "return_code=$(fit.retcode)")
        println(io, "parameters=$(fit.parameters)")
    end

    variants_v20 = NamedTuple[
        (name="initial_energy_accounting", params=copy(pnew_v20)),
    ...
    for variant in variants_v20
        println("[run_1D_v20] Running variant $(variant.name).")
        write_parameters_v20(
            joinpath(RUNNER_OUTPUT_DIR_v20, "parameters_$(variant.name)_1D_v20.csv"),
            variant,
        )
        metrics = compute_metrics_v20(variant.params; nodes=RUNNER_PLOT_NODES_v20)
        write_metrics_v20(
            joinpath(RUNNER_OUTPUT_DIR_v20, "analysis_results_$(variant.name)_1D_v20.csv"),
            metrics,
        )
        for row in metrics
            push!(summary_rows, merge(row, (variant=variant.name,)))
        end
        steady_results = build_steady_results_v20(
            variant.params; nodes=RUNNER_PLOT_NODES_v20,
        )
        write_steady_results_v20(
            joinpath(RUNNER_OUTPUT_DIR_v20, "steady_results_$(variant.name)_1D_v20.csv"),
            steady_results,
        )
        write_flow_slopes_v20(
            joinpath(RUNNER_OUTPUT_DIR_v20, "flow_slopes_$(variant.name)_1D_v20.csv"),
            steady_results,
        )
        save_runner_plot_v20(
            steady_comparison_plot_v20(steady_results; title_suffix=variant.name),
            "steady_comparison_$(variant.name)_1D_v20.png",
        )
        ...
        for simulation_id in axial_keys
            save_runner_plot_v20(
                axial_profile_plot_v20(
                    simulation_id, variant.params; variant_name=variant.name,
                    nodes=RUNNER_PLOT_NODES_v20,
                ),
                joinpath("axial_profiles", "axial_profile_$(simulation_id)_$(variant.name)_1D_v20.png"),
            )
            write_cell_diagnostics_v20(
                joinpath(RUNNER_DIAGNOSTIC_DIR_v20, "cell_diagnostics_$(simulation_id)_$(variant.name)_1D_v20.csv"),
                simulation_id, variant.params; variant_name=variant.name,
                nodes=RUNNER_PLOT_NODES_v20,
            )
        end
        ...
        for simulation_id in transient_heat_keys
            save_runner_plot_v20(
                plot_case_v20(
                    ...
                    variant_name=variant.name, nodes=RUNNER_PLOT_NODES_v20,
                ),
                joinpath("transients", "transient_$(simulation_id)_$(variant.name)_1D_v20.png"),
            )
        end
        ...
        for simulation_id in transient_cool_keys
            save_runner_plot_v20(
                ...
                joinpath("transients", "transient_$(simulation_id)_$(variant.name)_cooling_1D_v20.png"),
            )
        end
    end

    summary_path = joinpath(RUNNER_OUTPUT_DIR_v20, "analysis_results_all_variants_1D_v20.csv")
    ...
    println("[run_1D_v20] Optimization completed with objective: $(fit.objective)")
    println("[run_1D_v20] Fitted parameters: $fitted_params")
    println("[run_1D_v20] Complete.")
    println("[run_1D_v20] Output directory: $RUNNER_OUTPUT_DIR_v20")
end
