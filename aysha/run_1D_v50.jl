# run_1D_v50.jl
# Calibration, Verification, and Post-Processing Runner for Entire Converter Model 1D_v50.
...
begin # imports & directory setups
    ...
    const RUNNER_OUTPUT_DIR_v50 = joinpath(@__DIR__, "summaries", "1D_v50")
    const RUNNER_PLOT_DIR_v50 = joinpath(RUNNER_OUTPUT_DIR_v50, "plots")
    const RUNNER_TRANSIENT_DIR_v50 = joinpath(RUNNER_PLOT_DIR_v50, "transients")
    const RUNNER_AXIAL_DIR_v50 = joinpath(RUNNER_PLOT_DIR_v50, "axial_profiles")

    mkpath(RUNNER_OUTPUT_DIR_v50)
    mkpath(RUNNER_PLOT_DIR_v50)
    mkpath(RUNNER_TRANSIENT_DIR_v50)
    mkpath(RUNNER_AXIAL_DIR_v50)

    function runner_env_v50(key, default)
        haskey(ENV, key) ? ENV[key] : default
    end

    function runner_bool_v50(key, default)
        val = runner_env_v50(key, string(default))
        return lowercase(strip(val)) in ("1", "true", "yes", "on")
    end

    function runner_float_v50(key, default)
        parse(Float64, runner_env_v50(key, string(default)))
    end

    function runner_int_v50(key, default)
        parse(Int, runner_env_v50(key, string(default)))
    end

    const RUNNER_FIT_STAGE_v50 = runner_env_v50("RECEIVER1D_v50_FIT_STAGE", "full")
    const RUNNER_FIT_SECONDS_v50 = runner_float_v50("RECEIVER1D_v50_FIT_SECONDS", 600.0)
    const RUNNER_FIT_ITERATIONS_v50 = runner_int_v50("RECEIVER1D_v50_FIT_ITERATIONS", 500)
    const RUNNER_FIT_NODES_v50 = runner_int_v50("RECEIVER1D_v50_FIT_NODES", 15)
    const RUNNER_CALIBRATION_DATASET_v50 = runner_env_v50("RECEIVER1D_v50_CALIBRATION_DATASET", "heating")
    const RUNNER_WRITE_PLOTS_v50 = runner_bool_v50("RECEIVER1D_v50_WRITE_PLOTS", true)
end

begin # metrics & analysis helpers
    const SENSOR_NAMES_v50 = ["T8", "T12_perim", "T11_perim", "T9_core", "T10_core", "T3", "T2"]

    function compute_t90_v50(time, trace; is_cooling=false)
        t0 = time[1]
        t_end = time[end]
        v0 = trace[1]
        v_end = trace[end]
    ...
    function evaluate_case_metrics_v50(p, simulation_id; is_cooling=false, nodes=15)
        id_str = string(simulation_id)
        outputs, result, experiment = solve_case_v50(p, id_str; is_cooling=is_cooling, nodes=nodes)
        ...
        for j in 1:7
            ...
            t90_m = compute_t90_v50(time, m_trace; is_cooling=is_cooling)
            t90_e = compute_t90_v50(time, e_trace; is_cooling=is_cooling)
            t90_err = t90_m - t90_e
            
            shape = signal_loss_v50(time, m_trace, e_trace)
            
            push!(metrics, (
                simulation_id=id_str,
                phase=is_cooling ? "cooling" : "heating",
                sensor=SENSOR_NAMES_v50[j],
                rmse_K=rmse,
    ...
    function compute_metrics_v50(p; nodes=15)
        all_metrics = []
        for id in keys_heating_v50
            m, _, _, _ = evaluate_case_metrics_v50(p, id; is_cooling=false, nodes=nodes)
            append!(all_metrics, m)
        end
        for id in keys_cooling_v50
            m, _, _, _ = evaluate_case_metrics_v50(p, id; is_cooling=true, nodes=nodes)
            append!(all_metrics, m)
        end
        return all_metrics
    end
...
begin # steady state accounting & reporting
    function build_steady_results_v50(p; nodes=15)
        rows = []
        for id in keys_heating_v50
            id_str = string(id)
            conditions, time, experiment, data = experimental_case_v50(id_str)
            ...
            outputs, result, _ = solve_case_v50(p, id_str; is_cooling=false, nodes=nodes)
            t_end = time[end]
            balance = compute_energy_balance_v50(result, operating, t_end, p; nodes=nodes)
            
            push!(rows, (
                simulation_id=id_str,
                flow_lpm=flow,
                irradiance=irradiance,
                power_scale=absorbed_power_scale_v50(irradiance, p),
...
begin # flow slope calculation
    function compute_flow_slopes_v50(steady_rows)
        ...
        for (irr, group) in clusters
            ...
            if length(flows) >= 2
                for (sig_idx, sig_name) in enumerate(SENSOR_NAMES_v50)
...
begin # invariant checking
    function compute_invariants_v50(p, steady_rows; loss_components=nothing)
        invs = []
        C_tot = total_participating_heat_capacity_v50(p; nodes=15)
        ...
        slopes = compute_flow_slopes_v50(steady_rows)
        slope_mae = mean(abs(s.error_K_per_LPM) for s in slopes)
        push!(invs, (metric="flow_slope_MAE_K_per_LPM", value=slope_mae,
                     target="diagnostic; lower is better", status="diagnostic"))

        if loss_components !== nothing
            push!(invs, (metric="active_flow_slope_loss", value=loss_components.slope_loss,
                         target="> 0 at unreconciled seed", status=loss_components.slope_loss > 0 ? "pass" : "fail"))
        end
...
begin # table export writers
    function write_parameters_v50(path, variant)
        p = variant.params
        names = [
        ...
        open(path, "w") do io
            ...
            println(io, "# reference,measured_C_eff_J_K,$MEASURED_ASSEMBLY_CAPACITANCE_v50")
        end
    end

    function write_metrics_v50(path, metrics)
        open(path, "w") do io
    ...
    function write_steady_results_v50(path, rows)
        open(path, "w") do io
    ...
    function write_flow_slopes_v50(path, slopes)
        open(path, "w") do io
    ...
    function write_invariants_v50(path, invs)
        open(path, "w") do io
...
begin # plotting generators
    function plot_transient_cases_v50(p; nodes=15)
        for id in vcat(keys_heating_v50, keys_cooling_v50)
            is_cool = id in keys_cooling_v50
            id_str = string(id)
            outputs, result, experiment = solve_case_v50(p, id_str; is_cooling=is_cool, nodes=nodes)
            data = is_cool ? measurements_cooling : measurements
            time = observation_time(data, id_str)
            
            p_plot = plot(title="1D_v50 Transient: Case $(id_str) ($(is_cool ? "Cooling" : "Heating"))",
            ...
            out_file = joinpath(RUNNER_TRANSIENT_DIR_v50, "transients_$(id_str)_1D_v50.png")
            savefig(p_plot, out_file)
        end
    end

    function plot_axial_profiles_v50(p; nodes=25, result_label="fitted_energy_accounting")
        for id in keys_heating_v50
            id_str = string(id)
            outputs, result, experiment = solve_case_v50(p, id_str; is_cooling=false, nodes=nodes)
            
            p_ax = plot(title="1D_v50 Axial Temperature Profile: Case $(id_str)",
            ...
            exp_z = [sensor_positions[:T8], WALL_CHAIN_POSITIONS_v50.T12, WALL_CHAIN_POSITIONS_v50.T11,
            ...
            out_file = joinpath(RUNNER_AXIAL_DIR_v50, "axial_profile_$(id_str)_$(result_label)_1D_v50.png")
            savefig(p_ax, out_file)
        end
    end

    function steady_comparison_plot_v50(steady_results; title_suffix="fitted")
        p = scatter(
            title="1D_v50 Steady Parity Comparison ($(title_suffix))",
    ...
    function write_cell_diagnostics_v50(p; nodes=25)
        for id in keys_heating_v50
            id_str = string(id)
            outputs, result, experiment = solve_case_v50(p, id_str; is_cooling=false, nodes=nodes)
            conditions, time, _, data = experimental_case_v50(id_str)
            irradiance = conditions[Io]
            delivered = absorbed_power_scale_v50(irradiance, p) * irradiance * A_frt_v50
            chi = core_source_fraction_v50(p)
            core_absorbed = chi * delivered
            perim_absorbed = (1.0 - chi) * delivered
            dx = L_v50 / length(result.z_solid)
            Trear = result.rear_reservoir_temperature[:, end]

            path = joinpath(RUNNER_AXIAL_DIR_v50, "cell_diagnostics_$(id_str)_1D_v50.csv")
            open(path, "w") do io
...
begin # optimization runner
    function run_calibration_v50(; stage=RUNNER_FIT_STAGE_v50, max_time=RUNNER_FIT_SECONDS_v50,
                                nodes=RUNNER_FIT_NODES_v50, dataset=RUNNER_CALIBRATION_DATASET_v50)
        println("[run_1D_v50] Starting Option A calibration on heating runs (stage=$(stage), dataset=$(dataset), nodes=$(nodes), max_time=$(max_time) s)...")
        res = calibrate_v50(
            nodes=nodes,
            maximum_iterations=RUNNER_FIT_ITERATIONS_v50,
            maximum_time_seconds=max_time,
            stage=stage,
            dataset=dataset,
        )
        println("[run_1D_v50] Optimization completed with status: $(res.retcode)")
        println("[run_1D_v50] Final objective: $(res.objective)")
        return (params=res.parameters, objective=res.objective, retcode=res.retcode,
                evaluations=res.evaluations)
    end
end

function main_1D_v50()
    println("============================================================================")
    println("1D_v50 Calibration and Analysis Pipeline")
    println("============================================================================")

    run_cal = runner_bool_v50("RECEIVER1D_v50_RUN_CALIBRATION", true)
    
    variant = if run_cal
        run_calibration_v50()
    else
        println("[run_1D_v50] Using baseline v50 parameter vector.")
        (params=pnew_v50, objective=calibration_loss_v50(pnew_v50),
         retcode=:Manual, evaluations=0)
    end
    result_label = run_cal ? "fitted_energy_accounting" : "baseline_repaired"

    loss_components = calibration_loss_components_v50(
        variant.params; nodes=RUNNER_FIT_NODES_v50
    )

    # Write summary
    sum_path = joinpath(RUNNER_OUTPUT_DIR_v50, "optimization_summary_1D_v50.txt")
    open(sum_path, "w") do io
        println(io, "result_label=$(result_label)")
        println(io, "objective=$(variant.objective)")
        println(io, "return_code=$(variant.retcode)")
        println(io, "evaluations=$(variant.evaluations)")
        println(io, "stage=$(RUNNER_FIT_STAGE_v50)")
        println(io, "calibration_dataset=$(RUNNER_CALIBRATION_DATASET_v50)")
        println(io, "signal_loss=$(loss_components.signal_loss)")
        println(io, "flow_slope_loss=$(loss_components.slope_loss)")
        println(io, "capacitance_regularization=$(loss_components.capacitance_regularization)")
        println(io, "power_regularization=$(loss_components.power_regularization)")
        println(io, "parameters=$(variant.params)")
    end

    # Write parameters
    param_path = joinpath(RUNNER_OUTPUT_DIR_v50, "parameters_$(result_label)_1D_v50.csv")
    write_parameters_v50(param_path, variant)

    # Compute & write metrics
    println("[run_1D_v50] Computing multi-signal metrics...")
    metrics = compute_metrics_v50(variant.params)
    write_metrics_v50(joinpath(RUNNER_OUTPUT_DIR_v50, "analysis_results_$(result_label)_1D_v50.csv"), metrics)

    # Build & write steady results
    println("[run_1D_v50] Computing steady-state results and conservation residuals...")
    steady_results = build_steady_results_v50(variant.params)
    write_steady_results_v50(joinpath(RUNNER_OUTPUT_DIR_v50, "steady_results_$(result_label)_1D_v50.csv"), steady_results)

    # Compute flow slopes
    println("[run_1D_v50] Computing flow slopes...")
    slopes = compute_flow_slopes_v50(steady_results)
    write_flow_slopes_v50(joinpath(RUNNER_OUTPUT_DIR_v50, "flow_slopes_$(result_label)_1D_v50.csv"), slopes)

    # Compute invariants
    println("[run_1D_v50] Computing invariants...")
    inv_rows = compute_invariants_v50(
        variant.params, steady_results; loss_components=loss_components
    )
    write_invariants_v50(joinpath(RUNNER_OUTPUT_DIR_v50, "invariant_summary_1D_v50.csv"), inv_rows)

    # Plotting & Diagnostics
    if RUNNER_WRITE_PLOTS_v50
        println("[run_1D_v50] Generating transient plots...")
        plot_transient_cases_v50(variant.params)

        println("[run_1D_v50] Generating axial temperature profile plots...")
        plot_axial_profiles_v50(variant.params; result_label=result_label)

        println("[run_1D_v50] Generating steady-state comparison parity plot...")
        p_steady = steady_comparison_plot_v50(steady_results; title_suffix=result_label)
        savefig(p_steady, joinpath(RUNNER_PLOT_DIR_v50, "steady_comparison_$(result_label)_1D_v50.png"))

        println("[run_1D_v50] Writing cell-by-cell axial diagnostics CSVs...")
        write_cell_diagnostics_v50(variant.params)
    end

    println("[run_1D_v50] Pipeline completed successfully. Output in: $RUNNER_OUTPUT_DIR_v50")
end
