# ============================================================================
# run_2D_v6.jl - 2D_v6 Runner with T11 Distinct Visuals & Entry Acceleration
# ============================================================================
...
begin # Directory setup
    const OUTPUT_DIR_2D_v6 = joinpath(@__DIR__, "summaries", "2D_v6")
    const PLOT_DIR_2D_v6 = joinpath(OUTPUT_DIR_2D_v6, "plots")
    const TRANSIENT_DIR_2D_v6 = joinpath(PLOT_DIR_2D_v6, "transients")
    const PROFILES_DIR_2D_v6 = joinpath(PLOT_DIR_2D_v6, "2d_profiles")
    const AXIAL_DIR_2D_v6 = joinpath(PLOT_DIR_2D_v6, "axial_profiles")
    const DIAGNOSTICS_DIR_2D_v6 = joinpath(OUTPUT_DIR_2D_v6, "diagnostics")
    
    mkpath(OUTPUT_DIR_2D_v6)
    mkpath(PLOT_DIR_2D_v6)
    mkpath(TRANSIENT_DIR_2D_v6)
    mkpath(PROFILES_DIR_2D_v6)
    mkpath(AXIAL_DIR_2D_v6)
    mkpath(DIAGNOSTICS_DIR_2D_v6)
...
function plot_steady_parity_2D(steady_results)
    p_plot = plot(
        title="2D_v6 Multi-Domain Fitted Steady-State Parity Plot",
    ...
    path = joinpath(PLOT_DIR_2D_v6, "steady_comparison_2D_v6.png")
    savefig(p_plot, path)
    return path
end
...
function plot_axial_profile_2D(sim_id, case_data)
    ...
    path = joinpath(AXIAL_DIR_2D_v6, "axial_profile_$(sim_id)_2D_v6.png")
    savefig(p_plot, path)
    return path
end
...
function plot_transient_2D(sim_id, case_data; is_cooling=false)
    ...
    p_plot = plot(
        title="2D_v6 Fitted Transient: $sim_id $(is_cooling ? "(Cooling)" : "(Heating)")",
    ...
    path = joinpath(TRANSIENT_DIR_2D_v6, "transient_$(sim_id)$(suffix)_2D_v6.png")
    savefig(p_plot, path)
    return path
end
...
function plot_2d_heatmap(sim_id, result)
    ...
    p_heat = heatmap(
        z_mm, r_mm, T_final,
        title="2D_v6 Multi-Domain Temperature Field T_s(r, z) [K] - $sim_id",
        xlabel="Axial Position z (mm)", ylabel="Radial Radius r (mm)",
        c=:inferno, aspect_ratio=:equal, clim=(minimum(T_final), maximum(T_final))
    )
    
    path = joinpath(PROFILES_DIR_2D_v6, "heatmap_2D_$(sim_id)_2D_v6.png")
    savefig(p_heat, path)
    return path
end
...
function write_parameters_2D(path, p::ModelParameters2D)
    names = (
        ...
        "phi_0", "m_rec", "front_dep_fraction", "beta_opt", "A_entry"
    )
...
begin # Main Execution Loop
    println("==========================================================================")
    println("Running 2D_v6 Multi-Domain Calibration with Entry Nu Acceleration...")
    println("==========================================================================")
    ...
    open(joinpath(OUTPUT_DIR_2D_v6, "optimization_summary_2D_v6.txt"), "w") do io
        println(io, "objective=$(calib.objective)")
        println(io, "return_code=$(calib.retcode)")
        println(io, "parameters=$(pack_parameters2D(p_fitted))")
    end
    
    write_parameters_2D(joinpath(OUTPUT_DIR_2D_v6, "parameters_fitted_2D_v6.csv"), p_fitted)
    
    println("[run_2D_v6] Computing metrics and generating figures for fitted 2D_v6 model...")
    metrics = compute_all_metrics_2D(p_fitted)
    open(joinpath(OUTPUT_DIR_2D_v6, "analysis_results_2D_v6.csv"), "w") do io
    ...
    steady_path = joinpath(OUTPUT_DIR_2D_v6, "steady_results_2D_v6.csv")
    ...
    slopes_path = joinpath(OUTPUT_DIR_2D_v6, "flow_slopes_2D_v6.csv")
    ...
    for sim_id in IDs
        ...
        write_cell_diagnostics_2D(
            joinpath(DIAGNOSTICS_DIR_2D_v6, "cell_diagnostics_$(sim_id)_2D_v6.csv"),
    ...
    println("==========================================================================")
    println("Multi-Domain 2D_v6 Calibration and Runner Execution Complete.")
