# ============================================================================
# 1D_v25.jl - Energy-accounting 2-Zone Core/Perimeter Macro Model
# ============================================================================
# v25 keeps the v19 two-zone idea but makes the energy accounting explicit:
...
#
# Parameter Vector p[1:21] in v25:
...
begin # v25 fixed constants
    const EPS_FRONT_FIXED_v25 = EPS_FRONT_FIXED_V5
    const ETA_ABS_FIXED_v25 = 1.0
    const BETA_OPT_FIXED_v25 = BETA_OPT_FIXED_V5
    const H_FRONT_FIXED_v25 = 10.0
    const PRANDTL_EXPONENT_FIXED_v25 = 1.0 / 3.0
    const STANDARD_AIR_DENSITY_v25 = 101325.0 / (287.05 * 294.25)
    const REYNOLDS_REFERENCE_v25 = 50.0
    const NU_FLOOR_v25 = 3.61
    const MEASURED_ASSEMBLY_CAPACITANCE_v25 = 301.0
    const IRRADIANCE_LEVELS_v25 = (456000.0, 304000.0, 256000.0)
    const T3_SAMPLE_POSITION_v25 = 140.0e-3
    const CAPACITANCE_REGULARIZATION_WEIGHT_v25 = 0.10
    const FLUX_SIGMA_FIXED_v25 = 30.0e-3
    const FLUX_INTEGRATION_POINTS_v25 = 161

    # Cavity and housing geometry
    const CAVITY_OUTER_RADIUS_v25 = 75.0e-3
    const METAL_THICKNESS_v25 = 18.0e-3
    const INSULATION_OUTER_RADIUS_v25 = CAVITY_OUTER_RADIUS_v25 - METAL_THICKNESS_v25
    const METAL_OUTER_RADIUS_v25 = CAVITY_OUTER_RADIUS_v25
    const CAVITY_LENGTH_v25 = 165.0e-3

    const ADAPTOR_DIAMETER_v25 = 77.6e-3
    const ADAPTOR_RADIUS_v25 = ADAPTOR_DIAMETER_v25 / 2.0
    const ADAPTOR_LENGTH_v25 = 57.0e-3
    const ADAPTOR_TUBE_RADIUS_v25 = 6.5e-3
    const ADAPTOR_RECEIVER_OVERLAP_LENGTH_v25 = 28.0e-3
    const ADAPTOR_TUBE_OVERLAP_LENGTH_v25 = ADAPTOR_LENGTH_v25 - ADAPTOR_RECEIVER_OVERLAP_LENGTH_v25
    const ADAPTOR_OVERLAP_LENGTH_v25 = ADAPTOR_RECEIVER_OVERLAP_LENGTH_v25

    const RECEIVER_EQ_RADIUS_v25 = sqrt(A_frt / pi)
    const RECEIVER_TO_CAVITY_CONDUCTANCE_PER_LENGTH_v25 =
        4.0 * pi * 0.08 / log(INSULATION_OUTER_RADIUS_v25 / RECEIVER_EQ_RADIUS_v25)
    const ADAPTOR_CONTACT_RESISTANCE_v25 =
        1.0 / (100.0 * 4.0 * w_t * ADAPTOR_OVERLAP_LENGTH_v25)
    const RECEIVER_TO_TUBE_CONDUCTANCE_v25 = 1.0 / ADAPTOR_CONTACT_RESISTANCE_v25

    # Rear tube geometry
    const REAR_TUBE_LENGTH_v25 = 150.0e-3
    const REAR_TUBE_CAVITY_LENGTH_v25 = 46.0e-3
    const REAR_TUBE_DEFAULT_NODES_v25 = 12
    const REAR_TUBE_GAS_RADIUS_v25 = ADAPTOR_TUBE_RADIUS_v25
    const REAR_TUBE_WALL_THICKNESS_v25 = 1.5e-3
    const REAR_TUBE_OUTER_RADIUS_v25 = REAR_TUBE_GAS_RADIUS_v25 + REAR_TUBE_WALL_THICKNESS_v25
    const REAR_TUBE_FLOW_AREA_v25 = pi * REAR_TUBE_GAS_RADIUS_v25^2
    const REAR_TUBE_HYDRAULIC_DIAMETER_v25 = 2.0 * REAR_TUBE_GAS_RADIUS_v25
    const REAR_TUBE_EXCHANGE_PERIMETER_v25 = 2.0 * pi * REAR_TUBE_GAS_RADIUS_v25

    const ALUMINA_DENSITY_v25 = 3900.0
    const ALUMINUM_DENSITY_v25 = 2700.0
    const ALUMINUM_HEAT_CAPACITY_v25 = 900.0
    const ALUMINUM_CONDUCTIVITY_v25 = 205.0
    const METAL_EMISSIVITY_v25 = 0.2
    const H_NAT_CAVITY_v25 = 10.0
    const INSULATION_DENSITY_v25 = 140.0
    const INSULATION_HEAT_CAPACITY_v25 = 1360.0
    const WATER_FLANGE_TEMPERATURE_v25 = 25.0 + 273.15

    const CAVITY_FELT_VOLUME_v25 = max(
        0.0,
        pi * INSULATION_OUTER_RADIUS_v25^2 * CAVITY_LENGTH_v25 -
        A_frt * L - pi * ADAPTOR_RADIUS_v25^2 * ADAPTOR_LENGTH_v25,
    )
    const CAVITY_METAL_VOLUME_v25 =
        pi * (METAL_OUTER_RADIUS_v25^2 - INSULATION_OUTER_RADIUS_v25^2) * CAVITY_LENGTH_v25 +
        pi * METAL_OUTER_RADIUS_v25^2 * METAL_THICKNESS_v25
    const CAVITY_OUTER_AREA_v25 =
        2.0 * pi * METAL_OUTER_RADIUS_v25 * CAVITY_LENGTH_v25 + pi * METAL_OUTER_RADIUS_v25^2
    const CAVITY_HEAT_CAPACITY_v25 =
        INSULATION_DENSITY_v25 * INSULATION_HEAT_CAPACITY_v25 * CAVITY_FELT_VOLUME_v25 +
        ALUMINUM_DENSITY_v25 * ALUMINUM_HEAT_CAPACITY_v25 * CAVITY_METAL_VOLUME_v25
end

begin # v25 non-uniform front-flux partition
    transverse_flux_shape_v25(x, y; sigma=FLUX_SIGMA_FIXED_v25) =
        exp(-0.5 * (x^2 + y^2) / sigma^2)

    function flux_zone_fractions_v25(; radius=CAVITY_OUTER_RADIUS_v25,
                                     receiver_half_width=w_t / 2.0,
                                     points=FLUX_INTEGRATION_POINTS_v25)
        ...
        for ix in 1:points
            ...
            for iy in 1:points
                ...
                weight = transverse_flux_shape_v25(x, y) * dx_flux^2
    ...
    const FLUX_ZONE_FRACTIONS_FIXED_v25 = flux_zone_fractions_v25()
end

# Default v25 parameter vector p[1:21]
begin # v25 parameter values and bounds
    pnew_v25 = [
        4.924815049912292,  # p[1]  A_Nu (laminar developing prefactor)
        0.5162678835987395, # p[2]  B_Re (Reynolds exponent bounded 0.0 to 0.6)
        PRANDTL_EXPONENT_FIXED_v25, # p[3]  C_Pr (= 1/3)
    ...
    fit_heat_transfer_indices_v25 = [1, 2, 7, 8, 9, 10, 11, 12, 14, 15, 16, 17, 18, 19, 20]
    fit_source_indices_v25 = [14, 15, 16, 17, 18, 19, 20]
    fit_power_scale_indices_v25 = [7, 8, 9]
    fit_radiation_indices_v25 = Int[]
end

function irradiance_level_index_v25(irradiance)
    irradiance <= 0.0 && return 0
    differences = abs.(collect(IRRADIANCE_LEVELS_v25) .- Float64(irradiance))
    return argmin(differences)
end

power_scale_index_v25(irradiance) = begin
    level_index = irradiance_level_index_v25(irradiance)
    level_index == 0 ? 0 : 6 + level_index
end

function absorbed_power_scale_v25(irradiance, p)
    index = power_scale_index_v25(irradiance)
    return index == 0 ? 1.0 : p[index]
end

core_perimeter_conductance_per_length_v25(p) = max(p[10], eps(Float64))

perimeter_heat_capacity_total_v25(p) = max(p[11], eps(Float64))

core_receiver_heat_capacity_v25(nodes) =
    sum(rho_s * Cps_f(900.0) * A_solid * (L / nodes) for _ in 1:nodes)

participating_assembly_heat_capacity_v25(p, nodes=default_nodes) =
    core_receiver_heat_capacity_v25(nodes) + perimeter_heat_capacity_total_v25(p)

perimeter_axial_conductivity_v25(T, p) =
    max(p[12], 0.0) * (property_temperature(T) / 900.0)^3

flux_receiver_fraction_v25() = FLUX_ZONE_FRACTIONS_FIXED_v25.receiver

flux_spillover_fraction_v25() = FLUX_ZONE_FRACTIONS_FIXED_v25.spillover

perimeter_spillover_capture_v25(p) = clamp(p[14], 0.0, 1.0)

perimeter_source_fraction_v25(p) =
    clamp(perimeter_spillover_capture_v25(p) * flux_spillover_fraction_v25(), 0.0, 0.80)

modeled_absorbed_receiver_power_v25(irradiance, p) =
    ETA_ABS_FIXED_v25 * absorbed_power_scale_v25(irradiance, p) *
    max(0.0, irradiance) * A_frt

core_absorbed_power_v25(irradiance, p) =
    modeled_absorbed_receiver_power_v25(irradiance, p) *
    (1.0 - perimeter_source_fraction_v25(p))

perimeter_absorbed_power_v25(irradiance, p) =
    modeled_absorbed_receiver_power_v25(irradiance, p) *
    perimeter_source_fraction_v25(p)

modeled_participating_absorbed_power_v25(irradiance, p) =
    modeled_absorbed_receiver_power_v25(irradiance, p)

perimeter_source_attenuation_v25(p) = clamp(p[15], 0.0, 300.0)

core_tube_fraction_v25(p) = clamp(p[16], 0.0, 1.0)

flange_loss_scale_v25(p) = clamp(p[17], 0.10, 20.0)

rear_core_loss_per_length_v25(p) = clamp(p[18], 0.0, 300.0)

rear_perim_loss_per_length_v25(p) = clamp(p[19], 0.0, 300.0)

rear_sink_shape_v25(p) = clamp(p[20], 0.25, 1.0)

core_axial_conduction_scale_v25(p) = clamp(p[21], 0.0, 0.125)

function rear_receiver_sink_weight_v25(z, p=pnew_v25)
    z0 = sensor_positions[:T8]
    z <= z0 && return 0.0
    s = clamp((z - z0) / max(L - z0, eps(Float64)), 0.0, 1.0)
    return s^rear_sink_shape_v25(p)
end

function axial_exponential_weights_v25(beta, nodes, dx)
...
function active_flow_fraction_v25(reynolds, p)
    phi0 = clamp(p[4], 0.1, 1.0)
    m = max(p[5], 0.0)
    ratio = max(reynolds, eps(Float64)) / REYNOLDS_REFERENCE_v25
    return clamp(phi0 * ratio^m, 0.1, 1.0)
end

function receiver_active_flow_fraction_v25(flow_lpm, Tin, p)
    flow_lpm <= 1e-12 && return 1.0
    mdot = m_dot_standard_v25(flow_lpm)
    mu = mu_f_f(Tin)
    reynolds = mdot * Dh / (A_flow * mu)
    return active_flow_fraction_v25(reynolds, p)
end

function receiver_channel_nusselt_v25(z, reynolds, prandtl, p)
    A_nu = max(p[1], 0.0)
    ...
    return max(NU_FLOOR_v25, Nu_dev)
end

m_dot_standard_v25(flow_lpm) =
    STANDARD_AIR_DENSITY_v25 * max(0.0, flow_lpm) / 60000.0

alumina_conductivity_v25(T) =
    5.5 + 34.5 * exp(-0.0033 * (property_temperature(T) - 273.15))

alumina_heat_capacity_v25(T) =
    max(500.0, (1.00446 + 1.742e-4 * property_temperature(T) -
                2.796e4 * property_temperature(T)^-2) * 1000.0)

function rear_tube_outer_radius_v25(z_rear)
    return z_rear <= ADAPTOR_TUBE_OVERLAP_LENGTH_v25 ?
           ADAPTOR_RADIUS_v25 : REAR_TUBE_OUTER_RADIUS_v25
end

function rear_tube_solid_area_v25(z_rear)
    outer_radius = rear_tube_outer_radius_v25(z_rear)
    return pi * (outer_radius^2 - REAR_TUBE_GAS_RADIUS_v25^2)
end

function rear_tube_cavity_conductance_per_length_v25(z_rear, p)
    z_rear > REAR_TUBE_CAVITY_LENGTH_v25 && return 0.0
    outer_radius = min(rear_tube_outer_radius_v25(z_rear), 0.98 * INSULATION_OUTER_RADIUS_v25)
    return 2.0 * pi * 0.08 / log(INSULATION_OUTER_RADIUS_v25 / outer_radius)
end

function rear_tube_flange_conductance_per_length_v25(z_rear, Ttube, p)
    z_rear <= REAR_TUBE_CAVITY_LENGTH_v25 && return 0.0
    tube_wall_resistance =
        log(REAR_TUBE_OUTER_RADIUS_v25 / REAR_TUBE_GAS_RADIUS_v25) /
        (2.0 * pi * alumina_conductivity_v25(Ttube))
    aluminum_resistance =
        log(METAL_OUTER_RADIUS_v25 / REAR_TUBE_OUTER_RADIUS_v25) /
        (2.0 * pi * ALUMINUM_CONDUCTIVITY_v25)
    return flange_loss_scale_v25(p) / (tube_wall_resistance + aluminum_resistance)
end

function cavity_loss_to_ambient_v25(Tcavity, ambient)
    return H_NAT_CAVITY_v25 * CAVITY_OUTER_AREA_v25 * (Tcavity - ambient) +
           METAL_EMISSIVITY_v25 * sigma_sb * CAVITY_OUTER_AREA_v25 *
           (Tcavity^4 - ambient^4)
end

function rear_tube_capacity_v25(Ttube, z_rear, dx_rear)
    return ALUMINA_DENSITY_v25 * alumina_heat_capacity_v25(Ttube) *
           rear_tube_solid_area_v25(z_rear) * dx_rear
end

function rear_tube_htc_v25(Twall, Tgas, flow_lpm, Tin)
    ...
    mu = mu_f_f(Tfilm)
    kf = kf_f(Tfilm)
    reynolds = mdot * REAR_TUBE_HYDRAULIC_DIAMETER_v25 / (REAR_TUBE_FLOW_AREA_v25 * mu)
    prandtl = cp * mu / kf
    nusselt = reynolds < 2300.0 ? 3.66 : 0.023 * reynolds^0.8 * prandtl^0.4
    return nusselt * kf / REAR_TUBE_HYDRAULIC_DIAMETER_v25
end

function gas_profile_v25!(Tg, Qgas, hcell, Qgas_rear, hrear,
    ...
    mdot_total = m_dot_standard_v25(flow)
    phi_act = receiver_active_flow_fraction_v25(flow, Tin, p)
    ...
    for i in eachindex(Tcore)
        ...
        nusselt = receiver_channel_nusselt_v25(z[i], reynolds_active, prandtl, p)
    ...
    for j in eachindex(Ttube)
        ...
        cp = cpf_f(Tfilm)
        hrear[j] = rear_tube_htc_v25(Ttube[j], Tg[inlet_index], flow, Tin)
        UA = hrear[j] * REAR_TUBE_EXCHANGE_PERIMETER_v25 * dx_rear
...
function receiver_rhs_v25!(du, u, time, p, operating, z, dx, core_weights,
                          perim_weights,
                          z_rear, dx_rear,
    ...
    gas_profile_v25!(Tg, Qgas, hcell, Qgas_rear, hrear,
                    Tcore, Ttube, time, p, operating, z, dx, z_rear, dx_rear)
    fill!(du, 0.0)
    ...
    for i in 1:(nodes - 1)
        ...
        Qcond_core =
            core_axial_conduction_scale_v25(p) * kface * A_solid *
        ...
        k_perim = perimeter_axial_conductivity_v25(Tface_perim, p)
    ...
    scale = absorbed_power_scale_v25(irradiance, p)
    Qreceiver_absorbed = ETA_ABS_FIXED_v25 * scale * irradiance * A_frt
    f_perim_source = perimeter_source_fraction_v25(p)
    Qcore_solar = Qreceiver_absorbed * (1.0 - f_perim_source)
    Qperim_solar = Qreceiver_absorbed * f_perim_source

    G_cp = core_perimeter_conductance_per_length_v25(p)
    radial_cavity_conductance = RECEIVER_TO_CAVITY_CONDUCTANCE_PER_LENGTH_v25
    ...
    for i in 1:nodes
        ...
        rear_sink_weight = rear_receiver_sink_weight_v25(z[i], p)
        Qrear_core_sink =
            rear_core_loss_per_length_v25(p) * dx * rear_sink_weight *
            (Tcore[i] - WATER_FLANGE_TEMPERATURE_v25)
        Qrear_perim_sink =
            rear_perim_loss_per_length_v25(p) * dx * rear_sink_weight *
            (Tperim[i] - WATER_FLANGE_TEMPERATURE_v25)
    ...
    Qfront_perim = H_FRONT_FIXED_v25 * A_frt * (Tperim[1] - ambient) +
                   EPS_FRONT_FIXED_v25 * sigma_sb * A_frt * (Tperim[1]^4 - ambient^4)
    
    f_core_tube = core_tube_fraction_v25(p)
    Qreceiver_tube_core = RECEIVER_TO_TUBE_CONDUCTANCE_v25 * f_core_tube * (Tcore[end] - Ttube[1])
    Qreceiver_tube_perim = RECEIVER_TO_TUBE_CONDUCTANCE_v25 * (1.0 - f_core_tube) * (Tperim[end] - Ttube[1])
    ...
    for j in 1:(rear_nodes - 1)
        ki = alumina_conductivity_v25(Ttube[j])
        kj = alumina_conductivity_v25(Ttube[j + 1])
        kface = 2.0 * ki * kj / (ki + kj)
        Aface = 0.5 * (rear_tube_solid_area_v25(z_rear[j]) + rear_tube_solid_area_v25(z_rear[j + 1]))
    ...
    for j in 1:rear_nodes
        G_cavity = rear_tube_cavity_conductance_per_length_v25(z_rear[j], p)
        G_flange = rear_tube_flange_conductance_per_length_v25(z_rear[j], Ttube[j], p)
        Q_cavity = G_cavity * dx_rear * (Ttube[j] - Tcavity)
        Q_flange = G_flange * dx_rear * (Ttube[j] - WATER_FLANGE_TEMPERATURE_v25)
        dTtube[j] -= Qgas_rear[j] + Q_cavity + Q_flange
        du[cavity_index] += Q_cavity
    end

    Qcavity_ambient = cavity_loss_to_ambient_v25(Tcavity, ambient)
    du[cavity_index] -= Qcavity_ambient

    cell_volume = A_solid * dx
    perim_capacity_cell = perimeter_heat_capacity_total_v25(p) / nodes
    for i in 1:nodes
    ...
    for j in 1:rear_nodes
        dTtube[j] /= max(rear_tube_capacity_v25(Ttube[j], z_rear[j], dx_rear), eps(Float64))
    end
    du[cavity_index] /= max(CAVITY_HEAT_CAPACITY_v25, eps(Float64))
    return nothing
end

function receiver_ode_v25!(dTs, Ts, context, time)
    receiver_rhs_v25!(
...
function simulate_v25(p, operating, save_times;
                      initial_temperature=Tamb, nodes=default_nodes,
                      rear_nodes=REAR_TUBE_DEFAULT_NODES_v25,
    ...
    length(p) == 21 || throw(ArgumentError("1D_v25 expects 21 parameters"))
    all(isfinite, p) || throw(ArgumentError("all model parameters must be finite"))
    all(lb_full_v25[i] <= p[i] <= ub_full_v25[i] for i in eachindex(p)) ||
        throw(ArgumentError("1D_v25 parameters are outside declared bounds"))

    time = Float64.(save_times)
    dx = L / nodes
    dx_rear = REAR_TUBE_LENGTH_v25 / rear_nodes
    ...
    perim_weights = axial_exponential_weights_v25(perimeter_source_attenuation_v25(p), nodes, dx)
    ...
    problem = ODEProblem(receiver_ode_v25!, u_initial, (time[1], time[end]), context)
    solution = solve(
        problem, solver; saveat=time, save_everystep=false, dense=false,
        reltol=reltol, abstol=abstol, dtmax=dtmax, tstops=time,
    )
    successful_retcode(solution) || error("1D_v25 ODE solve failed with retcode $(solution.retcode)")
    ...
    for output_index in eachindex(time)
        ...
        gas_profile_v25!(Tg, Qgas, hcell, Qgas_rear, hrear, Tcore_now, Ttube_now,
...
begin # v25 experimental interface
    const WALL_CHAIN_POSITIONS_v25 = (
        T8=sensor_positions[:T8],
        T12=sensor_positions[:T9],
        T11=sensor_positions[:T10],
    )

    function measured_initial_perim_profile_v25(data, simulation_id)
        z8 = WALL_CHAIN_POSITIONS_v25.T8
        z12 = WALL_CHAIN_POSITIONS_v25.T12
        z11 = WALL_CHAIN_POSITIONS_v25.T11
    ...
    function measured_initial_core_profile_v25(data, simulation_id)
    ...
    function measured_initial_rear_profile_v25(data, simulation_id, p)
        core_profile = measured_initial_core_profile_v25(data, simulation_id)
        perim_profile = measured_initial_perim_profile_v25(data, simulation_id)
        f_core_tube = core_tube_fraction_v25(p)
        T_receiver_exit =
        ...
        return function (z_rear)
            fraction = clamp(z_rear / REAR_TUBE_LENGTH_v25, 0.0, 1.0)
            return T_receiver_exit + fraction * (T_tube_exit - T_receiver_exit)
        end
    end

    function experimental_case_v25(simulation_id; is_cooling=false)
    ...
    function gas_at_position_v25(result, position)
        z = result.z_gas
    ...
    function core_at_v25(result, position)
        z = result.z_solid
    ...
    function perim_at_v25(result, position)
        z = result.z_solid
    ...
    function extract_outputs_v25(result, p)
        T8 = perim_at_v25(result, sensor_positions[:T8])
        T12_perim = perim_at_v25(result, WALL_CHAIN_POSITIONS_v25.T12)
        T11_perim = perim_at_v25(result, WALL_CHAIN_POSITIONS_v25.T11)
        T9_core = core_at_v25(result, sensor_positions[:T9])
        T10_core = core_at_v25(result, sensor_positions[:T10])
        Tf = gas_at_position_v25(result, T3_SAMPLE_POSITION_v25)
    ...
    function solve_case_v25(p, simulation_id; is_cooling=false,
                           nodes=default_nodes,
                           rear_nodes=REAR_TUBE_DEFAULT_NODES_v25,
                           solver=Rodas5P(autodiff=AutoFiniteDiff()),
                           reltol=1e-6, abstol=1e-7, dtmax=30.0)
        conditions, time, experiment, data = experimental_case_v25(
        ...
        result = simulate_v25(
            p, operating, time;
            initial_temperature=measured_initial_core_profile_v25(data, simulation_id),
            initial_perim_temperature=measured_initial_perim_profile_v25(data, simulation_id),
            initial_rear_temperature=measured_initial_rear_profile_v25(data, simulation_id, p),
            initial_cavity_temperature=T2[1],
            nodes=nodes, rear_nodes=rear_nodes, solver=solver,
            reltol=reltol, abstol=abstol, dtmax=dtmax,
        )
        outputs = extract_outputs_v25(result, p)
        return outputs, result, experiment
    end
end

begin # v25 objective helpers
    const ORDERING_SCALE_v25 = 100.0
    const ORDERING_WEIGHT_v25 = 1.00
    const TIMING_WEIGHT_v25 = 0.15
    const SLOPE_WEIGHT_v25 = 0.25
    const COOLING_UPTURN_WEIGHT_v25 = 10000.0

    function normalized_slope_mse_v25(model, experiment)
    ...
    function timing_penalty_v25(time, model, experiment)
        duration = max(time[end] - time[1], 1.0)
        return ((get_t90_v3(time, model) - get_t90_v3(time, experiment)) / duration)^2
    end

    function signal_loss_v25(time, model, experiment)
        return normalized_mse_v3(model, experiment) +
               SLOPE_WEIGHT_v25 * normalized_slope_mse_v25(model, experiment) +
               TIMING_WEIGHT_v25 * timing_penalty_v25(time, model, experiment)
    end

    function cooling_upturn_loss_v25(model)
    ...
    function ordering_loss_v25(model, experiment)
        model_offsets = (
        ...
        return mean(abs2, collect(model_offsets) .- collect(experiment_offsets)) / ORDERING_SCALE_v25^2
    end

    function capacitance_regularization_v25(p; nodes=15)
        capacitance = participating_assembly_heat_capacity_v25(p, nodes)
        return CAPACITANCE_REGULARIZATION_WEIGHT_v25 *
               ((capacitance - MEASURED_ASSEMBLY_CAPACITANCE_v25) / MEASURED_ASSEMBLY_CAPACITANCE_v25)^2
    end

    function loss_cases_v25(p, keys; is_cooling=false, nodes=15)
        total = 0.0
        for simulation_id in keys
            try
                ...
                total += mean(
                    signal_loss_v25(result.time, model[:, j], experiment[:, j])
                    for j in 1:7
                )
                if !is_cooling
                    total += ORDERING_WEIGHT_v25 * ordering_loss_v25(model, experiment)
                else
                    total += COOLING_UPTURN_WEIGHT_v25 * cooling_upturn_loss_v25(model)
                end
            catch err
    ...
    function loss_heating_v25(p, keys=sim_key_heat; nodes=15)
        regularization = capacitance_regularization_v25(p; nodes=nodes)
        return loss_cases_v25(p, keys; is_cooling=false, nodes=nodes) + regularization
    end

    function loss_cooling_v25(p, keys=sim_key_cool; nodes=15)
        regularization = capacitance_regularization_v25(p; nodes=nodes)
        return loss_cases_v25(p, keys; is_cooling=true, nodes=nodes) + regularization
    end

    function calibrate_v25(; nodes=15, maximum_iterations=100, maximum_time_seconds=360.0)
        p0 = copy(pnew_v25)
        idx_fit = fit_heat_transfer_indices_v25
        function obj_free(theta)
            p_full = copy(p0)
            p_full[idx_fit] .= theta
            return loss_heating_v25(p_full; nodes=nodes) + loss_cooling_v25(p_full; nodes=nodes)
        end
