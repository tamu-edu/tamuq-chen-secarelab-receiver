...
$$P_{\rm del} = f\,G_0 A_{\rm frt},\qquad f_{456} = 1.340,\quad f_{304} = 1.577,\quad f_{256} = 1.108.$$

The factors are group means of the model-free steady energy closure on the present data, f = [Q_gas + K_loss(T̄_w − T_amb)]/(G₀A_frt), evaluated with the heating-identified loss conductance K_loss = 0.164 W K⁻¹, which is the value appropriate to the hot steady states these factors are applied to (Section 4.7). No thermal model enters their derivation. Evaluating the same closure with the cooling-identified K_loss = 0.096 W K⁻¹ gives the lower bracket 1.049, 1.226, 0.842; that difference, −22 to −24%, is carried as a one-sided systematic band, so every efficiency reported on the delivered basis is a lower bound within the bracket and the cooling-basis value would be 28–32% higher.
...
The campaign spans Re = 23–94, Pr = 0.684–0.689, Gz_L = 0.17–0.70, ε = 0.57–0.78, NTU = 0.85–1.51, apparent Nu = 0.028–0.212, and PR = 370–1911 kJ kg⁻¹ on the delivered-power basis. The per-run values and their uncertainties are tabulated in the supplementary data.
...
6. The measured gas output exceeds the nominal aperture-power accounting by 22 ± 4% at the highest flow. A model-free energy closure, built only on the measured gas enthalpy and the independently identified loss conductance, localizes the delivery error by lamp configuration (+34% at 456, +58% at 304 and +11% at 256 kW m⁻² on the hot-state loss, and 22–24% lower on the cold-state bracket). With the closure's per-configuration factors applied, all efficiencies are sub-unity and rise linearly with flow, with slopes agreeing across the three groups to within 7%, to a maximum recovery of 77% of delivered power; the receiver is flow-limited, not exchange-limited, throughout the campaign.
...
**Table 2.** Identified constants with 95% intervals: Nusselt law (prefactor and exponent), ε* per flux level, Λ₁₀₇(Re) coefficients, C_eff and K_loss from both datasets, monolith capacitance from measured mass, and delivered-power factors with their closure brackets.
