...
Two consequences of that gap are of immediate practical concern, and both are examined directly in this work. The first concerns closure. Assembly-level receiver models are routinely closed with a laminar duct or Graetz-type channel correlation applied to the nominal geometry, on the assumption that the channels behave as an ensemble of identical, uniformly bounded ducts. Whether that assumption survives one-sided irradiation of a monolith has never been tested against an assembly-level measurement. The second concerns validation. Models of this class are validated predominantly against steady-state temperatures and an outlet gas temperature, quantities that are integral over the whole field. Whether agreement on those quantities is sufficient evidence that a model has the internal physics right has not been tested either.
...
Two general statements follow from these results and are argued in Sections 5.1 and 5.6. The first is that a Graetz-type channel closure on nominal geometry fails structurally for a one-sided irradiated monolith of this class: the apparent exchange is one to two orders of magnitude smaller than that closure predicts, and it grows steeply with Reynolds number where the closure is essentially constant, so the discrepancy cannot be absorbed into a coefficient. The second is that agreement with steady-state temperatures and an outlet gas temperature is not sufficient to validate a receiver model: a model is shown here that reproduces the outlet gas temperature to within a fraction of a kelvin and the measured effectiveness envelope to within 0.02, and would pass a conventional validation, while misplacing the interior wall temperature by more than 50 K and failing to reproduce the volumetric inversion at all.
...
The scope of this conclusion should be stated carefully. It is established for a monolith irradiated on one face by a beam that overfills it, held in insulation, and operated at Re = 23–94; and it is a statement about the composite closure — a channel correlation applied to nominal geometry — not about the channel correlation in isolation, which may well be correct for the channels that are actually flowing and uniformly bounded. The campaign contains no run with symmetric or uniform irradiation, so one-sided irradiation is not varied and cannot be established as the cause. What the data do establish is that the conditions a Graetz-type correlation presumes — an ensemble of identical channels with a common, well-defined wall boundary condition — are not met in an assembly of this kind, and that the resulting error is one to two orders of magnitude and is not removable by fitting. That is sufficient to caution against this closure wherever monoliths are irradiated from one side, as they are in solar-thermal and solar-chemical conversion, while leaving the attribution to a future campaign in which the irradiation symmetry is varied.
