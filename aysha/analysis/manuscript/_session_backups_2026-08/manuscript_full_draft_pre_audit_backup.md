...
The factors are group means of the model-free steady energy closure on the present data, f = [Q_gas + K_loss(T̄_w − T_amb)]/(G₀A_frt), evaluated with the heating-identified loss conductance K_loss = 0.165 W K⁻¹, which is the value appropriate to the hot steady states these factors are applied to (Section 4.7). No thermal model enters their derivation. Evaluating the same closure with the cooling-identified K_loss = 0.096 W K⁻¹ gives the lower bracket 1.049, 1.226, 0.842; that difference, −22 to −24%, is carried as a one-sided systematic band, so every efficiency reported on the delivered basis is a lower bound within the bracket and the cooling-basis value would be 25–32% higher.
...
The campaign spans Re = 23–94, Pr = 0.684–0.690, Gz_L = 0.18–0.70, ε = 0.57–0.78, NTU = 0.85–1.51, apparent Nu = 0.028–0.212, and PR = 370–1911 kJ kg⁻¹ on the delivered-power basis. The per-run values and their uncertainties are tabulated in the supplementary data.
...
The mechanism follows from the front-face energy balance. The face loses heat by thermal emission and by convective quenching from the entering gas; both the absorbed flux and the quench term scale essentially linearly with irradiance at fixed geometry, so the balance that determines the location of the temperature maximum reduces to a condition on the gas-side recovery, that is, on ε. The small but statistically significant reduction of ε* at the lowest flux (0.626 against 0.671, nine times its uncertainty interval) is consistent with the growing relative weight of the flux-independent parasitic loss identified in Section 4.7: a fixed loss removes proportionally more from the face balance when the input is small, allowing the inversion at a slightly lower recovery. The crossings also collapse on the delivered specific energy. On the nominal basis they lie at PR* = 795, 670, and 958 kJ kg⁻¹, a 1.43-fold spread; on the corrected delivered basis of Section 3.5 they lie at PR* = 1066, 1056, and 1062 kJ kg⁻¹, a spread of 1%, and evaluating the alternative loss bracket instead gives 834, 822, and 807 kJ kg⁻¹, a spread of 3%. The collapse is therefore robust to the systematic band on f. It is, however, a consistency result rather than an independent criterion, since the delivered basis is itself constructed from a per-group energy closure on the same runs; the effectiveness collapse, which contains no power quantity at all, remains the primary statement.
