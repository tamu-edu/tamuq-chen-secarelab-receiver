...
*Draft manuscript sections (Methods, Results, Discussion). Reproducible pipeline: `../exp_analysis/exp_analysis.py`, `../exp_analysis/dimensionless_analysis.py`, `../exp_analysis/uncertainty_analysis.py`; data products `dimensionless_groups.csv`, `eigenvalue_verification.csv`, `uncertainty_per_run.csv`, `delivered_power_check.csv`; figures fig1–fig7. All temperature-based quantities (ε, Nu, Λ, ε\*, C_eff, K_loss, master curves) use nominal measured aperture fluxes only and involve no correction factors. Quantities in which power enters explicitly (η, PR) are shown on nominal and provisional delivered-power bases using per-configuration working factors f = 1.336, 1.374, 0.786 (456, 304, 256 kW/m²). These factors are not independently validated optical quantities; M6/R6 states their conditional status and nominal-basis values are retained.*
...
3. **Delivered-power statement.** Report the flux-map integral over the 19 × 19 mm² face and the spillage fraction onto the cavity front. Until those measurements are available, Section R6 establishes an apparent energy-accounting discrepancy, not a uniquely identified absorbed-power correction. Present `Io A_frt` as the nominal basis and every corrected efficiency as conditional.
4. **Gas and interior thermocouple bias.** T3 sits at the receiver exit and off-axis, but its transfer relation to bulk outlet enthalpy is not independently characterized; tube/rear radiation, finite response, and local sampling remain possible biases. T9/T10 view the surrounding channel walls and are radiation-biased *toward the solid temperature*; the consequence (their gas-side depression readings are lower bounds) is used explicitly in Section R5.
...
The nominal accounting `G₀A_frt` is insufficient to close the apparent outlet-energy balance (R6). The discrepancy can contain delivered-power error, spillage-mediated heating, and T3 observation bias. Where power enters a reported quantity explicitly — gas thermal efficiency η and power ratio PR — the following per-configuration values are retained as provisional working factors:

$$f_{456} = 1.336,\qquad f_{304} = 1.374,\qquad f_{256} = 0.786,$$

so that `P_work = f G₀A_frt`. The values originate from an earlier 1D inverse calibration in which power scales were fitted alongside other effective parameters. A T3-based steady closure, `f_closure = [Q_gas + K(T̄_w - T_amb)]/(G₀A_frt)`, gives a similar lamp-group ordering, but it uses the same experimental campaign and inherits the unresolved relation between T3 and bulk outlet enthalpy. It is therefore a consistency comparison, not independent validation. The ±8% propagation captures per-case scatter in the working factors but not their full structural uncertainty. The factors affect none of the temperature-only results (ε, Nu, Λ, ε*, C_eff, K_loss, or master curves), and nominal-basis power quantities remain in the data products so a future flux/spillage measurement can replace them without redoing the temperature reduction.
...
The apparent gas thermal efficiency η = Q_gas/(G₀A_frt), with `Q_gas = ṁ∫c_p dT` from T_amb to measured T3, rises near-linearly with flow within each flux group (≈ 6%/slpm) and reaches η = 1.23 ± 0.04 at the highest flow (E72; the neighboring E73 gives 0.99 ± 0.03). The Monte Carlo interval includes the ±3% HFG calibration, thermocouple class errors, and metering uncertainty; alternative metering conventions raise η. However, T3 is a local outlet-region thermocouple rather than an independently shielded bulk-enthalpy measurement. Its off-axis position removes direct source view but does not eliminate tube/rear radiation, local sampling, or dynamic probe bias. `Q_gas` and η are therefore explicitly apparent quantities.

The excess over `G₀A_frt` establishes an energy-accounting discrepancy. Beam overfill and spillage-mediated heating are plausible and independently motivated routes, but the current measurements do not uniquely separate them from T3 observation bias. Thus `G₀A_frt` is the reproducible nominal basis, while the magnitude of any delivered-power correction remains provisional. The monotone rise of apparent η with flow is robust; its interpretation as a delivered-power recovery fraction is conditional on the same outlet and power assumptions.
...
| G₀ [kW/m²] | f (K_cool) | f (K_heat) | earlier 1D fit |
|---:|---:|---:|---:|
| 456 | 1.05 | 1.34 | 1.34 |
| 304 | 1.23 | 1.58 | 1.37 |
| 256 | 0.84 | 1.11 | 0.79 |

The earlier 1D fit and the T3-based closure give a similar lamp-group ordering, but they are not independent: both use this campaign, and each contains unresolved model or observation physics. Their agreement supports retaining the three working factors for a conditional delivered-power view; it does not validate the values as optical calibration constants. Residual flow dependence (about +0.03 to +0.06 per slpm) further shows that the factors compensate physics beyond lamp delivery. The grouping, rather than the third digit, is the only robust comparison.

**Gas thermal efficiency on a provisional delivered basis.** With the M6 working factors applied (±8% scatter propagated), every efficiency falls below unity (fig3, right panel: solid symbols working delivered basis, hollow nominal):

$$\eta_{\rm del} = 0.30\text{–}0.67\ (456),\qquad 0.21\text{–}0.90\ (304),\qquad 0.32\text{–}0.87\ (256\ \mathrm{kW\,m^{-2}}),$$

with Monte Carlo uncertainties ±0.06–0.08 dominated by factor scatter. The slopes with flow remain, but the absolute recovery values are conditional on the provisional factors and T3 interpretation. The nominal-basis η is retained as the factor-free apparent measurement that motivates a future direct flux/spillage and outlet-enthalpy audit.
...
**D5 — The delivered-power discrepancy.** η > 1 against the nominal input is a strong reason to publish both the nominal calculation and its unresolved assumptions. It shows that `G₀A_frt`, the T3-to-bulk relation, or both are incomplete for system energy accounting. Beam overfill and spillage are plausible contributors, but the present dataset does not quantify them independently. Every efficiency is therefore labeled by basis, and the delivered-basis map remains provisional pending direct flux/spillage integration and a bulk outlet-enthalpy measurement.
...
6. The T3-based apparent gas output exceeds nominal aperture-flux × frontal-area input at the highest flow, exposing an unresolved system energy-accounting discrepancy. A T3-based closure and earlier 1D fit give similar lamp-group ordering, but neither independently identifies delivered power. Factors `f = 1.336/1.374/0.786` are retained only as a provisional alternative basis; absolute delivered-basis efficiency remains conditional on T3 transfer physics and the unmeasured flux/spillage distribution.
