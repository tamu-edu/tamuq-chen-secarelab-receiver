...
**Table 1.** Campaign summary. G is the aperture irradiance determined in Section 3.5; q the total standard volumetric flow; Re the per-channel Reynolds number at mean gas temperature (Section 3.2).

| G [kW m⁻²] | q [sL min⁻¹] | Re | runs |
|---:|---|---|---|
| 523 | 7.1–15.3 | 34–73 | 5 heating, 1 cooling (10.5) |
| 408 | 4.5–18.3 | 23–94 | 5 heating |
| 238 | 4.5–13.9 | 25–79 | 5 heating + 1 replicate, 2 cooling (6.6, 4.5) |
...
$$\bar T_w = 0.252\,T_8 + 0.350\,T_{12} + 0.398\,T_{11},$$
where the weights are trapezoid coefficients for the probe positions with constant extrapolation to the end faces. The interior probes T9 and T10 are used only for the nonequilibrium metrics of Section 3.2 and enter no other quantity.
...
Finally, the heating transients are tested for similarity. Each record is normalized as θ*(t) = (T − T₀)/(T_ss − T₀) and time is rescaled as t* = t(ε ṁc_p + K_loss)/C_eff, the reciprocal of the identified slow eigenvalue, using only constants identified from the cooling data. The quality of the collapse is quantified by the coefficient of variation of t* at θ* = 0.5 across the fifteen runs.

### 3.5 Aperture irradiance

The nominal power accounting, `P_nom = G₀A_frt` with `A_frt = 3.61 cm²`, does not close against the T3-based outlet estimate at the highest flow (Section 4.6). The beam overfills the 3.61 cm² frontal face, so a radiometric flux map over the aperture measures what arrives on that area, not the power delivered to the receiver system. Because neither total delivered power nor the T3-to-bulk transfer relation is independently measured, nominal-basis η and PR are reported as apparent quantities and the following per-configuration factors are shown only as a provisional alternative basis:

The delivered power is obtained from a steady energy balance on the receiver itself, using two quantities measured in this work and nothing else:

$$P = Q_{\rm gas} + K_{\rm loss}\,(\bar T_w - T_{\rm amb}),$$

with `Q_gas` the gas enthalpy rise from the outlet probe and `K_loss` the loss conductance identified from the transients (Section 4.7). These working factors originate from an earlier one-dimensional inverse calibration in which power scales were fitted alongside other effective parameters. A T3-based steady closure, `f = [Q_gas + K_loss(\bar T_w - T_amb)]/(G₀A_frt)`, gives a similar lamp-group ordering (Section 4.6), but it uses the same campaign and inherits the unresolved relation between T3 and bulk outlet enthalpy. The comparison is therefore not an independent validation. The factors affect no temperature-based result (ε, Nu, Λ, ε*, C_eff, K_loss, or the master curves), and all nominal-basis values are retained so direct future flux/spillage and outlet-enthalpy measurements can replace them without repeating the temperature reduction.

### 3.6 Uncertainty analysis

A Monte Carlo propagation with 4000 realizations treats instrument errors as systematic within a run and independent between instruments. Each thermocouple carries a bias drawn from a normal distribution with standard deviation 1.1 K (class accuracy ±2.2 K) plus a steady-window drift of standard deviation 0.5 K; each of the four flow controllers carries a bias of standard deviation 0.025 sL min⁻¹; property tabulations are perturbed by ±2% (μ, k) and ±1% (c_p), correlated across runs; and the aperture irradiance across the K_loss bracket, sampled uniformly over the −9% one-sided band with a further ±3% for the flow residual of the closure. For every realization the complete analysis chain is recomputed, including all per-run groups, the Nusselt-law fit, the three inversion crossings, and the eigenvalue regressions with each λ additionally perturbed by its fit standard error. Nonlinear amplifications, in particular dNTU = dε/(1 − ε), are thereby captured without linearization. Reported uncertainties are Monte Carlo standard deviations, and parameter intervals are 95% percentile ranges.
...
The campaign spans Re = 23–94, Pr = 0.684–0.689, Gz_L = 0.17–0.70, ε = 0.57–0.78, NTU = 0.85–1.51, apparent Nu = 0.028–0.212, and PR = 311–1630 kJ kg⁻¹. The per-run values and their uncertainties are tabulated in the supplementary data.
...
$$\varepsilon^* = 0.673 \pm 0.003,\quad 0.673 \pm 0.003,\quad 0.628 \pm 0.005\qquad (523,\ 408,\ 238\ \mathrm{kW\,m^{-2}}),$$

with 95% intervals of ±0.005–0.010. The volumetric inversion therefore occurs when the gas stream recovers approximately two thirds of the wall excess temperature, independent of irradiance over the 2.2-fold flux range tested.

The mechanism follows from the front-face energy balance. The face loses heat by thermal emission and by convective quenching from the entering gas; both the absorbed flux and the quench term scale essentially linearly with irradiance at fixed geometry, so the balance that determines the location of the temperature maximum reduces to a condition on the gas-side recovery, that is, on ε. The small but statistically significant reduction of ε* at the lowest flux (0.628 against 0.673, nine times its standard deviation) is consistent with the growing relative weight of the flux-independent parasitic loss identified in Section 4.7: a fixed loss removes proportionally more from the face balance when the input is small, allowing the inversion at a slightly lower recovery. The crossings do not collapse as tightly on any power coordinate. Evaluated at the same crossing flows, the specific energy delivered per unit mass is PR* = 852, 714, and 1166 kJ kg⁻¹, a 1.6-fold spread. The specific-energy coordinate is therefore far looser than the effectiveness coordinate: it spans 63% of its smallest value at the crossing, against 7% for the three effectiveness thresholds. The inversion is governed by the gas-side operating point, not by the specific energy supplied.
...
$$C_{\rm eff} = 301 \pm 24\ \mathrm{J\,K^{-1}},\qquad K_{\rm loss} = 0.097 \pm 0.011\ \mathrm{W\,K^{-1}}\qquad (r^2 = 0.96).$$

The heating experiments provide fifteen further eigenvalues through the late approach to steady state, spanning a wider flow range (Figure 5, right). Regression of these gives C_eff = 287 ± 27 J K⁻¹ and K_loss = 0.119 ± 0.018 W K⁻¹ (r² = 0.90). The two datasets share no measurements, one being recorded with the lamps on and the other with the lamps off, yet they return the same capacitance within 5%, well inside either interval. The identification is therefore confirmed by eighteen eigenvalue measurements in total, and no additional experiments are required to establish C_eff.
...
**Table 2.** Constants identified in this work, with 95% Monte Carlo intervals over the instrument, property and metering uncertainties of Section 3.6. No fitted transport model enters any entry.

| Quantity | Value | 95% interval | Note |
|---|---|---|---|
| Nu_app prefactor a  (Nu = a Re^b) | 0.000311 | [2.87e-04, 3.35e-04] | log-log r2 = 0.971, n = 15 |
| Nu_app exponent b | 1.443 | [1.436, 1.450] | n = 15 |
| eps* at 523 kW/m2 | 0.673 | [0.668, 0.678] |  |
| eps* at 408 kW/m2 | 0.673 | [0.667, 0.680] |  |
| eps* at 238 kW/m2 | 0.628 | [0.618, 0.638] |  |
| C_eff from cooling [J/K] | 301.0 | [254, 348] |  |
| K_loss from cooling [W/K] | 0.097 | [0.074, 0.119] |  |
| C_eff from heating [J/K] | 287.0 | [235, 340] |  |
| K_loss from heating [W/K] | 0.119 | [0.084, 0.155] |  |
| Monolith capacitance at 600 K [J/K] | 42.0 | - |  |
| Monolith capacitance at 900 K [J/K] | 46.8 | - |  |
| Lambda_107 intercept | 0.0379 | [0.0319, 0.0439] | r2 = 0.897 |
| Lambda_107 slope [1/Re] | 0.000823 | [7.12e-04, 9.36e-04] | reported as a lower bound |
| aperture irradiance G [kW/m2], 523 configuration | 523.0 | [480, 523] | secant-to-tangent K bracket, one-sided systematic |
| aperture irradiance G [kW/m2], 408 configuration | 408.0 | [374, 408] | secant-to-tangent K bracket, one-sided systematic |
| aperture irradiance G [kW/m2], 238 configuration | 238.0 | [216, 238] | secant-to-tangent K bracket, one-sided systematic |

### 4.8 Similarity of the heating transients

Rescaling each heating run by the single time scale identified from the cooling data, t* = t(ε ṁc_p + K_loss)/C_eff, with no parameter fitted to any heating transient, collapses the fifteen normalized histories (Figure 6). The half-rise points are t* = 0.20 with a coefficient of variation of 14% for the energy-weighted wall temperature, and t* = 0.63 with 9% for the outlet gas, across a 4.0-fold range of flow and a 2.2-fold range of flux.
...
The calibrated model of Section 3.7 reproduces the measured effectiveness envelope run by run (Figure 7, left). Over the fifteen heating runs the mean absolute deviation in ε is 0.018 and the maximum is 0.039, with a bias of +0.004; the modelled range 0.583–0.777 lies within the measured 0.574–0.781, and NTU agrees to a mean absolute deviation of 0.055. Mean steady residuals are +14 K at 11 mm, −21 and −40 K at 58 mm, +16 and +29 K at 107 mm, +20 K on the insulation and −0.1 K on the outlet gas. At the level of the operating point, therefore, the assembly-scale exchange embodied in ε and NTU is captured.

The volumetric inversion is not (Figure 7, right). Reducing the model output by the same convention used in Section 4.4 — regression of the indicator I_vol = T12 − T8 on flow to locate the crossing, then evaluation of the effectiveness regression there — the model inverts at ε* = 0.787, 0.764 and 0.727 for the 238, 408 and 523 kW m⁻² groups, against the measured 0.628, 0.673 and 0.673. It therefore requires 0.05–0.16 more gas-side recovery to move the wall peak inside than the receiver does, equivalently 1.2 to 3.6 times more flow (q* = 13.7, 16.3 and 13.5 against the measured 3.7, 10.3 and 11.1 sL min⁻¹ of Section 4.4). The residual flux dependence is not merely retained but reversed: the measured threshold rises weakly with irradiance while the modelled one falls. The amplitude fails as well. Where the measured indicator reaches +58, +60 and +59 K at the top of each group, the model reaches −9.5, +0.7 and +5.5 K in the same order; it never develops an interior wall maximum of more than a few kelvin, and in the lowest-flux group it does not develop one at all.
...
### 5.5 Determining the irradiance in a spilling configuration
...
The closure used here needs no additional instrument: it requires the gas enthalpy rise, which is already measured, and the loss conductance, which the cooling transients already identify. Its weakness is that the gas enthalpy appears on both sides, so the absolute level of η is a partition between the gas and the loss path rather than a ratio to an independently measured input, and its residual flow dependence warns against reading the per-configuration factors as purely optical constants. For future campaigns the direct remedy is to integrate the flux map over the actual receiver face and quantify the spillage fraction separately, which would make the determination independent of the receiver's own energy balance and would turn the present internal consistency into an external check.
...
Fourth, two quantities are not identifiable from this instrumentation, and this bounds what any model of this class can be asked to deliver. The absorbed power is one of them. Because the aperture subtends only about 7% of the beam, the power reaching the assembly is the product of a source scale and the captured fraction of the remaining 93%, and the temperature field responds only to that product; the two factors are individually unconstrained. The energy closure of Section 3.5, which is obtained algebraically from the gas enthalpy and the identified loss conductance with no thermal model in the loop, is therefore the only defensible route to that quantity, and any model-derived value of it should be read as a restatement of it. The receiver conductance UA(Re) is the second: no unique identification exists without an independent bulk outlet enthalpy or a verified transfer function for the outlet probe, and admissible parameter sets form a ridge rather than a basin. Both limits are properties of the measurement, not of the optimizer, and they carry a direct design consequence for campaigns of this type: the flux map should be integrated over the receiver face with the spillage fraction quantified separately, and a bulk outlet enthalpy should be measured independently of any single probe.
...
2. The volumetric inversion obeys a flux-independent operating-point criterion: the wall-temperature peak moves into the receiver when the gas-side effectiveness exceeds ε* ≈ 0.66 ± 0.03, resolved to ±0.005 at each of three aperture irradiances spanning a 2.2-fold range (523, 408 and 238 kW m⁻²).
...
**Figure 3.** Left: wall inversion metric T12 − T8 versus flow with zero crossings. Center: interior-minus-wall temperature differences at 58 and 107 mm. Right: gas thermal efficiency versus flow at the three aperture irradiances. *(fig3_inversion_gaps_eta.png)*
...
**Figure 6.** Collapse of the fifteen normalized heating transients in rescaled time t* = t(ε ṁc_p + K_loss)/C_eff for the energy-weighted wall temperature and the outlet gas temperature. *(fig6_master_curve.png)*
