...
The absorber is a monolithic silicon carbide honeycomb (Figure 1d) with a ten-by-ten array of square channels of 1.5 mm side and 0.40 mm web thickness, giving a 1.9 mm pitch, a 19.0 mm square frontal face of area 3.61 cm², an open frontal fraction of 0.6233 and an axial length of 137 mm. The measured mass is 40 g, which yields an effective bulk density of 2,147 kg m⁻³, consistent with a recrystallized SiC of roughly one third internal porosity; this is the value used for every capacitance estimate below. The geometry is of the same family as the extruded SiC honeycombs and catalyst-carrier substrates characterized in the solar literature [4,11] and modelled in our earlier work [16], with a channel size between the two samples of Fend and co-workers (2.18 and 1.43 mm) and a comparable web thickness.

The monolith is enclosed in an aluminum cavity (142 mm in diameter; Figure 1b) [Sarwar, 2015] held in place with alumina felt insulation,  and irradiated through a wider circular aperture by a seven-lamp high-flux solar simulator (Figure 1a), each lamp a 6 kW xenon short-arc unit; the apparatus and its optical characterization are described in detail elsewhere [22,23]. Air is metered by four Aalborg GFC17 thermal mass-flow controllers (0-5 sL min⁻¹), all carrying air, referenced to 21.1 °C and 1 atm, and the total flow is their sum. We opted for four controllers instead of one (0-20 sL min⁻¹) to reduce the flow-rate uncertainty, because their manufacturer specification is ±1.0% of full scale over the whole 0 to 100% range with ±0.5% full-scale repeatability. The factory calibration is gas-specific, however three out of four where calibrated for different gases, because the receiver runs air rather than the calibration gas, each unit was calibrated in place against a bubble flowmeter, which replaces the manual's gas-conversion factor with a directly measured one and leaves a residual proportional to reading. Both terms are carried in the uncertainty model of section 3.4. Per-unit readings span 1 to 100% of full scale, so neither term is negligible against the other. Because these are mass-flow devices reporting standard volumetric units, the mass flow follows from the reference density ρ_std = 1.1996 kg m⁻³ rather than from the density at the prevailing ambient temperature.

### 2.2 Instrumentation

Figure 2 shows the process flow diagram along with its instrumentation, mainly the nine thermocouples (type-N by TCDirect UK), characterizing the measuring campaign. Three sheathed probes (OD 1.5 mm, length 150 mm) contact the monolith side wall at axial positions of 11, 58 and 107 mm, denoted T8, T12 and T11. Two probes (OD 0.5 mm, length 500 mm) sit in the interior, flow-exposed region at 58 and 107 mm, denoted T9 and T10. One probe, T3 (OD 1.5 mm, length 300 mm), sits in the exit plenum and reports the outlet gas temperature. One further probe, T2 (OD 1.5 mm, length 150 mm), sits in the surrounding insulation and is used in section 4.6 to assess thermal stationarity; the wall-to-insulation drop T12 − T2 is 310 to 770 K, so the insulation resistance dominates radial transfer. The remaining two probes T1 (also type-N) and T5 (type-K by Omega UK) give the cavity ambient, whose mean is T_amb. The distinction between the wall chain and the interior pair is critical for this work: sections 4.3 and 5.1 show that the two report materially different temperatures at the same depth, and that which is taken to be the solid temperature changes every identified coefficient in this work. That the interior probe reading is potentially itself perturbed by the radiation field is established for similar geometries [19], which is why we treat the reference-temperature choice as a systematic rather than a detail. Thermocouple tolerance is treated per IEC 60584 for type-N as the greater of 1.5 K and 0.004|t|, which at the front probe near 1200 K is ±4.4 K. Installation error for the three sheathed wall probes, comprising contact resistance against the monolith and radiative exchange with the cavity, is not quantified, and we report it for future studies rather than omitting their data.

Differential pressure across the monolith is monitored with a Keller PD33X of ±200 mbar range and ±0.1% full-scale accuracy, hence an accuracy floor of ±0.2 mbar. Section 4.7 shows this to be the same order as the entire expected monolith pressure drop, so the channel pressure is practially unusable as installed; we report it for future studies and propose a corrective action because pressure drop is the quantity that discriminates between the two mechanisms left open by section 5.2.
...
> **Figure 1 placeholder** — drop the apparatus schematic at `figures/fig1_apparatus.png` and this slot fills automatically.

**Figure 1.** The apparatus, receiver geometry and thermocouple positions. The monolith is held in an insulated cavity and irradiated through a circular aperture; the side-wall chain T8, T12 and T11 contacts the monolith at 11, 58 and 107 mm, the interior flow-exposed pair T9 and T10 sits at 58 and 107 mm, T3 reports the outlet gas in the exit plenum, and T2 sits in the surrounding insulation.
...
![Figure 2](figures/fig2_steady_field.png)

**Figure 2.** Steady wall-chain and outlet gas temperatures against flow at the three lamp configurations. The response is strongly graded in depth: the front-face wall falls steeply with flow while the outlet gas is nearly invariant, so a single solid temperature cannot represent the field.
...
![Figure 3](figures/fig3_assembly_limitation.png)
...
![Figure 4](figures/fig4_inversion_ltne.png)

**Figure 4.** (a) The side-wall inversion indicator $T_{12}-T_8$ against flow at the three configurations, with the located crossings. (b) The same data against effectiveness, collapsing at $\varepsilon^\ast \approx 0.65$. (c) The wall-to-interior disequilibrium indices at 58 and 107 mm against Reynolds number, rising linearly within every flux group.
...
![Figure 5](figures/fig5_transient_identification.png)

**Figure 5.** (a) Cooling decays of all six receiver probes at three flows, normalised on the initial excess; the slow mode is shared. (b) The slow eigenvalue against gas advective conductance $\varepsilon\dot m c_p$ for the three identifications, the primary matched-effectiveness cooling points filled and the pooled-effectiveness abscissa shown open. (c) Similarity collapse of the fifteen heating transients on $\tau = C_{\rm eff}/(\varepsilon\dot m c_p + K_{\rm loss})$, wall in grey and outlet gas in red.
