...
This work, first, brings a new and extensive experimental (section 2) dataset of eighteen transients — fifteen heating and three cooling — each recorded at nine axial and radial locations of a lab-scale receiver, for various gas flow rates and irradiance levels. Second, presents an analysis framework (section 3) that reduces the measurements to dimensionless groups, identifies the receiver's constants from the transient eigenstructure, and tests the structure of reduced-order formulations against the measured exponent. This structural test is not an inverse calibration: instead of fitting a coefficient's value, it asks only which way the transfer-unit count moves as the flow rises. Any formulation whose volumetric conductance is a fixed function of axial position requires that count to fall in proportion to 1/ṁ, whereas the measurement has it rising; a disagreement in sign cannot be absorbed by recalibration, so the conclusion holds whatever coefficient is chosen. The framework also quantifies how much the identified coefficients depend on the analyst's choice of reference sensor, and thus how much information a conventional validation exercise carries. These yield (section 4) a dimensionless characterization of the installed SiC honeycomb receiver; the demonstration of the wall-to-interior disequilibrium based on the measurements rather than assumptions; identification of effective capacitance and loss conductance from transient eigenvalues with sensor-selection sensitivity quantified; a similarity rescaling on which the fifteen heating transients collapse. Finally, the work utilizes the experimental data and analysis framework to support two methodological findings (section 5) — that validation against a gas outlet temperature plus one or two solid temperatures is largely uninformative for this class of receiver, and that a uniform-temperature or uniform-flux coefficient evaluated on the fixed channel geometry cannot reproduce its flow response at any calibration.
...
Two qualifications matter. First, the ε–NTU relation is exact only for a wall at fixed temperature, whereas here the wall spans up to 321 K between T8 and T11 and carries an interior maximum in ten of the fifteen runs, the onset moving to higher flow as irradiance rises — 6.6, 9.0 and 10.5 sL min⁻¹ at 256, 304 and 456 kW m⁻² — so the inversion is approximate, with the bias quantified in section 4.2. Second, since h_app is defined through NTU, Nu(Re) is algebraically tied to ε(Re):

$$\frac{d\ln Nu}{d\ln Re} = 1 + \frac{d\ln NTU}{d\ln Re} + \frac{d\ln[c_p/k]}{d\ln Re},$$

the property group being c_p/k rather than a Prandtl number because c_p is evaluated at the bulk mean and k at the film temperature. A fitted Nu(Re) exponent is therefore not independent corroboration of the measured effectiveness range, which is what enables the structural test of section 5.2: comparing that exponent against channel-correlation values near 0.3–0.6 compares unlike objects, since the relevant null here is −1 for the transfer-unit count, not 0.5 for Nu.
...
All calculations were performed in cPython (version 3.11.15) utilizing mainly the packages numpy (version 2.4.6) and pandas (version 2.3.3.). In particular for the air properties, the package CoolProp (version 8.0.0) was utilized. However, instead of direct calls over the 4000-realization Monte Carlo passes, which would have resulted in several milltion function calls, the respective properties were evaluated once onto a 1K grid over 200-1600 K and then interpolated at the required calls.
...
The steady field against flow at each configuration is shown in Figure 3, and the response is strongly graded in depth. Front-face wall temperature falls steeply with flow, at −33.2, −24.3 and −20.8 K per sL min⁻¹ at 456, 304 and 256 kW m⁻² with r² of at least 0.97; the mid-depth wall falls at half to two-thirds that rate; the rear wall barely falls, at −1.1, −1.9 and −5.2 K per sL min⁻¹ with r² of 0.07, 0.12 and 0.76. Outlet gas temperature is nearly flow-invariant at the two higher fluxes (+0.6 and −0.1 K per sL min⁻¹, r² ≤ 0.03) and falls at the lowest, at −3.1 K per sL min⁻¹. At 456 kW m⁻² the outlet gas spans 26 K across the flow range against 264 K for the front-face wall, a ratio of ten; at 304 and 256 kW m⁻² the spans are 51 K against 331 K and 36 K against 194 K.

![Figure 3](figures/fig3_steady_field.png)
...
**Figure 4.** (a) Apparent Nusselt number against nominal channel Reynolds number, with the fully developed laminar square-duct reference $Nu_{H2}=3.09$ and the band spanning $Nu_T=2.98$ to $Nu_{H1}=3.61$. Solid lines are the primary common-slope grouping with a prefactor per configuration; the dashed line is the pooled single-prefactor fit. (b) Profile-corrected transfer-unit count against Reynolds number (filled) with the isothermal-wall identity $-\ln(1-\varepsilon)$ shown open, against the $Re^{-1}$ requirement of a conductance that is a fixed function of axial position (dashed).


$$Nu_{\rm app} = (3.10 \pm 0.12)\times10^{-4}\, Re_{\rm nom}^{1.443}, \qquad r^2 = 0.971,\ n = 15,$$

with pooled regression standard error ±0.069 and instrumental Monte Carlo term ±0.003; the regression term exceeds the instrumental one roughly twentyfold, so residual dispersion is physical and structured, not metrological or random. Fitted separately per irradiance configuration the exponent is 1.440, 1.473 and 1.522 at 256, 304 and 456 kW m⁻² (r² ≥ 0.9996 each), so pooled scatter reflects a flux-ordered offset plus a weak monotonic trend in the exponent. A common-slope model with a separate prefactor per configuration gives exponent 1.470, regression SE ±0.011, instrumental term ±0.003, r² = 0.9994, and prefactors 3.20, 2.70 and 2.55 ×10⁻⁴ in the same order; we report this grouped model as the primary correlation alongside the pooled fit, being better determined and the honest representation of data acquired at three discrete irradiance levels.
...
![Figure 6](figures/fig6_transient_identification.png)

**Figure 6.** (a) Cooling decays of all six receiver probes at three flows, normalised on the initial excess; the slow mode is shared. (b) The slow eigenvalue against gas advective conductance $\varepsilon\dot m c_p$ for the three identifications, the primary matched-effectiveness cooling points filled and the pooled-effectiveness abscissa shown open. (c) Similarity collapse of the fifteen heating transients on $\tau = C_{\rm eff}/(\varepsilon\dot m c_p + K_{\rm loss})$, wall in grey and outlet gas in red.
...
|---|---|---|---|---|---|
| $Nu_{\rm app}$ prefactor $a$ (pooled) | 3.11$\times10^{-4}$ | 0.12$\times10^{-4}$ | [2.87, 3.36] | $\times10^{-4}$ | 15 steady runs |
| $Nu_{\rm app}$ exponent, pooled | 1.443 | 0.004 | [1.435, 1.451] | – | instrumental MC; regression SE $\pm$0.069, $r^2$=0.971 |
| $Nu_{\rm app}$ exponent, grouped (primary) | 1.470 | 0.004 | [1.461, 1.477] | – | instrumental MC; regression SE $\pm$0.011, $r^2$=0.9994 |
| $N_{\rm prof}$ exponent (primary) | +0.341 | 0.004 | [0.333, 0.347] | – | instrumental MC; regression SE $\pm$0.040; fixed-$Nu$ requirement -1.000 |
...
| Inversion marker $\varepsilon^*$, 304 kW m$^{-2}$ | 0.655 | 0.003 | [0.650, 0.661] | – | operational marker under the adopted wall convention; see §5.1 |
| Inversion marker $\varepsilon^*$, 256 kW m$^{-2}$ | 0.642 | 0.003 | [0.635, 0.648] | – | operational marker under the adopted wall convention; see §5.1 |
| $\Lambda_{107}$ slope [$Re^{-1}$] | 8.51$\times10^{-4}$ | 0.35$\times10^{-4}$ | [7.83, 9.21] | $\times10^{-4}$ | common slope, per-flux intercepts |
| $C_{\rm eff}$, cooling matched-$\varepsilon$ (primary) | 276 | 37 | [221, 367] | J K$^{-1}$ | $n$=3, $r^2$=0.986 |
| $C_{\rm eff}$, cooling pooled-$\varepsilon$ | 299 | 41 | [238, 401] | J K$^{-1}$ | $r^2$=0.964 |
| $C_{\rm eff}$, joint 18 eigenvalues | 269 | 30 | [221, 338] | J K$^{-1}$ | $r^2$=0.899 |
| $C_{\rm eff}$, heating deep probes | 281 | 36 | [226, 365] | J K$^{-1}$ | 122 J K$^{-1}$ if all six probes used |
| $K_{\rm loss}$, cooling matched-$\varepsilon$ (primary) | 0.080 | 0.023 | [0.046, 0.135] | W K$^{-1}$ | secant conductance |
...
![Figure 7](figures/fig7_under_instrumentation.png)

**Figure 7.** (a) The apparent Nusselt correlation recomputed against five different solid reference probes from the same fifteen runs: the prefactor spans a factor of 28.9 and the exponent 1.09 to 1.84. (b) The effective capacitance from the same eighteen transients under four defensible analysis choices, with the fit-window range and the bare monolith capacitance marked.
...
Every derived table, uncertainty estimate and figure input in this paper and in the supplementary material is produced by one script, `receiver_reduction.py`, from the raw logger files with no hand-entered measured values; the figures are then drawn from those outputs by a second script, `make_figures.py`, which writes Figures 3 to 7. Figures 1 and 2 are not computed: Figure 1 is a photographic montage of the assembly and Figure 2 a process and instrumentation diagram, both supplied as source files in `figures/`, from which `make_figures.py` derives only the rendered copy of Figure 1. Both scripts are seeded, so a rerun on the same software stack reproduces the archived outputs; the archive was produced with Python 3.11.16, NumPy 2.4.6, SciPy 1.17.1, pandas 2.3.3 and Matplotlib 3.11.1, and the multi-start optimizer of section 5.2 is the one step whose last digits may differ across BLAS builds.
