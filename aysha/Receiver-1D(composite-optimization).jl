...
# ╔═╡ d0b7f2ae-18f3-11ee-0712-15e5c459fe0a
using MethodOfLines, ModelingToolkit, DomainSets, OrdinaryDiffEq, NonlinearSolve, PlutoUI, DifferentialEquations, Plots, XLSX, Statistics, Interpolations, Optim, DifferentialEvolutionMCMC, Symbolics

# ╔═╡ d5fdd534-2be7-4b8b-86c8-f583ec2551fd
begin 
...
# ╔═╡ 08f55655-caaf-49ab-b5fa-6c9bbb67f660
begin 
	md" I0: $(@bind I0 Slider(0.0:0.01:700.0, 296.0, true))"
	
end 

# ╔═╡ 4f087512-40a9-43b5-b0f9-e95a26700a83
begin
#solid
#using exp. 028
...
q= qlpm/(1000*60) #m3/s
V= q/(4e-6*100) #m/s - using the area of a single channel (2x2mm2)
...
# ╔═╡ 4563e652-8d11-4eb7-9ff4-62f8b9f57cdd
V

# ╔═╡ fa3bfc0f-d3b0-4f61-b169-229226898127
# begin
...
# t_max = 2000 # Experiment 28

# nc = 20
# x_num = range(x_min, x_max, length = nc)
# dx = (x_max - x_min) / (nc - 1)
# keff= (0.82 * ((1.52e-11 * (T(t, x)^3) - 4.86e-8 * (T(t, x)^2) + 1.02e-4 * T(t, x) - 3.93e-3) / 1000) +(1 - 0.82)*(52000 * exp(-1.24e-5 * T(t, x)) / (T(t, x) + 437)) / 1000)

# eq =  keff* Dxx(T(t, x)) + I0 * exp(-2300 * T(t, x)) - (3.018 * exp(-0.00574 * T(t, x)) + 0.8063 * exp(-0.0008381 * T(t, x))) * Cpf * V* Dx(T(t, x)) ~ ρs * Cps * Dt(T(t, x))

#  bcs = [T(t_min, x) ~ Ts,
#         - keff* Dx(T(t, 	x_max)) ~ 0,
#         -keff * Dx(T(t, x_min)) ~ (-(ϵ * σ * ((T(t, x_min)^4) - (Ts)^4)) + Q - h * (T(t, x_min) - Ts))]
...
# ╔═╡ a6fd3497-c059-4f18-a664-adcefab087b5
begin
#Exp 28
...
# ╔═╡ bbecfe16-1eda-44f9-bc95-738128488f0b
begin
	# Parameters, variables, and derivatives
	@parameters t x
	@variables T(..)
	...
	# ODE parameters
	tspan = (t_min, t_max)
	keff_val = 0.7 # initial value, will be updated
	
	# Define the symbolic expressions for the equations
	function eq1(T_val, keff_val, t)
	    eq = keff_val* Dxx(T_val) +
	         I0 * exp(-2300 * T_val) -
	         (3.018 * exp(-0.00574 * T_val)) + 0.8063 * exp(-0.0008381 * T_val) * Cpf * V * Dx(T_val) - ρs * Cps * Dt(T_val)
	    return eq
	end

# Define a function representing the differential equation
function ode_system!(du, u, p, t)
    du[1] = u[1] - t  # Example differential equation (u' = u - t)
end
...
# Solve the ODE
sol = solve(prob, Tsit5())  # Tsit5() is an ODE solver method

# Access the solution
t_values = sol.t
u_values = sol.u[1]  # Extract the first component of the solution

# You can now work with the time and solution values as needed.
	# Define the objective function
	function objective_function(keff_val)
    prob = ode_problem(keff_val)
    
    # Solve the ODE system with the updated keff
    sol_ = solve(prob, saveat=1)
    T_simulated = sol_.u
    
    # Interpolate experimental data to match the ODE solver time points
    itp = interpolate((xc_data,), y1c_data, Gridded(Linear()))
    interpolated_data = itp(sol_.t)
    
    # Extract the first vector from T_simulated
    T_simulated_vector = T_simulated[1][:]
    
    # Calculate the difference between the two vectors
    difference = sum((T_simulated_vector .- interpolated_data).^2)
    
    return difference
	end

	
	# ODE problem function
	function ode_problem(keff_val)
	    prob = ODEProblem(ode_system!, [Ts], tspan, [keff_val])
	    return prob
	end
end

# ╔═╡ b87e4c58-6e41-427e-8335-9734d3a4b269
begin
Q2=I0*exp(-1000*x)
end

# ╔═╡ c30b9f4c-3025-41ab-b0f0-79c6ada8e363
begin
	
	# To get the dimensions (shape) of T_simulated
	sol_ = solve(prob, saveat=1)
	T_simulated = sol_.u
	size_T_simulated = size(T_simulated)
	
	# To get the dimensions (shape) of interpolated_data
	itp = interpolate((xc_data,), y1c_data, Gridded(Linear()))
	interpolated_data = itp(sol_.t)
	size_interpolated_data = size(interpolated_data)
	
	# Print out the dimensions
	println("Dimensions of T_simulated: ", size_T_simulated)
	println("Dimensions of interpolated_data: ", size_interpolated_data)
end

# ╔═╡ 596116f2-4bf0-45ad-b6bf-92659f3814d2
begin
# Check if a variable is a vector
function is_vector(var)
    return typeof(var) == Vector
end
# Calculate the difference between simulated and interpolated experimental data
if is_vector(T_simulated) && is_vector(interpolated_data)
    difference = sum((T_simulated[1, :] .- interpolated_data).^2)
else
    println("T_simulated and/or interpolated_data is not a vector.")
    # Handle the case where one or both of them are not vectors
end
end

# ╔═╡ 83491ace-b01d-436b-941d-a814372c6136
begin 
println(typeof(T_simulated))
println(typeof(interpolated_data))
end

# ╔═╡ ec539940-d60f-4979-adca-4e6f105949f8
begin
keff_range = 0.74:0.02:0.9

# Calculate the differences for each keff value
differences = [objective_function(keff_val) for keff_val in keff_range]

# Find the index of the minimum difference
min_difference_index = argmin(differences)

# Get the corresponding optimized keff value
optimized_keff = keff_range[min_difference_index]

# Print the optimized keff value
println("Optimized keff: ", optimized_keff)

# Solve the ODE system with the optimized keff
prob_optimized = ode_problem(optimized_keff)
sol_optimized = solve(prob_optimized, saveat=1)
sol_optimized_u = sol_optimized[1, :]  # Use vec to convert to a vector

# Interpolate experimental data to match the optimized ODE solver time points
itp_optimized = interpolate((xc_data,), y1c_data, Gridded(Linear()))
interpolated_optimized_data = itp_optimized(sol_optimized.t)

# Plot the temperature profile as a function of time
plot(sol_optimized.t, sol_optimized_u, xlabel="Time (s)", ylabel="Temperature (K)", label="T1 (Simulated)", linewidth=2)
plot!(sol_optimized.t, interpolated_optimized_data, label="Experimental Data", linewidth=2)
title!("Optimized Temperature Profile vs Time")
end

# ╔═╡ 374790c8-894f-43ec-9451-bfffe0b94f27
begin
# Debugging: Print dimensions of arrays
println("Dimensions of sol_optimized.t: ", size(sol_optimized.t))
println("Dimensions of sol_optimized_u: ", size(sol_optimized_u))
println("Dimensions of interpolated_optimized_data: ", size(interpolated_optimized_data))
end

# ╔═╡ 6f19c855-fe02-40c7-b772-d41e2ad3e42a
# begin
# plot(
# 	sol_.t, 
# 	sol_[T(t,x)][:,1], 
...
# ╔═╡ 93ad40e1-8bd3-4710-8fe3-ff6327fcb5e6
# begin
...
# x__T8= 0.01 # 10 mm
# x__T3=0.13 # 130 mm
# x__T9= 0.104 #104 mm - T9 and T12
# x__T10=0.056 #56 mm - T10 and T11
# time_point = 2000
# idx_time_point = argmin(abs.(sol.t .- time_point))  # Find the index closest to the specified time point

# # Define the specific spatial positions you want to plot
# x_positions = [x__T8, x__T3, x__T9, x__T10]

# # Collect the temperature solution at the specified spatial positions for the specified time point
# T_exp_at_time_positions = [
#     sol(t, x_pos)[idx_time_point] for x_pos in x_positions
# ]

# # Create a 1D plot of T(x) at the specified time point and positions
# plot(
#     x__num,
#     T_exp_at_time_array,
#     xlabel = "Position x (m)",
...
#     linewidth = 2,
#     ylim = (y_min, y_max),
#     title = "Temperature Profile at t = $time_point s"
# )

# scatter!(
#     x_positions,
#     T_exp_at_time_positions,
#     label = ["T3 - gas", "T9 - solid, T12 - gas", "T10 - solid, T11 - gas"],
# )
# end

# ╔═╡ 1b77c67e-11e0-4c0e-9a4b-5c519eaca9aa
# begin
# #Access the temperature solution at a specific time point 
# y_min = 400.0 #K
# y_max = 1600.0 #K
...
# #x__num=[x__T8, x__T3, x__T9, x__T10]
...
# plot(x_num, T_exp_at_time_array, xlabel="Position x (m)", ylabel="Temperature (K)", label="T8 - solid", linewidth=2, ylim=(y_min, y_max), xlim=(x__min, x__T8), title="Temperature Profile at t = $time_point s")
...
#     x_num, 
#     T_exp_at_time_array,  
#     label="T9 - solid, T12 - gas",
#     xlim=(x__min, x__T9))
# scatter!(
#     x_num, 
#     T_exp_at_time_array,  
#     label="T10 - solid, T11 - gas",
#     xlim=(x__min, x__T10))
# end

# ╔═╡ 66e2c319-4b55-43c9-896d-ab00eb43d9c9
# begin
#     Access the temperature solution at a specific time point
#     y_min = 400.0
#    y_max = 2000.0
#    time_point = 2000
#    idx_time_point = argmin(abs.(sol.t .- time_point))

#     Vary the length (x range) for the plot
#    x__min = 0.0
#    x__max = 0.01
#    num_points = 100
#    x__num = LinRange(x__min, x__max, num_points)

#     Collect the temperature solution at all spatial points for the specified time point
#    T_exp_at_time_array = [sol[T(t,x)][idx_time_point, i] for i in 1:size(sol[T(t,x)], 2)]  # Make sure to use the correct size here
...
DifferentialEvolutionMCMC = "~0.7.6"
DomainSets = "~0.6.7"
...
julia_version = "1.8.4"
manifest_format = "2.0"
project_hash = "d1b6491bdb7a629a7cd02e1255e97cff2ad863e7"
...
git-tree-sha1 = "319b26d5751af3127108b48e87b74db14f375846"
...
git-tree-sha1 = "8622e18479e5e686956410112920ed53ea7e5251"
...
git-tree-sha1 = "f58e4f2a32559a88d52990b8ca286cbd57ddc8d0"
