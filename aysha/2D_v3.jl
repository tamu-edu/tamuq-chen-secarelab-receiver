# ============================================================================
# 2D_v3.jl - 2D Axisymmetric Continuum Model with Experimental Cooling t0 Initialization
# ============================================================================
# Key Enhancements in v3:
#   1. Cooling initial state u_0 is built directly from raw experimental t0 data
#      (T8(0), T12(0), T11(0), T9(0), T10(0), T3(0), T2(0)) instead of using the
#      simulated heating end state u_hot.
#   2. Churchill-Chu buoyant plume natural convection at front face h_front(T).
#   3. Temperature-dependent alumina felt insulation conductivity k_felt(T).
#   4. Preferential core channel flow partitioning u_z(r).
# ============================================================================

module Receiver2D_v3
...
# Spatial interpolation of initial state u_0 from raw experimental t0 values
function build_initial_state_2D(grid::DiscretizationGrid2D, p::ModelParameters2D, t0_sensors::Dict{Symbol, Float64}, T_ambient::Float64)
    ...
    for j in 1:nz
        ...
        # Axial interpolation for core (r=0) and perimeter (r=R_rec)
        if z_val <= 34.5e-3
        ...
        for i in 1:nr_total
            ...
            if r_val <= R_rec
                # Quadratic radial transition between core and perimeter
                frac_r = (r_val / R_rec)^2
                T_s0[i, j] = (1.0 - frac_r) * T_core_z + frac_r * T_perim_z
            else
                # Insulation & casing radial decay to T2 and ambient
    ...
    # Rear exit tube initial axial profile
    for j in 1:nz_rear
...
const LB_2D = [0.5, 0.10, 0.001, 0.10, 5.0, 0.01, 100.0, 0.40, 0.40, 0.30, 0.40, 0.00, 0.00, 0.10, 0.00]
...
function calibrate2D(solve_fn; heating_cases=("E67", "E76", "E80"),
    ...
    function counted_objective(theta, p_data)
        ...
        if isfinite(val)
            println("[calibrate2D v3] Eval $(evals[]): loss = $(round(val, digits=4))")
            flush(stdout)
        end
        return val
    end
    ...
    println("[calibrate2D v3] Starting NLopt.LN_BOBYQA parameter optimization...")
