# ============================================================================
# 2D_v5.jl - Multi-Domain 2D Model with Calibrated Radial Decoupling & Power Balance
# ============================================================================
# Key Physics & Numerical Enhancements in v5:
#   1. Radial Thermal Conductivity Scale Decoupling chi_r ∈ [0.003, 0.018]
#      maintaining distinct core-perimeter radial temperature gradients (T9 vs T12, T10 vs T11).
#   2. Net Power Delivery Scale Optimization f_scale ∈ [0.60, 0.90]
#      supplying exact equilibrium thermal power to match steady-state temperatures.
#   3. Balanced Loss Function Weighting (NMSE primary + 0.25*Shape + 0.10*Time).
#   4. Refined Non-Uniform Axial Mesh (fine Δz = 1.8 mm near front face).
# ============================================================================

module Receiver2D_v5
...
Base.@kwdef struct SolidProperties2D
    ...
    radial_conductivity_scale::Float64 = 0.010   # Effective radial conductivity scale (k_r_eff ~ 1 W/m/K)
...
Base.@kwdef struct HeatTransferParameters2D
    ...
    reynolds_exponent::Float64 = 0.380          # Reynolds exponent B_Re
...
Base.@kwdef struct OpticalParameters2D
    ...
    front_deposition_fraction::Float64 = 0.35    # Front surface absorption fraction
...
# ============================================================================
# Multi-Domain 2D Grid Discretization Struct (Refined Non-Uniform Axial Mesh)
# ============================================================================
...
function build_grid2D(p::ModelParameters2D)
    ...
    # Refined Non-Uniform Axial Mesh: Fine Δz = 1.8 mm near front face z ∈ [0, 15 mm]
    z_faces = zeros(nz + 1)
    z_faces[1] = 0.0
...
# Decoupled radial conductivity bounds chi_r ∈ [0.003, 0.018]
# and calibrated power scale bounds [0.55, 1.20]
const LB_2D = [0.5, 0.10, 0.003, 0.10, 5.0, 0.01, 100.0, 0.55, 0.55, 0.40, 0.40, 0.00, 0.10, 0.10, 0.00]
const UB_2D = [6.0, 0.70, 0.018, 3.00, 35.0, 0.40, 500.0, 1.20, 1.20, 1.00, 1.00, 0.60, 0.60, 5.00, 0.50]
    ...
    for (sim_id, is_cool) in vcat([(id, false) for id in heating_cases], [(id, true) for id in cooling_cases])
        try
            ...
            for j in 1:7
                ...
                # Balanced NMSE primary + 0.25*shape + 0.10*t90
                total_loss += (nmse + 0.25 * shape + 0.10 * t90)
            end
...
function calibrate2D(solve_fn; heating_cases=("E67", "E76", "E80"),
    ...
    function counted_objective(theta, p_data)
        ...
        if isfinite(val)
            println("[calibrate2D v5] Eval $(evals[]): loss = $(round(val, digits=4))")
            flush(stdout)
        end
        return val
    end
    ...
    println("[calibrate2D v5] Starting NLopt.LN_BOBYQA parameter optimization...")
