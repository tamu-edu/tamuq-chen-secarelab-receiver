# ============================================================================
# 2D_v8.jl - Corrected-Geometry Conservative 2D Axisymmetric Macro-ECM
# ============================================================================
# Corrections relative to v1-v7:
...
function build_initial_state_2D(grid::DiscretizationGrid2D, p::ModelParameters2D, t0_sensors::Dict{Symbol, Float64}, T_ambient::Float64)
    ...
    function axial_sensor_profile(z_val, T_mid, T_deep)
        ...
        elseif z_val <= 58.0e-3
            frac = (z_val - 11.0e-3) / (58.0e-3 - 11.0e-3)
    ...
    enforce_sample!(perim_profile, 11.0e-3, T8)
...
function energy_rate_ledger2D(u::AbstractVector, p::ModelParameters2D,
    ...
    expected_denergy_dt = solar.deposited - gas_receiver - gas_rear -
...
function sensor_predictions2D(result::SimulationResult2D)
    ...
    T8 = _sample_solid(result, r_perim, 11.0e-3; receiver_only=true)
...
function calibrate2D(solve_fn; heating_cases=("E67", "E76", "E80"),
    ...
    function counted_objective(theta_fit, p_data)
        ...
        if isfinite(val)
            println("[calibrate2D v8] Heating-Only Eval $(evals[]): loss = $(round(val, digits=4))")
            flush(stdout)
        end
        return val
    end
    ...
    println("[calibrate2D v8] Starting Heating-Only Parameter Optimization...")
